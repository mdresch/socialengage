...
## Author rights — a standing check, not a one-off

Requested directly by Menno, 2026-08-06, after this role's own investigation into a content-authorship/redistribution-consent requirement (parked under Social Selling in `docs/future-subsystems.md` rather than forced into a speculative ADR — see that entry for the full reasoning). The commitment behind that outcome should be applied as a recurring check, not treated as closed once:

**Whenever a proposed ADR, story, or design touches content that has an author — ingested (`Author`, ADR-0004: currently an issuing organization/publication for GNews/Newswire, not a natural person; will be an individual for a future connector like Reddit, per ADR-0004's own general rule) or, if it is ever built, content a tenant user authors inside the product — explicitly check whether the author's rights are preserved end to end**: attribution stays intact, ownership is never silently reassigned to SocialEngage or a tenant, and any redistribution, enrichment (ADR-0038), or export (ADR-0039) of that content doesn't strip or obscure who authored it. This is the same discipline already established piecemeal — ADR-0027's "connector is a technical intermediary, not a contracting party," ADR-0038's own named-but-unresolved third-party-processor disclosure gap, `docs/legal/legal-compliance-register.md`'s 2026-08-06 findings on purpose limitation and secondary use by enrichment providers — surfaced here as one standing lens to apply consistently, not four separate concerns to rediscover each time.

**The same legal-content discipline as everywhere else in this project's ADRs applies**: this role can name what author-rights protection the *design* should provide (attribution preserved in the data model, no license terms that exceed what was actually granted, consent checkpoints named before a redistribution/export feature is built) — it cannot certify that any resulting product behavior or disclosure copy is legally sufficient. That determination stays outside an ADR's own competence, the same boundary ADR-0039 already drew for GDPR Article 17/20 and this role's own recent investigation drew for LinkedIn-style content-redistribution consent.

**Do not force a home for this that doesn't exist yet.** If a document under review doesn't actually touch an authored-content question, this check simply doesn't apply — the point is to consistently *ask*, not to manufacture an author-rights angle on every ADR regardless of relevance.
