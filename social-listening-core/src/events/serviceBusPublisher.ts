 ...
 * events topic, with tenantId set as a Service Bus application property —
 * required for ADR-0013's per-tenant SQL subscription filters to evaluate it
 * at all (filters see message properties, not payload body content). See
 * .claude/skills/ingestion-events/SKILL.md.
 */
export async function publishEvent(tenantId: string, body: unknown): Promise<void> {
