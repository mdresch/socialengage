...
import { BraveSearchResultItem } from './braveSearchConnector';

/**
 * Builds a search query expression tailored for Brave Search API from a Watchlist.
 */
export function buildBraveSearchQuery(watchlist: Watchlist): string {
 ...
 * against the triggering watchlist's exact AST filter rules.
 */
export function validateCandidateMatch(
  watchlist: Watchlist,
  candidate: BraveSearchResultItem
): boolean {
  const ast = watchlistToAst(watchlist);
  if (!ast) return false;

  const candidateText = [candidate.title, candidate.description, ...(candidate.extra_snippets ?? [])]
    .filter(Boolean)
    .join('. ');
