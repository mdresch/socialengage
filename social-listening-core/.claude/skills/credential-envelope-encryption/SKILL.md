...
- **`deleteCredentialById(tenantId, credentialId)` (Story 6.27, ADR-0060 Decision §2) — new, additive, deletes exactly one row by its own primary key.** Added when Facebook's own multi-Page-per-user support (`facebook_connected_pages`) made `deleteCredential()`'s tuple-scoped delete unsafe for a caller that needs to remove exactly one of several credentials sharing the same `(tenantId, platformId, ownerType, userId)` tuple — `deleteCredential()` itself is completely unchanged, every other caller unaffected. See `.claude/skills/facebook-connector/SKILL.md` for the real caller (`facebookPagesRouter.ts`'s `DELETE .../pages/:id`).
