...
- Retryable-error classification around a gate wait exists (`queue_ttl_exceeded`/`queue_depth_exceeded` in `errorClassification.ts`, Story 2.4) but nothing wires a real connector's `attempt()` to actually call `acquireForProvider()`/`acquireForAiModel()` and reclassify the gate's errors yet — see `connector-health-and-error-handling`'s SKILL.md.
