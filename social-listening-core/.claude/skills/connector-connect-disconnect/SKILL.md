...
- `contracts/epic-1/story-1.7.ownership-tier-connect-disconnect.contract.test.ts` — `platform_credentials`' additive `owner_type`/`user_id` schema; `ownerType: 'tenant'` connect requires `tenant_admin` (403 otherwise); `ownerType: 'user'` connect always sets `user_id` to the caller's own identity, ignoring a spoofed body value; tenant-wide disconnect requires `tenant_admin`; user-bound disconnect succeeds for the owning user or a `tenant_admin`, no one else; disconnecting one owner-type's credential never removes a coexisting other-owner-type credential for the same platform; no route references `X-Tenant-Id`, and a stray one has no effect. **AC9 (2026-08-17):** `POST .../connect` fails clearly (500, naming `KEY_VAULT_KEY_ID`) when that env var is genuinely unset, never silently attempting an invalid literal key identifier — see `.claude/skills/credential-envelope-encryption/SKILL.md`'s matching Load-bearing constraint.
...
- ~~**Whether connector activation needs its own table separate from `platform_credentials`** — ADR-0034's own answer is implicitly no (`owner_type`/`user_id` on the existing table suffice for what's currently named), but this remains an inherited, not freshly-resolved, question.~~ **Resolved 2026-08-12 (Story 1.11, ADR-0051):** yes — two new tables (`connector_activations`, `connector_user_activations`), credential-independent, mirroring this component's own tenant/user ownership split exactly. See `.claude/skills/connector-activation/SKILL.md`.
