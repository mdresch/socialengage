...
| ADR-0053 | `ingestGNewsArticles()` populates `body_markdown`/`body_markdown_version` via the shared `htmlToMarkdown()` utility, using `content` (free-tier-truncation-marker trimmed) over `description` as its own richest-field precedence; composes `enrichmentText` as `[title, body_markdown].filter(Boolean).join('. ')`, replacing the previous `[title, description].filter(Boolean).join('. ')` | 3.10 |
...
- `contracts/epic-3/story-3.10.canonical-markdown-post-body-normalization.contract.test.ts` — `ingestGNewsArticles()`'s richest-field precedence (`content` over `description`, `null` if neither), the free-tier truncation-marker (`[+N chars]`) trim applied before `htmlToMarkdown()`, `body_markdown`/`body_markdown_version` population via `ingestGNewsArticles()` directly (a synthetic `GNewsArticle` fixture, not a live fetch), confirmation that `raw_payload` gains no `rawXml` key (GNews's own `rawPayload` already spreads the complete article object, unlike Newswire/tenant-owned-feed), and the new `enrichmentText` composition. See `.claude/skills/canonical-markdown-conversion/SKILL.md` for the shared conversion pipeline itself.
...
- **`enrichPost()`'s text argument is no longer `[article.title, article.description].filter(Boolean).join('. ')` (Story 3.10, 2026-08-13)** — `description` is no longer joined directly; it's folded into `body_markdown`'s own precedence rule instead (`content` if present, else `description`), so `enrichmentText` becomes `[article.title, bodyMarkdown].filter(Boolean).join('. ')`, the same title+body shape Newswire/tenant-owned-feed now also use. Don't restore the old two-field join — it would silently drop `content`'s own real signal.
- **GNews's free-tier truncation marker (commonly `[+N chars]`) is stripped from `content` via a named, revisable regex before it ever reaches `htmlToMarkdown()`** — the exact literal format is unconfirmed against a real live response (ADR-0053 Open Question 11); revisit the regex if a real GNews credential ever surfaces a differently-shaped marker.
...
- **`canonical-markdown-conversion`**: `ingestGNewsArticles()` calls the shared `htmlToMarkdown()` utility, same as Newswire and tenant-owned-feed.
- **`social-post-lineage`**: `body_markdown`/`body_markdown_version` are two more `insertSocialPost()` input fields this connector now populates.
