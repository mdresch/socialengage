...
description: GET/POST/PATCH /v1/admin/tenants — the Platform Admin-only REST surface over tenantStore.ts, including the domain field's admin-recovery grant (ADR-0037 §9) and the 2026-08-12 name-rename enhancement. Read this before touching adminTenantsRouter.ts, before adding a new admin-facing tenant field, or before wiring another platform_admin-only route.
---
...
- `contracts/epic-5/story-5.12.platform-admin-tenant-management.contract.test.ts` — full CRUD-surface list/create/update proven against a real `platform_admin` identity; `tenant_admin`/`tenant_user` and unauthenticated callers rejected (403/401); `active_seat_count` cannot be set via `PATCH` (DB-enforced, re-proven at the HTTP layer); `domain` can be set and cleared (`null`); `name` can be renamed (added 2026-08-12); every write appears in `platform_admin_audit_log`.

## How to extend this safely

- **A new admin-facing tenant field:** add it to `tenantStore.ts`'s `Tenant`/`TenantRow`/`UpdateTenantAdminInput` first, decide its column-level grant in a new migration (never edit `0017`/`0020`/`0029` after they ship) the same way `0020` added `domain` and `0029` added `name`, then surface it here — don't invent a second admin router for one more field. **Write the migration and run the contract before assuming the grant is covered** — an interface field compiling is not proof of a working grant; this exact mistake (assuming `platform_admin_role` already had `UPDATE` on the whole row) produced a real `aclcheck_error` on 2026-08-12 when `name` was first wired up, caught only by running the contract, not by code review.
...
- **`domain: null` in a `PATCH` body means "clear it," `domain` omitted means "don't touch it."** These are read from `req.body` directly (JS already distinguishes an absent key from an explicit `null`) — don't collapse this to a single "falsy means clear" check, which would also clear on an empty string. `name` follows the same "omitted means don't touch" rule but has no clear-to-null case — it's `NOT NULL` (migration `0017`).
- **`platform_admin_role`'s `UPDATE` grant on `tenants` is column-scoped, not a blanket grant on the row** — `status`/`license_seat_count` from migration `0017`, `domain` from `0020`, `name` from `0029`. Every new writable field needs its own explicit `GRANT UPDATE (column)` migration; see `.claude/skills/tenants/SKILL.md`'s own load-bearing constraint on this.
