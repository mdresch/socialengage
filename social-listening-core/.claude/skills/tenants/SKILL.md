...
- **`platform_admin_role`'s `UPDATE` grant on `tenants` is column-scoped to `(status, license_seat_count)` from migration 0017, `(domain)` added by migration 0020 (ADR-0037 §9), and `(name)` added by migration 0029 (2026-08-12 enhancement) — never a blanket `UPDATE`.** A blanket grant would silently defeat AC3's "Platform Admin cannot write `active_seat_count`" boundary, which today is a real, DB-enforced permission-denied error, not an application-code convention that trusts the caller not to. Each new writable column needs its own explicit `GRANT UPDATE (column)` migration — this has already been missed twice (`domain` in 0017, `name` in the same PR that first tried to wire it up), confirmed both times only by a real `aclcheck_error` at runtime, not by code review. Don't assume a new field in `UpdateTenantAdminInput` is grant-covered just because the interface compiles — check the actual `GRANT` migrations.
