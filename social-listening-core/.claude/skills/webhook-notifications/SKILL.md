...
- **`real-time-alert-rules` skill** — the alert evaluation engine is the primary caller of the webhook dispatcher; when an alert fires, the dispatcher fans out HTTP POSTs to all active subscriptions for the tenant. **Documentation Steward note, 2026-09-14: not yet backed by a real-call-site contract.** `alertEvaluationWorker.ts` does not call `dispatchWebhookEvent`/`deliverWebhookWithRetry` anywhere outside `contracts/` — see the matching note in `real-time-alert-rules/SKILL.md`. The only real production caller of the dispatcher today is the test-ping route and `crisisEscalationEngine.ts`. Flagged for Menno to route to contract-authoring, not fixed here.
