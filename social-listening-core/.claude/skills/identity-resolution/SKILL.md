...
- `contracts/epic-5/story-5.17.access-history-read-endpoint.contract.test.ts` — the read side of the same audit trail: `GET /v1/tenants/users/:id/access-history` (tenant_admin only, 403/401 otherwise) returns real entries shaped `{id, targetUserId, actorUserId, operation, oldValue, newValue, occurredAt}`; both a set and a subsequent clear are visible, most recent first, the clear's own `newValue: null`; a cross-tenant guess at `targetUserId` returns an empty array (RLS-filtered), never another tenant's rows; `breakGlassCredentialReset.ts` never references `access_ends_at`/`user_access_audit_log` at all (structural check, confirming the two mechanisms stay independent per ADR-0032 §9/ADR-0030 §3).
...
- **`access_ends_at`'s audit trail (write side, Story 1.9) and its read side (`listAccessHistory()`, Story 5.17) both live here.** A new caller needing this history reads it through `listAccessHistory(tenantId, targetUserId)`, never a second, parallel query against `user_access_audit_log` — this function is the one sanctioned read path, the same "one function per table's own access pattern" discipline the rest of this module already follows.
...
- **`access_ends_at` writes ARE now audited (Story 1.9), and readable (Story 5.17)** — `setAccessEndsAt()` writes a row to `user_access_audit_log` (migration `0024`, created by Story 1.9) with `tenant_id`, `target_user_id`, `actor_user_id`, `operation`, `old_value`, and `new_value`. The `actorUserId` parameter is required on `setAccessEndsAt()` — callers must supply the resolved tenant_admin's own `userId`, never a client-supplied value. `GET /v1/tenants/users/:id/access-history` (Story 5.17) reads it back, tenant_admin only.
- **A real, deliberately-uncorrected schema/AC naming drift, named honestly rather than hidden:** Story 5.17's own AC1 (drafted 2026-08-05) specifies columns `user_id`/`changed_by`/`previous_value`/`changed_at`; migration `0024` (built ahead of schedule by Story 1.9, 2026-08-09) actually shipped `target_user_id`/`actor_user_id`/`old_value`/`occurred_at`, plus an `operation` column AC1 doesn't mention. Not re-migrated when Story 5.17 was finally built (2026-08-10) — the already-shipped names are already load-bearing in Story 1.9's own passing contract and this module's own code; see `docs/user-stories/epic-5-security-isolation-and-messaging.md`'s own dated correction note on Story 5.17 for the full account. The REST response (`listAccessHistory()`'s own `AccessHistoryEntry`) uses ordinary camelCase equivalents of the real column names (`targetUserId`, `actorUserId`, `oldValue`, `newValue`, `occurredAt`), not AC1's original literal names.
