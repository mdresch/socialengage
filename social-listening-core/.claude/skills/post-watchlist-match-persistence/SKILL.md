...
- **`post_watchlist_matches.post_id` carries no DB-enforced foreign key into `social_posts` — deliberately, discovered at implementation time.** `social_posts` has been partitioned by `created_at` since migration `0012` (Story 3.5, ADR-0018), which forced its primary key to become composite (`id`, `created_at`) — a hard Postgres requirement for partitioned tables. `social_posts.id` alone has no unique constraint to reference, so `REFERENCES social_posts(id)` fails at migration time regardless of how it's written (confirmed empirically). This is the identical conflict already resolved once for `social_posts.acquisition_id → ingestion_runs(id)` (`data-retention-and-archival/SKILL.md`'s own load-bearing constraints) — a real FK is also fundamentally incompatible with `archiveAgedRawPayloads()`'s own partition-detach cycle. `post_id` is app-enforced only: `insertPostWatchlistMatches()`'s only real caller, `publishSocialPostIngestedEvents()`, always passes a real, just-inserted post id. `watchlist_id → watchlists(id) ON DELETE CASCADE` is unaffected — `watchlists` is not partitioned. See ADR-0063's own Amendment Log for the dated correction; don't add this FK back without re-checking `social_posts`'s partitioning first.
