---
name: daily-digest-email
description: Backend service and worker for timezone-aware daily digest email generation, metrics aggregation, notable post ranking, and bounded AI summaries.
---

# Daily Digest Email (Backend)

Governed by **ADR-0096**, **BRD-0096**, **FDD-0096**, and **Story 11.3**.

## Contracts that constrain this component

- `social-listening-core/contracts/epic-11/story-11.3.daily-digest-email.contract.test.ts` — Story 11.3 contract test.

## Key Architecture & Components

1. **Database Schema (`user_digest_preferences`)**:
   - `id`, `tenant_id`, `user_id`, `is_enabled`, `send_at_local`, `timezone`, `watchlist_ids`, `include_ai_summary`, `include_top_posts`, `include_topic_breakdown`, `last_sent_at`.
   - Index on `(is_enabled, timezone, send_at_local)` for efficient hourly worker queries.
   - Forced PostgreSQL RLS with `tenant_isolation` policy.

2. **Timezone-Aware Scheduling (`digestPreferenceStore.ts` & `dailyDigestScheduler.ts`)**:
   - Hourly job matching `to_char(now() AT TIME ZONE timezone, 'HH24') = to_char(send_at_local, 'HH24')`.
   - 20-hour duplicate send cooldown guard: `last_sent_at < now() - INTERVAL '20 hours'`.

3. **Data Assembly & Precomputed Views (`dailyDigestBuilder.ts`)**:
   - Reads 24-hour slices from `sentiment_daily_counts` and `source_daily_counts` (ADR-0087).
   - Ranks candidate posts using the blended impact score:
     $$\text{ImpactScore} = (\text{reach} \times 0.4) + (\text{engagement} \times 0.4) + (\text{is\_negative} \ ? \ 300 : 0)$$
   - Caps `notablePosts` at top 5.
   - Bounded AI summary generation via `AIProviderConnector.research()`.

4. **Rendering & Delivery (`dailyDigestRenderer.ts`)**:
   - Generates dual MIME parts (`text/html` and `text/plain`).
   - Standardized branding tokens and RFC 8058 compliant one-click unsubscribe links.

5. **API Endpoints (`digestRoutes.ts`)**:
   - `GET /v1/users/me/digest-preferences`
   - `POST /v1/users/me/digest-preferences`
   - `POST /v1/users/me/digest-previews`
   - `GET /v1/digest/unsubscribe`

## Relations to other components

- **`user_digest_preferences` table** — user subscription state, timezone, watchlist scoping, and feature flags; scheduler queries this table hourly.
- **`precomputed-analytics-views` skill** — `sentiment_daily_counts` and `source_daily_counts` (ADR-0087) provide the 24-hour sentiment breakdown and volume metrics that the digest builder reads.
- **`social_posts` table** — notable post ranking reads the top 5 posts by blended impact score from the last 24 hours.
- **`azure-openai-connector` skill** — bounded AI summary is generated via `AIProviderConnector.research()` when `include_ai_summary` is enabled. **Documentation Steward correction, 2026-09-14: this relationship does not exist in the real code.** `dailyDigestBuilder.ts`'s "Bounded AI Summary Generation" step (guarded by `prefs.includeAiSummary`) never calls `AIProviderConnector.research()` or any Azure OpenAI client — `narrative`/`keyThemes`/`sentimentTrend` are built entirely from template-string interpolation over real sentiment/topic/post data, not LLM output. Same class of gap as `ai-insights-digest/SKILL.md`'s matching correction. Flagged for Menno (component: `daily-digest-email`; relationship: Azure OpenAI summary; story: 11.3) — not fixed here.
- **`social-post-enrichment` skill** — enrichment data (sentiment labels, reach) on `social_posts` is what makes the impact score meaningful for notable post ranking.
- **`watchlist-crud` skill** — digest preferences scope delivery to specific watchlists; the scheduler resolves `watchlist_ids` to filter the post pool.
