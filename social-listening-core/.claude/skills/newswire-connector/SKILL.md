...
| ADR-0053 | `ParsedRssItem` gains `description`/`contentEncoded`/`rawXml`; `ingestNewswireItems()` populates `body_markdown`/`body_markdown_version` via the shared `htmlToMarkdown()` utility and composes `enrichmentText` as `[title, body_markdown].filter(Boolean).join('. ')`, replacing the previous title-only `enrichPost()` call | 3.10 |
...
- `contracts/epic-3/story-3.10.canonical-markdown-post-body-normalization.contract.test.ts` — `rssFeedParser.ts`'s widened `description`/`contentEncoded`/`rawXml` fields (against a fixture, not a live fetch); `ingestNewswireItems()`'s richest-field precedence (`content:encoded` over `description`, `null` if neither), `body_markdown`/`body_markdown_version` population via `ingestNewswireItems()` directly (bypassing the live `pollNewswireFeeds()` wrapper — see `canonical-markdown-conversion`'s own SKILL.md), `raw_payload.rawXml` propagation via the pre-existing spread, and the new `enrichmentText` composition. See `.claude/skills/canonical-markdown-conversion/SKILL.md` for the shared conversion pipeline itself.
...
- **`enrichPost()`'s text argument is no longer bare `item.title` (Story 3.10, 2026-08-13)** — it's `[item.title, bodyMarkdown].filter(Boolean).join('. ')`, where `bodyMarkdown` is `htmlToMarkdown(item.contentEncoded ?? item.description ?? '')` collapsed to `undefined` when empty. Don't revert this to bare `title` — that would silently regress the exact enrichment-quality gap ADR-0053 exists to close.
- **`rawXml` is `raw_payload`-only — never a `body_markdown` source.** The precedence rule (`content:encoded` → `description` → `null`) is unchanged by adding `rawXml`; it exists purely to close a separate, previously-silent `raw_payload` under-capture gap (ADR-0053 Context, "second finding").
