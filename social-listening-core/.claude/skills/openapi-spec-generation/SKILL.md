...
- **Story 20.1's other two acceptance criteria were infrastructure, not code, and remain uncovered by this skill or its contract** — *Documentation Steward correction, 2026-09-26: both are now done, stale here since 2026-09-17.* (1) "`social-listening-core` repo builds and passes CI with no frontend code present" — the actual standalone-repository split **has** been performed: `social-listening-core` is now a real, separate private GitHub repo (`https://github.com/mdresch/social-listening-core`, clean-cut extraction, initial commit `3c1e636`; rollback point tagged `pre-story-20.1-repo-split` on this repo's own history). (2) "original combined repo history is preserved via `git filter-repo` or clean-cut" — Menno chose clean-cut, per TDS-0144's documented alternative. See `docs/implementation-log.md`'s two Story 20.1 entries (the OpenAPI-generation pass, then the repo-extraction follow-up) and `docs/user-stories/epic-20-adr-0144.md`'s own Notes for the full account. **Not touched by this correction:** this document itself continues to describe the copy of this code that lives in *this* workspace repo (`socialengage`), which — per `CLAUDE.md`'s own Map section — remains the actual development home; the standalone repo is a separately-CI'd artifact, not a second place this skill's own guidance applies to.
- **No CI workflow step in *this* workspace repo wires `npm run openapi:generate` into an actual pipeline yet** — still true here (confirmed against `.github/workflows/ci.yml`/`ci-simple.yml` at this repo's root, neither of which references `openapi`). *Documentation Steward note, 2026-09-26:* the new standalone `social-listening-core` repo (see above) does have this wired — its own `.github/workflows/ci.yml` runs build → typecheck → test → `openapi:generate` → upload `openapi.json` as a build artifact (a follow-up commit there, `b4d0711`, added `if: always()` so the artifact step isn't skipped when the pre-existing Key Vault credential-gap test failures fail the job first). That workflow file lives in the external repo, not here, so it isn't this file's own claim to make — noted for context, not corrected as if it were this repo's own gap.
