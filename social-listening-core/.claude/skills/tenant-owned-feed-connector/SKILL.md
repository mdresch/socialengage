...
| ADR-0050 (2026-08-20 Amendment Log entry) | Narrows Open Question 2's UX half (a tenant-owner-set, optional `name` per activation) and resolves Open Question 5's display-layer half (per-item byline extraction, `rawPayload.author`, never touching Author modeling) | 2.19 |
...
- `contracts/epic-2/story-2.19.tenant-owned-feed-naming-and-byline.contract.test.ts` — `POST /connect` accepts an optional `name` (400 on an empty/whitespace-only string; `null` when omitted, never required); `PATCH /:id` now accepts `feedUrl` and/or `name` independently (400 when neither is present); `name: null` clears a previously-set name; `extractByline()` (`feedItemParser.ts`, tested directly via `parseFeedItems()`) tries `<dc:creator>`, then flat RSS `<author>`, then Atom's nested `<author><name>`, `null` when none are present; `ingestTenantOwnedFeedItems()` denormalizes `activation.name` into `rawPayload.feedName` (genuinely absent, not `null`, when unset) and the item's own byline into `rawPayload.author` — while the post's real `authorId`/`Author` row still resolves to the verified domain regardless, proving this is display-only.
...
tenant_owned_feed_activations
  ...
  name                TEXT        -- migrations/0037 (Story 2.19) — optional, tenant-owner-set display label; null when unset
```
...
- **Adding a further per-item field from the feed itself** (Story 2.19's own `extractByline()` precedent): add the extraction to `feedItemParser.ts`, add the field to `ParsedFeedItem`, and — if it's meant for admin display, not Author/analytics modeling — denormalize it into `rawPayload` in `ingestTenantOwnedFeedItems()` the same way `item.author`/`activation.name` are (never `null`-fill; omit the key entirely when absent, matching Facebook's `shares` precedent). Don't route a new display-only field through `Author`/`authorId` — that's a structurally different, larger change (see the next bullet).
- **Replacing organization-as-Author (verified domain) with individual-as-Author** (the harder half of ADR-0050 Open Question 5, still open after Story 2.19): Story 2.19 deliberately only denormalized the byline into `rawPayload.author` for display — it did **not** change `authorExternalId`/what `upsertAuthor()` is called with, which stays the verified domain. Doing the harder version (an `authors` row per individual byline) is a real, separate, not-yet-decided design question — see ADR-0050's own 2026-08-20 Amendment Log entry.
...
- **`Author.externalAuthorId` is the verified `domain`, fixed per activation — never derived from a per-item field.** Story 2.19 added per-item byline extraction (`extractByline()`), but it feeds `rawPayload.author` (display-only) — `upsertAuthor()`'s own call in `ingestTenantOwnedFeedItems()` is unchanged, still keyed on `activation.domain`. Don't wire the byline into `authorExternalId` without a new, separate ADR decision (see "How to extend this safely" above).
...
- **No individual per-item Author *identity*** — Story 2.19 closed the narrower "can a per-item byline be surfaced at all" half (display-only, `rawPayload.author`), but `Author`/`authorId` modeling itself is still fixed to the verified domain; a real per-individual `Author` row (and what that means for topic-signals/analytics grouping) would need a separate, not-yet-decided ADR — see ADR-0050's own 2026-08-20 Amendment Log entry and this file's own "How to extend this safely" note above.
...
- **`social-listening-admin`'s `postDisplay.ts`**: `rawPayload.author` (Story 2.19's byline) is picked up automatically by that file's already-existing `extractAuthor()` precedence chain (same pattern Newswire's `issuer`/Facebook's `pageName` already established) — no admin-side code change was needed for the byline half. `rawPayload.feedName`/the setup UI's own `name` field are a separate, admin-repo-only follow-on (see that repo's `post-feed`/`tenant-owned-feed-connector-setup` SKILL.md files for what, if anything, has picked them up).
