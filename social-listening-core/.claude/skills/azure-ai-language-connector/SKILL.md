...
- **No admin-UI connect flow exists for this provider — corrected 2026-08-10, this bullet previously overstated what Story 6.3 actually built.** Verified directly against the real code: `social-listening-admin/src/app/tenant/connectors/page.tsx` is a static placeholder — a hardcoded `[gnews, newswire]` array with hardcoded `connected` booleans, no credential-entry form, and no API call anywhere in `core-client.ts` to the backend's connect/disconnect endpoint. Story 6.3's own contract only asserts the page exists and contains specific text strings ("GNews", "Newswire") — it never exercises a real credential submission. There is no way for a real tenant to connect this provider (or GNews/Newswire, for that matter) through the actual UI today; every credential used in this project's own contracts is written directly via `storeCredential()`, not through any screen. A real connect form (for this provider, still awkward either way — a tenant must hand-construct the `{endpoint,key}` JSON unless a dedicated two-field form is built) remains fully unbuilt.
