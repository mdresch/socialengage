...
- **`azure-openai-connector` skill** — the digest generator calls Azure OpenAI (gpt-5-mini or equivalent) to synthesize the executive summary, theme analysis, and strategic recommendations from aggregated post/sentiment data. **Documentation Steward correction, 2026-09-14: this relationship does not exist in the real code.** `src/analytics/aiDigestGenerator.ts`'s `generateAiInsightsDigest()` imports only `withTenant`/`getPool`/`PoolClient` — no Azure OpenAI or any `azure-openai-connector` call anywhere in the file. `executiveSummary` is a template string interpolating real sentiment counts; `topThemes` and `strategicRecommendations` are hardcoded static arrays, not LLM output. Flagged for Menno as a gap (component: `ai-insights-digest`; relationship: Azure OpenAI synthesis; story: 10.14) — not fixed here, since writing the real integration is contract-authoring, not documentation-audit.
