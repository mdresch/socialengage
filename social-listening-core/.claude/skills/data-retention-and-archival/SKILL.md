...
- **Partition eligibility uses `monthStart < cutoff`, not `monthEnd <= cutoff` (corrected 2026-08-09).** Any partition whose month started before the retention cutoff is eligible — this correctly handles the boundary month (the month that straddles the cutoff) which can contain both aged rows (before the cutoff) and recent rows (after it). The inner `SELECT` within each detached partition always adds `AND created_at < $1` (the cutoff) so rows still within the retention window are never archived, even from a partially-aged partition. The prior `monthEnd <= cutoff` condition caused the boundary month to be skipped entirely, leaving genuinely aged rows unarchived whenever they landed in that month.
...
- **`social_posts` table** — rows whose `raw_payload` is archived to Azure Blob Storage after the retention window; the row itself remains in Postgres with a blob pointer replacing the `raw_payload` field.
- **`ingestion_runs` table** — whole-row archival target; aged-out rows are exported to blob and removed from the live table, recoverable only via `resolveIngestionRun()`.
- **`live-ingestion-polling-scheduler` skill** — ingestion run records written by the scheduler are the primary source of rows that eventually age through `archiveAgedIngestionRuns()`.
- **`post-watchlist-match-persistence` skill** — `social_posts` schema constraints (partitioned by `created_at`, composite PK) are load-bearing for this skill's archival mechanism; schema changes to `social_posts` must be checked here first.
- **`export-jobs` skill** — archived `raw_payload` fields can be omitted from exports; export queries should not assume `raw_payload` is always populated in Postgres.

- **Documentation Steward correction, 2026-08-13 — a real, named, still-open operational risk was surfaced 2026-08-10 (`docs/implementation-log.md`'s own Story 2.8 entry, `social-listening-core@f70b07d`) but never recorded here until now.** Neither `archiveAgedRawPayloads()` (`socialPostArchival.ts`) nor `archiveAgedIngestionRuns()` (`ingestionRunArchival.ts`) wraps its own detach → process (blob upload) → reattach/drop sequence in a `try`/`finally`, confirmed directly against the current source (both still `DETACH PARTITION` with no surrounding guard as of this note). If `uploadArchiveBlob()` throws partway through a run — a transient Blob Storage failure in production, or the real Azure-subscription-move RBAC gap that actually triggered this on 2026-08-10 — the function exits with that month's partition **permanently detached** from its parent table, poisoning every subsequent read/write touching that table until a human manually reattaches it. Named in the Implementation Log as "worth its own dedicated healing pass given the real production risk" at the time; still not built as of this note — a real, open, unfixed gap, not a hypothetical one, and worth treating as higher-priority than the other bullets in this section given the blast radius (a stuck partition, not just missing automation).
