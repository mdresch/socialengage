...
- **`azure-openai-connector` skill** — token consumption telemetry surfaced by this dashboard comes from Azure OpenAI API usage tracked during enrichment; the dashboard makes that cost visible at the platform level. **Documentation Steward note, 2026-09-13: not yet backed by a real-call-site contract.** `platformMetricsStore.ts` never queries the `azure-openai-connector` for real usage data — `totalTokensLast30d` is a flat `posts30d * 850` estimate, and the `platform_metrics` row it prefers (`metric_name = 'estimated_total_cost'`) is read here but never written by any worker found in the codebase, so it always falls back to the estimate. No real Azure OpenAI token-usage call site exists. Flagged for Menno to route to `qa-contract-author` if real token telemetry is intended, rather than authored here.
