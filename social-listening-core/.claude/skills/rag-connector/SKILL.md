...
5. **`ensureTenantNamespace?` is optional and unimplemented by pgvector:** pgvector's isolation is a static, already-provisioned table-level RLS policy, not a resource needing per-tenant provisioning before first write. **Documentation Steward correction, 2026-09-16:** this line previously said a "future namespace/shard-capable provider (Story 19.3, ADR-0138) implements it" — Story 19.3 has since shipped (`1e71d7e`) and its own AC6 explicitly scoped a Pinecone/Azure AI Search connector *out* ("No Pinecone/Azure AI Search connector is built... explicitly out of scope"); its real work was fixing `RAGReconciliationService`'s missing `withTenant()` call (see `rag-vector-rls/SKILL.md`), not implementing this hook for a new provider. No connector implements `ensureTenantNamespace?` as of this pass (confirmed via grep of `src/rag/`) — it remains a defined-but-unimplemented optional hook, with no story currently scoped to build its first real implementer.
...
9. **Only `pgvector` is registered** in `ragConnectorRegistry.ts`, still true as of Story 19.3 (*Documentation Steward correction, 2026-09-16: previously attributed a future Pinecone/Azure AI Search connector to "Story 19.3/ADR-0138's scope" — Story 19.3 has since shipped and its own AC6 explicitly excluded building any such connector; see Invariant 5's matching correction above. No story currently scoped to build a second connector is known as of this pass.*).
