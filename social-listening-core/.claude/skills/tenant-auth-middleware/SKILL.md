...
- **A new route needing Platform Admin identity:** use `requirePlatformAdmin()` (`requireTenantUser.ts`) — built for real, Story 5.12, the admin tenant-management REST surface (`.claude/skills/platform-admin-tenant-management/SKILL.md`). Same 403-on-wrong-shape pattern as `requireTenantUser()`/`requireTenantUserIdentity()`, opposite identity type.
