...
- **Files touched:** docs/adr/README.md, docs/implementation-plan.md, docs/pending-documentation-steward-reviews.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md, docs/user-stories/epic-3-data-model-storage-and-archival.md, docs/user-stories/epic-6-tenant-admin-ui.md — confirmed via `git diff-tree --no-commit-id --name-only -r 3a8ff91`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — this is this role's own prior scheduled pass landing its own proposed corrections; audited for accuracy rather than re-doing the work.** Spot-checked the substantive claims: the Story 1.15/6.27 build-date fix in `docs/implementation-plan.md` (2026-08-19 → 2026-08-18) checks out exactly against `docs/implementation-log.md`'s own entries (`social-listening-core@b270662`, `social-listening-admin@b58b323`, both headed "2026-08-18, later the same day"); the `docs/adr/README.md` "Proposed" section fix (ADR-0074/0075/0076 marked superseded/Accepted) is correct per each ADR's own Status line; Story 6.28's reconstructed section in `epic-6-tenant-admin-ui.md` is clearly marked as reconstructed, not invented, and cites a real commit/SKILL.md pairing. The large ADR-0077–0121 "Proposed" reconciliation this entry's own note flagged as outstanding for Menno was not left to rot — a later, dated 2026-08-27 batch-review note already exists at the top of that same section in the current file, so no action needed here. No new drift found in this commit's own diff.
...
- **Files touched:** docs/management/pending-manager-reviews.md, docs/pending-documentation-steward-reviews.md, docs/pending-learning-development-reviews.md, docs/time-tracking.md, social-listening-admin/.claude/skills/polypost-composer/SKILL.md, social-listening-admin/next-env.d.ts, social-listening-admin/src/app/globals.css, social-listening-admin/src/app/layout.tsx, social-listening-admin/src/app/loading.tsx, social-listening-admin/src/app/page.module.css, social-listening-admin/src/app/platform-admin/loading.tsx, social-listening-admin/src/app/skeleton.css, social-listening-admin/src/app/tenant/loading.tsx, social-listening-admin/src/components/composer/PolypostComposer.tsx, social-listening-admin/src/components/composer/PublishTargetsDialog.tsx, social-listening-admin/src/components/ui/Skeleton.tsx, social-listening-admin/src/components/ui/index.ts — confirmed via `git diff-tree --no-commit-id --name-only -r 6a8b69e`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — real drift found under the 2026-08-13 relationship-assertion convention and corrected.** This commit (explicitly bypassing the contract-first pre-commit hook per its own message, "a catch-up commit of already-written UI wiring ahead of a dedicated contract pass") added a new "Relations to other components" entry to `polypost-composer/SKILL.md`: "`PublishTargetsDialog.tsx` calls `GET /api/connectors/facebook/pages`." Verified the real source (`PublishTargetsDialog.tsx` does call `fetch('/api/connectors/facebook/pages')`) but found **no contract exercises that specific call site** — `story-6.27...contract.test.ts` only covers the route handler/`core-client.ts` functions, and the later `story-6.39...contract.test.ts` only covers this same file's non-Facebook-platform-disabling behavior, never the fetch call itself. This is exactly the class of gap the convention exists to catch. Corrected directly in `social-listening-admin/.claude/skills/polypost-composer/SKILL.md` with a dated note on that relationship entry, per the charter's instruction to fix the doc but not author the missing contract. **Flagged for Menno to route to `qa-contract-author`:** a contract asserting `PublishTargetsDialog.tsx`'s own `GET /api/connectors/facebook/pages` call site is still missing. All other touched files (loading-state/skeleton UI, `PolypostComposer.tsx` wiring, the three review-queue files) checked clean — no other doc claim affected.
...
- **Files touched:** .devin/skills/competitor-query/SKILL.md, .devin/skills/deep-feature-research/SKILL.md, .devin/skills/product-comparison/SKILL.md — confirmed via `git diff-tree --no-commit-id --name-only -r 5c1e521`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — clean, out of this role's chartered file scope.** These are `.devin/skills/*` tooling files (Devin's own research skill definitions), not the component `SKILL.md` files (`social-listening-core/.claude/skills/*`, `social-listening-admin/.claude/skills/*`) this role's charter covers, and they make no ADR/story/PM-doc factual claim. Nothing here for this role to check.
...
- **Files touched:** social-listening-core/.claude/skills/connector-health-and-error-handling/SKILL.md, social-listening-core/.claude/skills/outbound-engagement/SKILL.md, social-listening-core/.claude/skills/provider-connector-framework/SKILL.md, social-listening-core/contracts/epic-2/story-2.26.connector-reply-framework.contract.test.ts, social-listening-core/src/connectors/requestGate.ts, social-listening-core/src/connectors/types.ts, social-listening-core/src/ingestion/errorClassification.ts, social-listening-core/src/outbound/outboundEngagementService.ts — confirmed via `git diff-tree --no-commit-id --name-only -r e3df7d9`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — real drift found and corrected, but minor and formatting-only, not a factual mismatch.** New `outbound-engagement/SKILL.md`'s "Relations to other components" claims (calls into `requestGate.ts`, `types.ts`, `errorClassification.ts`) are each backed by `story-2.26...contract.test.ts`'s own ACs, and the honestly-disclosed "Known gaps" section correctly notes `outboundEngagementService.invoke()` has no real REST call site yet (Story 3.14) — not a `publishEvent()`-style false claim. Found a real markdown-table defect this commit introduced in both `connector-health-and-error-handling/SKILL.md` and `provider-connector-framework/SKILL.md`: the new ADR-0073 governing-ADR row was appended as `|| ADR-0073 | ...` after a blank line, which GFM does not render as part of the existing table at all (an orphaned, unreadable line) — corrected directly in both files by removing the blank line and the extra leading pipe, folding the row back into its table. (`provider-connector-framework/SKILL.md` has the same broken-append pattern recurring for ADR-0075/0076/0077 rows added by later commits — left alone here and will be fixed when this pass reaches those commits, to keep each fix scoped to the commit that introduced it.) No ADR/story/PM-doc factual claim was wrong.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md — confirmed via `git diff-tree --no-commit-id --name-only -r 9231a67`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — clean, no drift.** Story 2.26's `**Built:**` field correctly set to `2026-08-23 — social-listening-core@e3df7d9`, matching the new `docs/implementation-log.md` entry's own file list against `e3df7d9`'s real diff exactly (append-only, no prior entry touched). This commit also correctly flips Stories 2.28–2.32 from Blocked to Ready now that ADR-0075/ADR-0076 are Accepted (2026-08-23) — verified directly against both ADRs' own current Status lines.
...
- **Files touched:** social-listening-core/.claude/skills/bing-search-connector/SKILL.md, social-listening-core/.claude/skills/brave-search-connector/SKILL.md, social-listening-core/contracts/epic-2/story-2.31.brave-and-bing-one-off-research-search-helpers.contract.test.ts, social-listening-core/src/connectors/bingSearch/bingSearchConnector.ts, social-listening-core/src/connectors/braveSearch/braveSearchConnector.ts — confirmed via `git diff-tree --no-commit-id --name-only -r 4949200`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — clean, no drift.** Both touched `SKILL.md`s' new ADR-0076/Story 2.31 rows and contract citations are accurate (ADR-0076 genuinely Accepted 2026-08-23, `story-2.31...contract.test.ts` genuinely exists and matches the described `searchForResearch()` scope). Neither file has a "Relations to other components" section, so the 2026-08-13 relationship-assertion convention doesn't apply here; new "one-off research is non-persistent" constraints are self-contained, contract-testable claims, not cross-component call claims.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md — confirmed via `git diff-tree --no-commit-id --name-only -r 0364adb`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — clean, no drift.** Story 2.31's `**Built:**` field correctly set to `2026-08-23 — social-listening-core@4949200`; the new `docs/implementation-log.md` entry's own file list matches `4949200`'s real diff exactly, appended only.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — out of this role's chartered file scope; file-list claim verified exact.** Same reasoning as `eb319d6` (BRDs) above: `docs/project docs/Functional-Design/FDD-*.md` and the FDD template are a new document type not named in this role's charter. Spot-checked FDD-0067 (Reconfirm Facebook Connector) against ADR-0067/BRD-0067 for a sanity check only — consistent. Not swept further.
...
- **Files touched:** social-listening-core/.claude/skills/facebook-connector/SKILL.md, social-listening-core/.claude/skills/outbound-engagement/SKILL.md, social-listening-core/contracts/epic-2/story-2.27.facebook-page-reply-implementation.contract.test.ts, social-listening-core/src/connectors/facebook/facebookConnector.ts — confirmed via `git diff-tree --no-commit-id --name-only -r 2da26eb`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — one formatting defect found and corrected; otherwise clean.** `outbound-engagement/SKILL.md`'s updated contract list correctly and honestly notes this is "the first real `SocialConnector.reply()` call site" (still contract-exercised, not yet reachable via a real REST endpoint pending Story 3.14 — no false claim). No new "Relations to other components" entry was added by this commit's own diff, so nothing new to verify against the 2026-08-13 convention there. Found the same broken-table-append defect already fixed under `e3df7d9` above: the new ADR-0073 row in `facebook-connector/SKILL.md`'s governing-ADR table was appended as `|| ADR-0073 | ...` after a blank line, orphaning it from the table under GFM rendering — corrected directly by removing the blank line and the extra leading pipe.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md — confirmed via `git diff-tree --no-commit-id --name-only -r 3bf8786`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — clean, verified a larger-than-titled scope carefully.** Story 2.27's own `**Built:**` field and the new `docs/implementation-log.md` entry both check out exactly against `2da26eb`'s real diff. This commit's title undersells its own scope, though — beyond Story 2.27, it also retroactively backfills `**Built:**` fields onto eight pre-2026-08-13 stories (2.1–2.8) that had none, which `docs/user-stories/README.md`'s own "Built convention" note frames as forward-only/no-retroactive-sweep **for a routine audit pass**, but explicitly allows once a story is "next touched... for some other reason." Verified all eight backfilled hashes directly against `git log` — every date, hash, and story number is exactly correct (`7604c25`/2.1, `00322f2`/2.2, `10e7c43`/2.3, `6a21c8b`/2.4, `fc4245c`/2.5, `0169143`/2.6, `8e3a54d`/2.7, `f70b07d`/2.8) — including correctly normalizing Story 2.8's pre-convention "Status: Built 2026-08-10..." header back to "Status: Ready" plus a proper `**Built:**` field, resolving a real, older Status/Built-meaning conflation. No factual drift found in any of it.
...
- **Files touched:** social-listening-core/.claude/skills/outbound-engagement/SKILL.md, social-listening-core/.claude/skills/outbound-post/SKILL.md, social-listening-core/.claude/skills/provider-connector-framework/SKILL.md, social-listening-core/contracts/epic-2/story-2.28.connector-publish-framework.contract.test.ts, social-listening-core/src/connectors/requestGate.ts, social-listening-core/src/connectors/types.ts, social-listening-core/src/ingestion/errorClassification.ts, social-listening-core/src/outbound/outboundPublishService.ts — confirmed via `git diff-tree --no-commit-id --name-only -r f1e0f9b`, exact match.
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 — real formatting/content drift found and corrected in two files; relationship claims are properly contract-backed.** New `outbound-post/SKILL.md`'s "Relations to other components" claims (calls into `requestGate.ts`/`types.ts`/`errorClassification.ts`) are each backed by `story-2.28...contract.test.ts`'s own ACs, honestly disclosing no real REST call site yet (Story 3.15). Found and fixed two real defects this commit introduced in `provider-connector-framework/SKILL.md`: (1) the same broken-table-append pattern already fixed under `e3df7d9`/`2da26eb` — the new ADR-0075 governing-ADR row was orphaned from its table by a stray leading pipe; (2) a genuinely garbled Load-bearing-constraints bullet where this commit's edit spliced the new Story 2.28 sentence into the middle of the existing Story 2.26 bullet, producing duplicated, unbalanced-bold, nonsensical prose ("Outbound and ingestion gates are separate keys (replies); **Outbound posts are gated separately... (Story 2.26, ADR-0073).** `acquireForOutbound()`... conservatively." with the tail of the original sentence reattached to the wrong claim). Split back into two clean, correct bullets, one per story/ADR, preserving both real facts. Also found and fixed an unclosed bold marker this commit introduced in `outbound-engagement/SKILL.md`'s "Known gaps" bullet about the sibling `outbound-post` component (a stray leading `**` never closed) — closed it after the summary clause, matching this section's own established style.
...
## 2026-08-28 — a2f134e — Scheduled doc review: Learning & Development Writer pass (2026-08-28)

- **Full commit:** `a2f134e912c829cf05b3af8be4dc4b38528447e8`
...
## 2026-08-28 — e875e18 — Queue commit 89b6e75 for Manager, Documentation Steward, and Learning & Development review

- **Full commit:** `e875e181dac2bafb6b69625f023a40ded9ada42a`
