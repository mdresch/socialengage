...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — this is a prior scheduled Documentation Steward pass's own commit; re-verified its claims against current HEAD and did the reconciliation it flagged as outstanding.** Spot-checked each claim in the commit message: `docs/implementation-plan.md`'s Stories 1.15/6.27 build-date fix (2026-08-19 → 2026-08-18) still holds, confirmed against `docs/implementation-log.md`'s own `b270662`/`b58b323` entries. `docs/user-stories/epic-6-tenant-admin-ui.md`'s Story 6.28 section (added by this commit, previously missing entirely) is present and consistent with its own SKILL.md. Story 6.32's honest "Built date corrected, commit hash unresolved" flag still stands unchanged (still no resolving commit in `git log --all` as of this pass — a standing gap, not re-litigated here). The one claim that had NOT actually been acted on: this commit's own flag that `docs/adr/README.md`'s "Proposed (not yet decided)" section needed "a dedicated reconciliation pass enumerating the real current Proposed/Accepted split across ADR-0077–0121+" — a later pass (see the `658b392` entry above) pointed back at this same flag without doing the reconciliation. Did it here: checked every ADR-0077–0121 file's own Status line directly and added a new dated top paragraph to that section (ADR-0077–0082 Accepted, ADR-0083–0121 genuinely still Proposed). The underlying post-commit-hook queuing gap (ADR-0077–0121's own drafting/acceptance commits never queued into this file at all) remains standing and outside this role's scope to fix — already flagged twice over by this file's own prior entries, not re-flagged a third time here.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** `polypost-composer/SKILL.md`'s updated "Known gaps" and Load-bearing-constraints text (`PublishTargetsDialog` is real, `handlePublish` still simulates dispatch) matches the real `PolypostComposer.tsx` at current HEAD exactly — `handlePublish` is still a `setTimeout`-based simulation with an explicit `// Simulated Publish` comment, even though ADR-0075/Stories 2.28–2.30 have since built a real backend publish path. Checked whether that now-wider gap (real backend, still-simulated admin UI) is tracked anywhere: it is — `docs/user-stories/epic-6-tenant-admin-ui.md` Story 6.39 ("Polypost Composer Real Publish Flow", Source ADR-0075, Status Ready/not yet built) names exactly this wiring as its own scope. No drift for this commit to fix.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** Pure `.devin/skills/` research-agent tooling (query `docs/product-research/competitor-feature-matrix.json`, explicitly read-only, explicitly not for writing ADRs/code/contracts) — no ADR, story, `SKILL.md`, `implementation-plan.md`, or PM-doc claim to go stale.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift; one scope observation flagged.** Spot-checked: file count under `docs/project docs/Business-Requirements/` (119 BRDs + BRD-Template.md) exactly matches the real ADR roster (ADR-0001-0121 minus the two never-real 0045/0046 numbers, per `docs/adr/README.md`'s own footnote) — no missing or orphaned BRD numbers (`comm -3` against the two number sets returns empty). BRD-0059 (Facebook connector) spot-checked against ADR-0059's own Decision/Source line — Source ADR line, scope description, and Approved status all consistent. **Scope observation, not fixed:** this role's own charter (`.claude/agents/documentation-steward.md`) names a specific PM-doc list (Stakeholder Register, Business Case, Charter, the nine named Project Management Plans) as in-scope, and does not mention `docs/project docs/Business-Requirements/` (or, per the `dc22ecc`/later FDD-dump entries, `docs/project docs/Functional-Design/`) at all — these are new artifact types this pass predates. Reviewed this commit's own content anyway since it makes real traceability claims (BRD-to-ADR mapping) squarely in this role's zero-drift-consistency wheelhouse, but flagging for Menno that the charter's own "What to check" list may need a deliberate scope decision on whether BRD/FDD upkeep belongs to this role, the `ba-requirements-analyst` agent, or neither — not decided unilaterally here.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** Self-contained process update to `implement-story/SKILL.md` (Step 1 now also reads the matching BRD/FDD, with an explicit ADR-wins-on-conflict rule). Consistent with `docs/implementation-methodology.md`'s existing Intent-block convention; no other file makes a claim this contradicts.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** Mirrors the same-day `c63752e` change for the sibling skill (Step 1's Re-validate-Intent now also re-reads the matching BRD/FDD). Same ADR-wins-on-conflict rule, worded consistently between the two skill files.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** New `outbound-post/SKILL.md` accurately describes the real `outboundPublishService.invoke()` (optional `SocialConnector.publish?()`, separate `RequestGate` key, doesn't persist the audit row itself — Story 3.15 does) — verified directly against `outboundPublishService.ts`. Consistent with the sibling `outbound-engagement`/`provider-connector-framework` SKILL.md updates in the same commit.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift; also fixes real pre-existing drift.** New `docs/implementation-log.md` entry for Story 2.28 matches the commit exactly. Beyond just logging 2.28, this commit also applies the project's established "Built convention" fix (split `**Status:** Ready`/`**Built:** <date> — <commit>` instead of an inline `Status: Built <date>`) to six older Story 2.10/2.11/2.13/2.14/2.18/2.20 headers in `epic-2-ingestion-connectors-and-rate-limits.md` that still used the non-standard inline form — spot-checked two of the cited commit hashes (`f2c7788`, `afcb59e`) directly against `docs/implementation-log.md`'s own matching entries; both correct.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** First real `SocialConnector.publish()` call site, matching `outbound-post/SKILL.md`'s own contract list (added by the sibling `f1e0f9b` commit) exactly.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** Log entry's file list matches `git diff-tree --no-commit-id --name-only -r 4234834` exactly; appended only.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** Second real `SocialConnector.publish()` call site, matching `outbound-post/SKILL.md`'s own contract list exactly.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-27 — clean, no drift.** Log entry's file list matches `git diff-tree --no-commit-id --name-only -r 3a65691` exactly; appended only.
