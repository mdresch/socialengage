...
**Built 2026-08-12.** `migrations/0026_create_tenant_owned_feed_activations.sql` (new, tenant-scoped RLS, `pending`/`verified`/`expired` state); `src/connectors/tenantOwnedFeed/` (new: `dnsVerification.ts` — real `dns.promises.resolveTxt` TXT lookup, `_socialengage-verify.<domain>` host scoping resolving Open Question 1, 7-day token TTL; `feedItemParser.ts` — this connector's own RSS+Atom `<item>`/`<entry>` extractor, deliberately not shared with Newswire's own wire-specific parser; `tenantOwnedFeedConnector.ts` — the registered `SocialConnector`, `authMode: 'none'`, domain-as-Author, a real self-identifying `User-Agent`; `tenantOwnedFeedStore.ts`; `pollTenantOwnedFeed.ts` — only ever iterates `verified` activations, the DB-layer guarantee AC3 depends on); `src/http/versions/v1/tenantOwnedFeedRouter.ts` (new — `POST connect`/`POST verify-domain`), mounted in `router.ts` at `/connectors/tenant-owned-feed` **before** the generic `/connectors` mount, deliberately never as routes inside `connectorsRouter.ts` itself — see `contracts/epic-2/story-2.11.tenant-owned-feed-connector.contract.test.ts` (13/13) and `docs/implementation-log.md`. **AC1's own no-core-path-edit evidence requirement is satisfied literally, not just narratively**: Story 2.10's own contract (`story-2.10.connector-registration-transparency.contract.test.ts`) was extended to include this connector's `providerId`, per that file's own documented "extend when a new connector is introduced" convention. The one deliberate seam in this story's own contract: `checkTxtRecord()`'s real DNS resolver is spied for exactly one test (the "record genuinely matches" success path, since this project controls no domain it could publish a real TXT record for) — the "no match" path runs against real DNS for a real domain (`example.com`) throughout. Full `social-listening-core` suite after: 52/52 suites, 370/370 tests passing.