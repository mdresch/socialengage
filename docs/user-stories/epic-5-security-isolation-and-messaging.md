...
**As a** platform operator whose Service Bus events are meant to drive downstream, real-time features (watchlist-hit notifications, connector-health alerting),
**I want** every real connector's ingestion loop and the shared polling orchestration to actually publish the events those features will eventually consume,
**so that** the event-driven half of this architecture is real and exercised, not a fully-built, fully-tested mechanism with no real caller.

**Acceptance Criteria**
- A new `listActiveWatchlistsForTenant(tenantId): Promise<Watchlist[]>` store function (`watchlistStore.ts`) — tenant-wide (`app.tenant_id` RLS only, no owning-user filter), returning every `is_active = true` row regardless of owner — proven distinct from the existing, deliberately owner-scoped `listWatchlists(tenantId, userId)` by a test confirming a watchlist owned by user B is returned for a call with no `userId` context at all.
- Each of the three real connectors' own ingest functions (`ingestGNewsArticles()`, `ingestNewswireItems()`, `ingestTenantOwnedFeedItems()`) fetches this tenant's active watchlists **once per poll batch** (not once per post), filters to those whose `platform_ids` include the connector's own `providerId`, and — after each post's `insertSocialPost()` call has resolved — resolves dispatch (`resolveWatchlistAstDispatch()`) and matches (`matchPostsForWatchlistAst()`) against each filtered watchlist, publishing one `SocialPostIngestedEvent` per matching watchlist (`buildSocialPostIngestedEvent()`, unmodified shape). Proven by a test asserting a post matching two watchlists publishes exactly two events, each with the correct distinct `watchlistId`, and a post matching zero watchlists publishes none.
- Proven by a test (or code-path assertion) that watchlist fetch happens exactly once per poll batch regardless of post count — not once per post — closing ADR-0058 Decision §6's own named N+1 hazard.
- `publishEvent()` for `SocialPostIngestedEvent` is called only after `insertSocialPost()`'s own promise has resolved (post-commit), never from inside an open transaction — proven by ordering assertion in the contract (the DB write is observably complete before the publish call fires).
- `runIngestionAttempt()` calls `deriveConnectorHealth(tenantId, platformId)` once immediately before opening the run and once immediately after closing it (success or failure), publishing exactly one `ConnectorHealthChangedEvent` (`buildConnectorHealthChangedEvent()`, unmodified shape) when and only when `status` differs between the two snapshots — proven by a test driving a connector from `healthy` to `failing` across one attempt and asserting exactly one event with the correct `previousStatus`/`newStatus`, and a second test asserting zero events when status is unchanged.
- Every publish call site (both event types, all four call locations — three connectors plus `runIngestionAttempt()`) wraps `publishEvent()` in a catch that logs and swallows, never re-throws — proven by a test that a rejected/throwing `publishEvent()` does not fail or roll back the surrounding ingestion attempt (matching `enrichPost()`'s own "never blocks ingestion" precedent, ADR-0038).
- `docs/adr/README.md`, `docs/user-stories/README.md`'s Epic 5 row, and `ingestion-events/SKILL.md`'s own "Known gaps" section are all updated to reflect real publishing now exists — the SKILL.md gap this story closes must actually say so, not be left describing a state this story has already fixed.

**Explicitly out of scope (per ADR-0058 Decision §6's own naming, not silently deferred):** throttling or batching multiple `SocialPostIngestedEvent`s from a single post matching many watchlists — real Service Bus message-throughput is watched during this story's own staging validation instead, not solved architecturally here. Wikipedia's own connector (Story 2.13) is not touched by this story — it adopts this same pattern as part of its own separate, still-paused build.

**Documentation Steward correction, 2026-08-19.** This story's own header already carried a correct, real `**Built:** 2026-08-17 — social-listening-core@3ef32ad` field (confirmed directly against `docs/implementation-log.md`'s own matching 2026-08-17 entry), but `**Status:**` still read "Ready." Now reads "Built 2026-08-17," matching the `**Built:**` field and the Log; no Acceptance Criteria text changed.
