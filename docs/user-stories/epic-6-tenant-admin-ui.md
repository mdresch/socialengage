...
**Source:** Phase 1 "also build, not storied" (`docs/implementation-plan.md`), against Story 4.3's derived `ConnectorHealth` · **Status:** Ready, with a named, real backend gap (unchanged, see below) — previously marked "Built" in error; see the 2026-08-12 rework note below. Practically sequenced after Story 6.1/6.3.
...
**Source:** Phase 1/Phase 3 "also build, not storied" (`docs/implementation-plan.md`), against Story 3.4's real `GET /v1/posts` REST surface (ADR-0011 cursor pagination) and Story 5.1's `GET /v1/posts/:id` (ADR-0012) · **Status:** Ready — no new ADR needed, both endpoints are already Accepted/built; same "ordinary CRUD/read surface" precedent Stories 6.4/6.8/6.9 were originally sourced from.
