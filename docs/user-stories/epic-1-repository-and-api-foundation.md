...
**As a** Tenant-Admin who has connected, verified, and activated a connector,
**I want** that connector to actually poll on a real, recurring cadence without me doing anything further,
**so that** "active" in the connector status screen means "actually ingesting," not merely "configured."

**Acceptance Criteria**

- `bootstrapConnectors()` (new) registers every real `SocialConnector`/`AIProviderConnector` module (GNews, Newswire, tenant-owned-feed, Azure AI Language, Azure OpenAI) into the shared registry (`registry.ts`) exactly once, called from `server.ts`'s `main()` before the HTTP listener starts accepting traffic and before the scheduler's first tick — proven by a test confirming `listSocialConnectors()` returns every real poll-mode connector after real startup, where it returned `[]` before this story (ADR-0052 Decision §3).
- `SocialConnector` gains two new optional members, `poll?(tenantId: string): Promise<RunIngestionAttemptResult>` and `pollCadenceMs?: number`; GNews, Newswire, and tenant-owned-feed's registered connector objects each gain a `poll()` wrapper delegating unchanged to their existing, already-contract-verified `pollGNewsSearch()`/`pollNewswireFeeds()`/`pollTenantOwnedFeed()` functions, plus a `pollCadenceMs` value (GNews 15 min, Newswire 15 min, tenant-owned-feed 30 min per ADR-0050's own already-decided number, not re-litigated) — proven by a test asserting each registered connector's `poll()` produces an identical result to calling its underlying function directly (ADR-0052 Decision §4).
- The scheduler never hardcodes a per-`providerId` switch/map to a poll function — every invocation goes through `getSocialConnector(platformId)?.poll?.(tenantId)` — proven the same way Story 2.10/ADR-0048's own CI guardrail already proves this for every other core-pipeline file (no literal `providerId` branching in the scheduler module itself).
- A single in-process loop, started only from `server.ts`'s `main()` (never from `createApp()`), ticks every 60 seconds (configurable) and, per tick, evaluates every `(tenant, poll-mode connector)` pair from `listTenants() × listSocialConnectors().filter(c => c.deliveryMode === 'poll')` — proven by a test that starts the scheduler against a fixture of tenants/connectors and confirms every eligible pair is considered exactly once per tick (ADR-0052 Decision §1/§2).
- A pair is polled only if `shouldAttemptIngestion(tenantId, platformId)` (default `ownerType: 'tenant'`) returns `true` **and** `now - last_started_at >= connector.pollCadenceMs * (1 + jitterFraction(tenantId, platformId))`, where `last_started_at` is that pair's most recent `ingestion_runs.started_at` (or treated as unconditionally due if none exists) and `jitterFraction` is a deterministic, hash-derived value in `[-0.05, +0.05]` computed once per pair (never `Math.random()`, never recomputed per tick) — proven by: (a) a test confirming an inactive or `failing`-health connector is never polled regardless of elapsed time; (b) a test confirming a pair under its jittered cadence is not polled; (c) a test confirming two `(tenantId, platformId)` pairs with identical `pollCadenceMs` and identical `last_started_at` receive different, but individually deterministic and repeatable, jittered due-times across repeated evaluations (ADR-0052 Decision §5).
- A `poll()` call's own `ClassifiableError` (raised via `runIngestionAttempt()`, e.g. a rate-limit or credential failure) propagates out of `poll()` uncaught and is recorded exactly as `runIngestionAttempt()` already records it (an `IngestionRun` row; `ConnectorHealth`/auto-disable derivation unaffected) — proven by a test that a scheduler-triggered poll failure updates `ConnectorHealth` identically to a directly-invoked poll failure. A non-`ClassifiableError` exception from a `poll()` call is caught and logged at the scheduler level and never halts the loop for any other pair in the same or a later tick — proven by a test that one pair throwing a raw (non-`ClassifiableError`) exception does not prevent every other eligible pair's poll from running in the same tick (ADR-0052 Decision §8, both directions of the invariant).
- The scheduler is gated by a new `SCHEDULER_ENABLED` env var (default: on outside `NODE_ENV === 'test'`, off inside it) in addition to only ever starting from `main()` — proven by a test confirming `createApp()` alone (the construction path every contract test already uses) never starts a background timer, and confirming the full accumulated contract suite (`npm test`) makes zero real outbound calls to GNews/Newswire/any tenant feed as a side effect of any test file that doesn't itself explicitly invoke a poll function (ADR-0052 Decision §9).
- Scope is `ownerType: 'tenant'` poll connectors only — no user-scoped (`connector_user_activations`) pair is ever enumerated or polled by this story's scheduler, proven by a test confirming a user-scoped activation row with no matching tenant-scoped activation is never polled (ADR-0052 Decision §6).
- No distributed lock, no Redis, no multi-instance coordination is built — a single scheduler instance is the only supported deployment shape for this story, matching this project's actual current single-instance shape (ADR-0052 Decision §7). Named here to keep this story's own scope honest against that Decision, not proven by a test — there is nothing to assert the absence of.

**Explicitly out of scope, per ADR-0052's own Decision §6/Open Questions:** Tier-3 (user-bound) poll connector scheduling — no real one exists yet (Reddit unbuilt), though the future `poll(tenantId, userId)` / `RequestGate` key `(tenantId, userId, providerId)` shape is already named in ADR-0052 Decision §6 for whoever eventually builds it; multi-instance distributed locking (`pg_try_advisory_lock` named as the likely future mechanism, not built here); a cost/quota budget ceiling or spend alerting for continuous polling — a real, unresolved gap ADR-0052 itself flags directly for Menno, not this story's to close; watchlist-driven query construction for GNews (`pollGNewsSearch()`'s hardcoded `'technology'` default stays exactly as-is — a separate "what to poll for" gap, not this story's "when/how to trigger polling" scope); `social-listening-admin` UI surfacing of scheduler activity (e.g. a "last polled at" indicator on the connector status screen) — a future, separate UI story, not bundled here.

**Built 2026-08-13** (`social-listening-core@a479383`, `contracts/epic-1/story-1.13.live-ingestion-polling-scheduler.contract.test.ts`, 16/16, full suite 58/58 suites / 425/425 tests — see `docs/implementation-log.md`). `bootstrapConnectors()` is the real production call site for connector registration; `pollScheduler.ts`'s in-process `setInterval` loop, gated by `SCHEDULER_ENABLED` and started only from `server.ts`'s `main()`, actually invokes each poll-mode connector's `poll()` on the derived, jittered cadence this story's own AC describes. **Documentation Steward correction, 2026-08-13/14:** this Status line and the Acceptance-Criteria-only framing above were stale — the same commit (`a479383`) that added this story's text to this file also shipped the real code and a real, passing contract (confirmed directly against `docs/implementation-log.md`'s own matching entry and `docs/implementation-plan.md`'s/`docs/user-stories/README.md`'s own "Story 1.13 built" dated notes, both added in that same commit), but this file's own Status line was left reading "Ready" with no "Built" note ever appended, the one file of the three touched by that commit not updated to match. Corrected here; no Acceptance Criteria text changed.
...
**Drafted 2026-08-18**, following a real, live investigation this session into a reported Wikipedia duplication/missing-posts symptom. Investigated directly against the dev database before concluding anything (per this project's own root-cause-first discipline): zero actual duplicate `(tenantId, providerId, externalId)` rows existed in `social_posts` at investigation time — the specific "Microsoft Edge repeatedly published" complaint traced mostly to the Wikipedia connector's own correct, if noisy, one-post-per-real-edit behavior on a heavily-edited article, not a dedup failure. But the same investigation found real, concrete evidence of an actual gap: four Wikipedia `ingestion_runs` rows stuck at `status: 'running'`, started 1–14 minutes apart — far more often than the connector's own 30-minute `pollCadenceMs` should ever allow — and one real completed run that took 28 minutes, close enough to its own cadence for a same-process overlap to be genuinely plausible (51 tracked articles, all sequentially re-polled every cycle, per `wikipedia-connector/SKILL.md`'s own named inefficiency). `pollScheduler.ts`'s eligibility check (ADR-0052 Decision §5) is elapsed-time-only — it never checks whether the pair's most recent run has actually finished — so a slow cycle, or a prior run interrupted by a dev-server restart and left permanently `'running'`, lets the next eligible tick start a second, overlapping poll for the same `(tenantId, platformId)` pair. `insertSocialPost()`/`findSocialPostByExternalId()` has no database-level uniqueness constraint, so a genuine overlap could silently produce a real duplicate row — a risk confirmed plausible by this session's own evidence, not merely theoretical.

**As the** operator of a single `social-listening-core` instance,
**I want** the scheduler to never start a second poll for a `(tenant, platform)` pair whose most recent run hasn't finished,
**so that** a slow poll cycle or an interrupted prior run can't cause an overlapping, potentially duplicate-producing re-poll.

**Acceptance Criteria**

- `runSchedulerTick()`'s eligibility comparison (`pollScheduler.ts`) gains one additional, mandatory condition on top of ADR-0052 Decision §5's existing elapsed-time check: a pair is treated as due only if its most recent `ingestion_runs` row for that `(tenantId, platformId)` is **not** `status: 'running'` — proven by a test confirming a pair whose most recent run is `'running'` is never polled, even when its jittered cadence has genuinely elapsed.
- A pair with no prior run, or whose most recent run is `'succeeded'`/`'failed'`, is unaffected — proven by the existing Story 1.13 AC5 tests (elapsed-time-under-cadence, no-prior-run-is-due) continuing to pass unmodified, plus a new test confirming a `'succeeded'`/`'failed'` most-recent run is still evaluated purely on elapsed time, same as today.
- This is implemented as a strictly narrower version of the existing comparison (removes false-positive "due" evaluations, never adds one) — no change to `jitterFraction()`, no change to the cadence formula itself, no new stored field (ADR-0052 Decision §5b's own explicit scope boundary against reopening §5).
...
## Story 1.15 — Tier-3 (user-bound) poll scheduling: per-user enumeration, cross-user health/in-flight isolation

**Source:** ADR-0061 (Accepted 2026-08-18) · **Status:** Built 2026-08-18. Accepted as drafted, verbatim: *"ADR-0061 is complete, coherent, and ready for my acceptance. It cleanly bridges the gap between ADR-0060's data model and the background scheduling engine, resolving all implicit cross-user race conditions before they ever manifest in production. Excellent work and I approve ADR 0061"* — no review-round amendments, so every Acceptance Criterion below is unchanged from the original drafting pass.

**Built:** 2026-08-18 — social-listening-core@b270662

**Documentation Steward correction, 2026-09-09.** Status line read "Ready" directly beside its own already-populated `**Built:**` field — confirmed against `docs/implementation-log.md`'s matching entry (`## 2026-08-18, later the same day — Story 1.15 — social-listening-core@b270662`, hash matches exactly). Corrected directly.

**Drafted 2026-08-18, alongside ADR-0061 itself**, per this series' own convention for a story whose source ADR isn't accepted yet (see Story 8.1/8.2/8.3's own original "Blocked — pending ADR-0054" pattern). Resolves ADR-0052 Decision §6/Open Question 2 — the Tier-3 poll-scheduling shape it named without designing, deliberately deferred until a real Tier-3 poll connector existed. One now does: `pollFacebook(tenantId, userId)` (ADR-0060/Story 2.15), confirmed via a repo-wide grep to have zero real call sites in the running server today — a tenant's real, connected Facebook Page is never actually polled by anything. Also fixes, as a direct consequence of the same investigation: a confirmed cross-user in-flight/due-check blending bug (exactly the one Menno asked to have checked — `getMostRecentRunStatus()`/`deriveConnectorHealth()` are keyed by `(tenantId, platformId)` only, with no `user_id` column anywhere in `ingestion_runs` today) and a real, pre-existing cross-user auto-disable false-negative already present in already-shipped `shouldAttemptIngestion()` (Story 1.11) — silently ignoring the `ownerType`/`userId` arguments its own callers already pass it.

**As the** operator of a single `social-listening-core` instance,
**I want** every tenant user's own connected Tier-3 (user-bound) connector — starting with Facebook — to actually be polled on a real, recurring, per-user cadence, without one user's failures or in-flight polls incorrectly blocking another user's,
**so that** a tenant connecting a real Facebook Page (single, or multiple once Story 6.27 ships) sees it actually ingest, the same "connected means actually ingesting" guarantee Story 1.13 already gives every tenant-wide connector.

**Acceptance Criteria**

- `ingestion_runs` gains a new, nullable `user_id UUID REFERENCES users(id)` column, populated only by a Tier-3 connector's own poll path — proven by a test confirming every existing tenant-wide connector's `IngestionRun` rows still write `user_id = NULL` with no behavior change to any existing query or contract (ADR-0061 Decision §1).
- `connectorActivationStore.ts` gains `listActiveUserActivations(tenantId, platformId): Promise<string[]>`, returning every `userId` with an active `connector_user_activations` row for that `(tenantId, platformId)` — proven by a test with three users (two active, one deactivated) confirming exactly the two active `userId`s are returned (ADR-0061 Decision §2).
- `ingestionRunStore.ts` gains `getMostRecentRunStatusForUser(tenantId, platformId, userId): Promise<IngestionRunStatus | null>`, scoped by `user_id`, leaving the existing `getMostRecentRunStatus()` completely unchanged — proven by a test with two users' own `ingestion_runs` rows for the same `(tenantId, platformId)` confirming each user's own call returns only that user's own most recent status, never the other's (ADR-0061 Decision §2).
- `deriveConnectorHealth()` gains a fourth optional parameter, `userId?: string`, filtering both its `ingestion_runs` query and its `platform_credentials`/`credentialStatus` query by `user_id` when supplied; omitted, behavior is byte-for-byte unchanged — proven by (a) re-running every existing `deriveConnectorHealth()` contract test unmodified, and (b) a new test with two users' own runs/credentials for the same `(tenantId, platformId)` confirming `deriveConnectorHealth(tenantId, platformId, undefined, userA)` and `(..., userB)` return independently correct `status`/`credentialStatus` values, never blended (ADR-0061 Decision §2). **If Story 6.27 (ADR-0060) has already shipped its own third `pageId?` parameter by the time this story is implemented, `userId` is added as the fourth parameter; if this story ships first, the third (`pageId?`) parameter is still reserved, structurally present but unused, so Story 6.27's own already-written Acceptance Criteria remain valid regardless of build order** — proven by a test confirming the final four-parameter signature compiles and behaves correctly regardless of which story added which parameter first.
- `shouldAttemptIngestion(tenantId, platformId, ownerType, userId)`'s own internal `deriveConnectorHealth()` call is corrected to pass `userId` through when `ownerType === 'user'` — proven by a test confirming one user's `failing`-status Tier-3 connector no longer causes `shouldAttemptIngestion()` to return `false` for a different user's own healthy Tier-3 connection on the same `(tenantId, platformId)`, and a regression test confirming the `ownerType: 'tenant'` default path (every existing connector) is completely unaffected.
- `SocialConnector` (`types.ts`) gains a new, distinct optional method, `pollUser?(tenantId: string, userId: string): Promise<RunIngestionAttemptResult>`, alongside — never replacing — the existing `poll?(tenantId)`; `pollCadenceMs` is reused unmodified for either dispatch path — proven by a type-level test confirming a connector may declare `poll`, `pollUser`, both, or neither, and a runtime test confirming the scheduler enumerates each independently (ADR-0061 Decision §3).
- `bootstrapConnectors.ts`'s Facebook registration is revised: the throwing-placeholder `poll` is removed entirely, and `pollUser: pollFacebook` is registered instead, with its existing `pollCadenceMs` (30 minutes) unchanged — proven by a test confirming Facebook's registered connector object has no `poll` property at all, and that `pollUser(tenantId, userId)` delegates to `pollFacebook()` unchanged (ADR-0061 Decision §3).
- `runSchedulerTick()` gains a third, independent enumeration level: for every tenant, for every registered connector with a `pollUser` method and a `pollCadenceMs`, for every `userId` returned by `listActiveUserActivations(tenant.id, platformId)`, the identical due-check shape the tenant-wide path already uses (`shouldAttemptIngestion(tenantId, platformId, 'user', userId)`, the elapsed-time-plus-jitter comparison against that user's own most recent run, and the Story 1.14 in-flight guard via `getMostRecentRunStatusForUser()`) gates a call to `connector.pollUser(tenant.id, userId)` — proven by a test with two users under one tenant, each with their own connected Facebook credential and their own independent `ingestion_runs` history, confirming each user's own `pollUser()` is invoked (or correctly skipped) based solely on that user's own due-check, never the other's (ADR-0061 Decision §4).
