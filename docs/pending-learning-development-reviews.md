...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** This is this very role's own prior scheduled pass (2026-08-24) — it already updated `docs/manuals/tenant-admin-manual.md` and `docs/manuals/user-manual.md` directly for the four connectors and screens it lists, and both manuals' current "Current coverage" notes (dated 2026-08-24) already reflect that pass's own content. Nothing further to add on this commit's own account.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — this is the Documentation Steward's own internal traceability pass (ADR README, implementation plan, story files), not product code or UI.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Real change, at the time already reflected accurately — confirmed against the actual diff that `handlePublish` was still explicitly commented `// Simulated Publish` and still awaited a fake `setTimeout`, so this commit only added the real "pick which of your connected Facebook Pages to send to" dialog in front of that still-simulated send, plus route-level loading skeletons (no manual-worthy content of its own). **However, reviewing this commit surfaced a real staleness this pass then corrected directly:** `docs/implementation-log.md` records a subsequent Story 6.39 (`social-listening-admin@e0abfdd`, 2026-08-26, contract-verified 10/10, "polypost-composer-real-publish-flow") that makes Facebook publishing genuinely real — confirmed by reading that contract test and the current `PolypostComposer.tsx`/`PublishTargetsDialog.tsx` source directly (real `POST /api/outbound/posts` call, real per-Page success/error handling, other networks shown as "not yet available for publishing"). That commit has no entry anywhere in this queue file (the post-commit hook appears to have missed it — a gap worth the Documentation Steward or Ideal Manager's attention, not something this role can fix in its own queue). Since both manuals' "Drafting a cross-platform post" sections were about to be edited on this pass's account regardless, they were corrected in the same edit rather than left visibly wrong: both now say Facebook publishing is real (Story 6.39) and every other network is still not available for publishing.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — adds internal research skills for the Devin agent role. No product code or UI touched.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — this commit only adds internal Business-Requirements-Document (BRD) paperwork (a template plus one retrospective BRD per existing ADR 0001–0121, including many still-Proposed roadmap ADRs) via the Devin `brd-writer` skill. No product code, screen, or endpoint touched.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Backend-only, no screen — Story 2.26's `social-listening-core` half: a connector reply framework and outbound rate gate (ADR-0073), confirmed against the file list to touch only `social-listening-core/src` and its own skills/contract, no `social-listening-admin` files. Nothing here is reachable from any manual's tier yet.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — appends Story 2.26's own Implementation Log entry, reviewed separately above (e3df7d9).
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Backend-only, no screen — Story 2.31 adds one-off research search helper functions to the existing Brave/Bing connectors (`social-listening-core/src/connectors/...`), with no `social-listening-admin` files touched, so no screen consumes this yet. Both connectors' own consumer-facing UI (Stories 6.30/6.32) is already documented separately.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — appends Story 2.31's own Implementation Log entry, reviewed separately above (4949200).
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — this commit only adds internal Functional-Design-Document (FDD) paperwork (a template plus one retrospective FDD per existing ADR/BRD 0001–0121) via the Devin `fdd-writer` skill. No product code, screen, or endpoint touched.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Backend-only, no screen — Story 2.27's `social-listening-core` half: the Facebook connector gains a real Page-reply implementation on top of the Story 2.26 reply framework. Only `social-listening-core/src` and its own skill/contract touched; no admin UI wired to it in this commit.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — appends Story 2.27's own Implementation Log entry, reviewed separately above (2da26eb).
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — updates the `implement-story` skill's own process instructions to pull in BRD/FDD context; internal process tooling, not product code or UI.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — the same process update as c63752e immediately above, applied to the `heal-contract-failure` skill instead.
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Backend-only, no screen — Story 2.28's `social-listening-core` half: a generic connector publish framework and outbound-post rate gate (ADR-0075), the shared plumbing later used by the real Facebook/LinkedIn publish stories. Only `social-listening-core/src` and its own skills/contract touched.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — appends Story 2.28's own Implementation Log entry, reviewed separately above (f1e0f9b).
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Backend-only, no screen — Story 2.29's `social-listening-core` half: real Facebook Page post publishing built on the Story 2.28 framework. Only `social-listening-core/src` and its own skill/contract touched; no admin UI wired to it yet in this commit (that lands separately as Story 6.39, still not reflected anywhere in this queue file — see the note under 6a8b69e above).
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — appends Story 2.29's own Implementation Log entry, reviewed separately above (4234834).
...
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** Backend-only, no screen — Story 2.30's `social-listening-core` half: real LinkedIn post publishing built on the same Story 2.28 framework. Only `social-listening-core/src` and its own skill/contract touched; the admin composer still shows LinkedIn as "not yet available for publishing" as of Story 6.39 (see current `PublishTargetsDialog.tsx`), so this backend capability isn't reachable from any screen yet.
...
- **Files touched:** docs/implementation-log.md, docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md
- **Status:** ~~Pending review~~ **Reviewed 2026-08-28 (Learning & Development Writer, scheduled queue pass):** No user-facing change — appends Story 2.30's own Implementation Log entry, reviewed separately above (3a65691).
