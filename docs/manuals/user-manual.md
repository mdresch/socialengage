...
**Current coverage, as of 2026-08-20:** sign-in, plus every tenant-facing screen a Tenant User (as opposed to a Tenant-Admin) can actually reach in `social-listening-admin`'s tenant screen tree — connecting your own personal platform credential and turning it on or off, now across GNews, Wikipedia, and your own Facebook Page (Stories 6.3, 6.15, 6.21, 6.23), managing your own watchlists, including Wikipedia as a selectable source (Stories 6.4, 6.22), checking connector status, now including stalled-ingestion alerts (Stories 6.5, 6.29), viewing your tenant's user list on a redesigned Team & access screen with a real seat-utilization meter (Story 6.8, restyled 2026-08-17), a read-only view of your tenant's own settings (Story 6.9), browsing your tenant's ingested posts — newest-first, with search and filtering that now covers every ingested post rather than just the current page, and a manual "run enrichment now" button (Stories 6.11, 6.16, 6.18, 6.19, 6.25, 6.26), and a full Analytics dashboard covering post volume, sentiment, conversations/key phrases, languages, per-source breakdowns, watchlist coverage, and country-level geospatial insights (Stories 8.1–8.10). **All of these screens are real, backed by your tenant's actual data, with working actions.** **Corrected 2026-08-19: setting up your own company domain's content feed (Story 6.12) is no longer something a Tenant User can do** — Story 6.20 (2026-08-17) restricted that entire screen to Tenant-Admins only; see "What's not built yet" below. A few things stay a Tenant-Admin's job only, called out plainly wherever that's the case — this manual only ever describes what a Tenant User can actually do, never an admin-only screen in detail. The app also gained its first real stylesheet on 2026-08-12, and the main content area was widened from 900px to 1280px on 2026-08-18 — both purely visual changes, nothing about how any screen below works changed because of either.
...
Signing in lands you on this screen — a real, at-a-glance summary of your tenant.

1. Four metric cards show your tenant's real, live counts: Active Watchlists, Ingestion Connectors, Ingested Posts, and Seat Utilization (active users out of your tenant's licensed seats, with a progress bar). Each card links to the relevant screen.
2. If any connector is reporting a degraded or failing response, a warning banner appears above the cards.
3. A "Recent Ingestion Stream" panel shows your most recently ingested posts, most-recently-ingested first (corrected 2026-08-20), each with its source and sentiment where already enriched.
...
5. **Stalled-ingestion alerts (Stories 1.16, 6.29).** If a connector's ingestion has gone quiet in a way that isn't a normal failure — stuck rather than merely erroring — SocialEngage now detects it and marks that connector "Stalled," shown with its own distinct badge separate from Active/Inactive/failing. A banner also appears at the top of this screen (and on your tenant overview) whenever any connector is stalled. **Triggering an immediate re-sync from that banner is a Tenant-Admin action** — as a Tenant User you'll see the banner and the badge, but not the "Re-sync now" button itself.
...
1. From your tenant's screens, open "Posts" to see what SocialEngage has ingested for your tenant — **newest-ingested first** (Story 6.25), each with the platform it came from, the author where known, a title and body preview, and its published date. If a post has already been analyzed by an AI provider, you'll also see its sentiment, key phrases, entities, and the detected language right in the list. **As of a 2026-08-20 refinement:** a post from a Newswire wire service now correctly shows the wire service itself in the author position; a post from a Facebook Page shows that Page as the author, together with a direct link back to the original post on Facebook.
...
Open "Analytics" from your tenant's screens to see aggregated, dashboard-style views over your tenant's own ingested posts — every number here is computed live from your real posts, never a fabricated or placeholder figure. **The Overview tab was substantially rebuilt on 2026-08-19/20 (Story 8.7 and its follow-ups) — the simpler three-card layout described in earlier versions of this manual no longer exists.**

1. **Date range, in the header.** A date-range picker at the top controls every tab at once — pick a standard range (Today, Last 7/14/30 Days, This Month, Last Month, All Available History) or a custom start/end date. **As of Story 8.7, there is no "Compare to previous period" toggle or percentage-change figure anywhere on this dashboard any more.**
2. **Overview tab — an 8-widget grid, filterable by clicking almost any widget.** Clicking a source, author, keyword, sentiment, language, watchlist, or (since Story 8.10) a country narrows every other widget on the tab at once, with removable filter chips and a shareable filtered link. The grid includes a Sentiment Gauge, an Authors-by-Source breakdown (each source with its own real platform icon/brand color), a post-volume-over-time chart with a short-term forecast, a keyword cloud, a Languages breakdown, a Sources volume breakdown (with real platform icons and percentage bars), a Top Authors list, and an AI Spike Storyteller that can explain a volume spike on request. Clicking through a filtered widget opens a drawer of matching posts, and clicking a post there opens its full detail without leaving the tab.
3. **Watchlist Coverage and the Topics filter (Story 8.9).** A "Topics" dropdown filters the Overview tab to just the posts matching one of your own watchlists; a Watchlist Coverage donut chart shows real post counts for your top 6 active watchlists by volume.
4. **Location & Geospatial Insights (Story 8.10).** An interactive world map and a ranked "Top Countries" list show which countries your ingested posts are associated with, with an honest "Unknown" bucket for posts with no detectable country. Click a country to filter the Overview tab down to just that country. **This is what closes the "no Location tab" gap named in earlier versions of this manual.**
...
6. **Conversations tab.** A key-phrase cloud and a top-phrases-over-time chart, plus a Languages breakdown — the real, detected language of each enriched post, ranked by count.
7. **Sources tab.** Post volume by source, each with its own real platform icon, a detail list per source with its own post count, sentiment counts, and a real 0–10 weighted sentiment score — honestly blank when a source has no enriched posts yet — and a volume-over-time chart broken out per source.

**Current limitations:** there is no per-widget export, and no period-over-period percentage-change figure anywhere on the dashboard as of Story 8.7.
...
- **Per-widget export, and any period-over-period percentage-change figure, on the Analytics dashboard** — see above.
