...
**Learning & Development Writer addendum, 2026-09-18 (scheduled queue pass, clearing the remaining 25-entry backlog through `aadb1795`):** this manual's body (though not its own addendum notes, until now) had already caught up through the Epic 12–18 batch via an earlier in-progress checkpoint commit (`b9e5215`) — the boolean query builder, aspect-level sentiment breakdown, Topic Evolution's semantic drift explanation, the composer's Deep Research button, role-tailored onboarding tabs (Story 17.2), and watchlist volume/cost preview (Story 18.1) sections below were all already present and were re-checked against real source this pass, not re-written. Two real corrections and one real addition this pass. First correction: the "📊 Preview volume" button (Story 18.1) is offered while both creating and editing a watchlist — confirmed by reading `WatchlistForm.tsx` directly, which renders the button in both its `mode === 'create'` and `mode === 'edit'` branches — not edit-only as the section previously said. Second correction: the Analytics dashboard's "four tabs" claim was stale — a fifth, real, contract-tested "Location" tab (distinct from the existing Location widget on the Overview tab) is genuinely wired in and now documented below; it carries no story or ADR citation of its own and conflicts with this project's own ADR-0054/ADR-0064 decisions on Location tab scope, a real governance conflict already flagged for Menno elsewhere (see the matching, fuller note in the Tenant Admin Manual) and not re-litigated here. The remaining queued commits were all either genuinely backend-only with no `tenant_user` surface (Story 18.2's analytics-view RLS enforcement; Stories 19.1–19.3's RAG namespace isolation, none of which touch `social-listening-admin`), doc-only backfills/telemetry syncs, merge commits, or other AI roles' own scheduled-review checkpoints outside this role's file scope. The queue (`docs/pending-learning-development-reviews.md`) has no pending entries left as of this pass.

**Learning & Development Writer addendum, 2026-09-13 (scheduled queue pass, continuing oldest-first through the Epic 11 batch):** added six genuinely new, previously-undocumented capabilities, all confirmed built and contract-verified against real source and `docs/implementation-log.md`: pushing a post or lead to a connected CRM (Dynamics 365, Salesforce, or HubSpot — Stories 11.1/11.2), a daily digest email with a live preview (Stories 11.3/11.4), a Topic Evolution timeline page (Stories 11.5/11.6, reachable only by direct URL — no menu link exists yet), a second, separate outbound-posts composer/queue screen that can genuinely publish to both Facebook and LinkedIn, ahead of the older cross-platform composer (Stories 11.7/11.8, also reachable only by direct URL), a Social Inbox worklist (Stories 11.9/11.10, linked from the sidebar), and @mention suggestions inside the new outbound composer (Stories 11.11/11.12). This also corrected a now-stale gap note that said LinkedIn outbound publishing had no UI trigger — it does now, just not from the older composer. Many of the underlying commits queued for this pass (`docs/pending-learning-development-reviews.md`) reference commit hashes that no longer exist in this repository's own history (an artifact of squash-merges from per-story git worktrees changing the hash that lands on `main` — see `CLAUDE.md`'s own note on the worktree-per-story workflow); where that happened, this pass verified the underlying capability directly against current source and `docs/implementation-log.md`'s own entry for the same story (which carries a real, resolvable hash) rather than against the queue file's stale hash. This pass still did not reach the remainder of the backlog (Epic 12 onward — connector capability matrix, boolean query builder, dashboard widgets, webhooks, RBAC, influencer discovery, and more, several of which already have real routes under `social-listening-admin/src/app/tenant/`); left pending for a future pass.
...
6. **A "📊 Preview volume" button (Story 18.1)**, offered both while first creating a watchlist and while editing an existing one, estimates how many posts your current query would actually match, plus a rough cost/quota indicator, flagging unusually high volume or heavy quota use.
...
Open "Analytics" from your tenant's screens to see aggregated, dashboard-style views over your tenant's own ingested posts — every number here is computed live from your real posts, never a fabricated or placeholder figure. The dashboard has five tabs — Overview, Sentiment, Conversations, Sources, and Location — but the Overview tab was substantially rebuilt on 2026-08-19/20; the description below reflects the rebuilt version.
...
6. **Location tab.** A separate tab (distinct from the Location widget on the Overview tab above), built around a large interactive world map, with sentiment broken down by country and by continent, top countries and cities lists, a donut showing whether your posts' locations came from an author's profile or the post content itself, key phrases grouped by country, and a languages breakdown — filterable by clicking a country, continent, language, or phrase. **Not cited to a story number** — this one was found by reading real source rather than any story or ADR; flagged as a documentation gap, not something this manual invents a citation for.
...
Separate from the cross-platform post composer described above, an outbound-posts screen lets you compose a post for one or more of your connected platforms and either publish it immediately or schedule it for a specific future date and time. Scheduled posts sit in a queue you can filter by status (Scheduled, Published, Failed, Cancelled), and from there you can cancel a still-pending post or move its scheduled time. Publishing genuinely reaches Facebook and LinkedIn from this screen (the only two platforms with a real send capability so far) — this is notably ahead of the older cross-platform composer above, which only sends to Facebook. As of this writing there is no menu link to this screen — reach it directly at `/tenant/posts/outbound`.

## Social inbox (Stories 11.9, 11.10)

Open "Inbox" from your tenant's screens for a combined, priority-sorted worklist of items needing a response, drawn from your ingested posts. Filter by Urgent, High Priority, Snoozed, or Resolved, or search by keyword. Selecting an item opens a detail pane showing its sentiment and reach, and lets you assign it to a teammate, snooze it, add an internal note visible only to your team, reply to it inline, or mark it resolved.
...
## Webhook notifications (Story 10.12/12.11)
