...
**Learning & Development Writer addendum, 2026-09-16 (scheduled queue pass, continuing oldest-first from `be3807d` through `97a52d4e`, 2026-08-18 through 2026-09-15 by commit date):** confirmed and added the same six genuinely new, reachable capabilities documented in full in the Tenant Admin Manual's own matching addendum, all of them available to you the same way as to a Tenant-Admin since none is Tenant-Admin-restricted: the visual boolean query builder and a volume/cost preview on the watchlist form (Stories 12.3/12.4, 18.1), a per-post aspect-level sentiment breakdown (Stories 12.5/12.6), real server-side image storage in the cross-platform composer (Story 13.9), a semantic drift explanation on the Topic Evolution page (Stories 13.11/13.12), a "🔬 Deep Research" button in the composer (Stories 14.3/14.4), and role-tailored onboarding journey tabs (Story 17.2). Also added a citation for Story 13.14 to the already-documented Prospecting-list export/CRM-push buttons. Several Epic 12 "frontend" stories built real, contract-tested components never actually wired into any page (topic curation, dashboard widget renderer, webhook management, workspace/RBAC settings, influencer discovery — Stories 12.8/12.10/12.12/12.14/12.16) — named in "What's not built yet," not documented as usable. This pass cleared the full 193-entry backlog it started with; everything else queued in that range was governance/tooling, telemetry, internal test healing, backend-only work with no admin-UI contract at all, or a merge commit — judged to have shipped nothing end-user-facing. See `docs/pending-learning-development-reviews.md` for the per-commit resolution notes.
...
4. **Conversations tab.** A key-phrase cloud and a top-phrases-over-time chart, plus a Languages breakdown and an entity-mentions breakdown by entity type (top 5) — the real, detected language of each enriched post, ranked by count.
...
1. Write your message once; each network's own live preview card updates as you type. You can attach images (uploaded to real server-side storage as of Story 13.9) with accessibility alt-text, import a Markdown or plain-text document as a starting draft, and ask an AI assistant to rewrite your copy.
