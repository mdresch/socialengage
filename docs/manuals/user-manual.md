...
**Learning & Development Writer addendum, 2026-09-15 (scheduled queue pass, continuing oldest-first through the `docs/pending-learning-development-reviews.md` backlog from commit `4eac552`):** confirmed and documented a batch of real, contract-verified Epic 11/12/13 capabilities: a boolean watchlist's own visual query builder (Story 12.4), a Topic Evolution timeline with semantic drift analysis (Stories 11.5/11.6/13.11/13.12), pushing a post or author to your CRM (Stories 11.1/11.2), a bulk "Push to CRM" for a whole Prospecting list (Story 13.14), a Daily Digest email preference screen (Stories 11.3/11.4), a Webhook subscriptions screen (Story 10.12 — this corrects the note below, which said Epic 10's mislabeled stories had no real UI to document; 10.12's actual shipped capability, despite its wrong title in the stories document, is a real webhook screen), a Plan & seats screen (Story 13.6), a separate "Outbound Publishing & Scheduling" screen giving LinkedIn a real publish trigger for the first time plus live @-mention suggestions (Stories 11.7/11.8/11.12), and a Unified Social Inbox triage screen (Stories 11.9/11.10). **Also found and corrected a stale claim:** Story 6.41's composer Deep Research panel is genuinely built and wired into the real "Compose" screen's "🔬 Deep Research" button — the root `CLAUDE.md`'s own status line still calls Story 6.41 "Ready but not yet built," which is now stale; flagged for the Documentation Steward. Also corrected a real, previously-undocumented redesign of the ordinary Tenant settings screen (Story 6.40, shipped 2026-08-26 but never queued for this role's review) with real Data Export, CRM Connectors, and Notification Preferences cards. Several other Epic 12 "frontend" stories checked this pass (12.2, 12.8, 12.10, 12.16, and the duplicate 12.12) each built a real component with a passing contract test, but none is wired into any page you can actually reach — so none is documented here; flagged for the Documentation Steward.
...
1. Pick a topic from a fixed list (Artificial Intelligence, Customer Support, Product Feedback, Pricing, Security — not yet generated from your own tenant's real topics), a granularity (day/week/month), and optionally "Compare Previous." A stacked bar chart shows mention volume split into positive/neutral/negative sentiment per period, with a trend marker on any period moving sharply up or down. Clicking a bar shows that period's mention count, unique-author count, top keywords, and top contributing authors.
2. **Semantic drift analysis.** Pick a "then" and "now" date range and click "Analyze Drift" to see a real, computed drift score, the top semantic clusters and sample posts from each window side by side, and a warning badge if the drift is significant. A "Generate AI Summary" button asks the AI assistant (the same one behind Discovery's "Ask AI Assistant") to explain the shift in plain language, grounded in the clusters and posts shown.
...
4. **A boolean watchlist's query has its own visual builder now (Story 12.4, ADR-0102).** A "🧭 Guided Builder" / "⚡ Advanced Text" toggle lets you either build the query clause-by-clause (keyword, phrase, hashtag, mention, author, source, sentiment, or date, combined with AND/OR/NOT, with nested groups) or type the raw boolean syntax directly — the two stay in sync, and a "View AST JSON" toggle shows the exact structured query that will be saved. The screen checks your query against the real query limits of whichever platforms the watchlist covers and shows an inline warning for anything a platform can't natively evaluate, or a blocking error if the query exceeds a platform's hard limits.
...
1. From your tenant's screens, open "Tenant settings." A "Workspace Configuration" card shows your tenant's real name, status, domain, how many of its licensed seats are currently used out of the total, and when the tenant was created — read-only, with no way to change anything here.
2. **A "Data Export" card offers two downloads.** "Export Full Workspace (JSON)" is Tenant-Admin only — you'll see it disabled with a "Requires Tenant-Admin role" note. "Export Matched Posts (CSV)" is available to you too, and downloads your tenant's ingested posts.
3. **A "CRM Connectors" card** links to "Manage CRM Connectors" (see "Escalating a post or author to your CRM" above).
4. **A "Notification Preferences" card** links to "Manage Daily Digest Email" (see "Daily digest email" below).
5. An "Offboarding and Decommission" section with a tenant-deletion link only appears here for your Tenant-Admin — you won't see it.
...
A "Push to CRM" (💼 CRM) button — on a post's detail panel, and on each author row inside a Prospecting list — lets you escalate that post or author into your organization's own CRM system as a support case, lead, opportunity, account, or contact record.

1. Before you can push anything, your tenant's CRM connector needs its own credentials configured under "Manage CRM Connectors" in Tenant settings — any tenant member can configure this, not just your Tenant-Admin.
2. Clicking "Push to CRM" opens a dialog: pick which CRM connector to send to, the record type, and optionally an assignee/queue ID and internal notes.
3. Clicking "Escalate to CRM" sends it immediately — this is a real, live write to your organization's own Dynamics 365, Salesforce, or HubSpot environment, not a simulation. On success you get a direct link to the new CRM record.
4. **If that exact item has already been escalated before, you're warned rather than silently creating a duplicate** — you can open the existing record instead, or explicitly choose "Create duplicate anyway."

## Daily digest email (Stories 11.3, 11.4, ADR-0096)

Open "Manage Daily Digest Email" from Tenant settings to set up your own personal morning summary email — this is a per-user preference, separate from your Tenant-Admin's own.

1. Turn the subscription on or off, and choose a delivery time and your own timezone.
2. Choose what's included: an AI-generated executive summary, the top 5 highest-impact conversations, and/or a trending-topics breakdown — and optionally scope it to specific watchlists.
3. "Preview Daily Digest" renders a real, live preview using your tenant's current data before you save anything.

## Webhook subscriptions (Story 10.12, ADR-0092)

Open Tenant settings and go to `/tenant/settings/webhooks` (there's no direct link to it yet) to receive real-time HTTP notifications when an alert fires or a crisis-wizard threshold is exceeded. Register your own HTTPS endpoint URL, use "⚡ Test Ping" to send a real signed test payload and confirm it's wired up, and remove an endpoint at any time.

## Plan & seats (Story 13.6, ADR-0112)

Open "Plan & seats" from your tenant's screens to see your tenant's current plan name, a real seat-usage meter, and which features your plan currently makes available. A feature your plan doesn't include (for example, CRM push or data export) is shown as disabled here with a note to contact your Platform Admin, rather than just failing silently elsewhere in the app.
...
4. **Conversations tab.** A key-phrase cloud and a top-phrases-over-time chart, plus a Languages breakdown — the real, detected language of each enriched post, ranked by count — and an Entity mentions breakdown by entity type (person, organization, location, and so on).
...
3. **"🔬 Deep Research" is real, not simulated.** Once you've written at least 10 characters, clicking it researches the public conversation around your draft using your tenant's connected search provider and returns key phrases, a written summary, how your draft compares to what's actually being said, and real source links. This needs an active Azure OpenAI connection and an active Brave or Bing Search connection on your tenant — if either is missing, you're told which one to connect.
...
A separate "Outbound Publishing & Scheduling" screen — distinct from the "Compose" screen above — lets you schedule a post for later rather than only publishing immediately, and gives LinkedIn a real publishing option too.

1. The screen lists every outbound post your tenant has queued, sent, or attempted, filterable by status, each with "Reschedule" and "Cancel" for anything still scheduled.
2. "✍️ Compose Post" offers **LinkedIn, Facebook, Bluesky, and X/Twitter** as targets, with Page/account targeting for LinkedIn and Facebook. Write your text, choose "Publish now" or "Schedule for later," and submit.
3. **As you type, an "@" mention gets live suggestions** drawn from real accounts on your selected platforms — click one to insert it.
4. **LinkedIn and Facebook publish for real** through this screen, immediately or at your scheduled time.

**Current limitations:** Bluesky and X/Twitter are offered as targets here too, but neither has a real connector behind it yet — selecting either and submitting will fail. This screen only offers "Reschedule" and "Cancel" for a post that's still scheduled — there's no way yet to edit or delete one that's already published.

## Unified Social Inbox (Stories 11.9, 11.10)

Open "Inbox" for a dedicated triage view — a filterable queue of individual mentions on the left, and a detail-and-reply pane on the right.

1. Selecting an item shows its content and priority, with actions to **Snooze** it, **Resolve** it, or **Reply** to it directly.
2. **Replying here is real, not simulated, for any platform whose connector supports it** — today that's Facebook only, the same real send described under "Replying to a post" above; replying also automatically marks the item Resolved.

**Current limitation:** nothing in the app currently adds an item to this inbox queue — there's no "send to inbox" action anywhere else yet, so the Inbox will show "No conversations in this view" for a real tenant today, even though triage, snoozing, resolving, and replying all genuinely work once an item exists.
...
3. **Export CSV** and **Push to CRM** are both available from a list's own page, but only to the list's owner — not to anyone it's merely shared with, and only if your tenant's plan includes those features (see "Plan & seats" below).
4. **Push to CRM (Story 13.14) pushes the whole list, not just one author at a time.** It opens a dialog listing every entry, each with its own checkbox (all selected by default) so you can leave some out; pick which connected CRM connector to send to and confirm. When it finishes, you're told how many entries were actually pushed and how many were skipped (for example, because they'd already been pushed before) — pushing the same list again never silently double-creates records for an entry that's already there.
...
- **Publishing from the cross-platform "Compose" post composer to anything but Facebook** — drafting and previewing work for all seven networks, and publishing to your own connected Facebook Pages is now genuinely real (Story 6.39, extended by Story 13.10), but Instagram, X/Twitter, Bluesky, Mastodon, and Threads are still "not yet available for publishing" from that screen. **LinkedIn now publishes for real, but only from the separate "Outbound Publishing & Scheduling" screen** (Stories 11.7/11.8), not from "Compose."
- **Bluesky and X/Twitter are offered as targets in the Outbound Publishing & Scheduling composer**, but neither has a real connector built yet — selecting either will fail.
- **Replying to a post works for Facebook only** (Stories 2.26/2.27/3.14/6.38, 11.9/11.10) — the Reply button (and the Unified Social Inbox's own reply action) stays disabled or fails for every other platform. If you have more than one of your own Facebook Pages connected, replying uses your most-recently-connected Page's credential, not necessarily the Page the post came from.
- **Nothing in the app currently adds an item to the Unified Social Inbox queue** — the queue stays empty in practice, even though triage, snoozing, resolving, and replying all genuinely work.
...
- **The Topic Evolution screen's topic picker is a fixed list**, not yet generated from your tenant's own real topics.
- **No in-app link to the Webhook subscriptions screen** from Tenant settings yet — it's real and works, but you have to go to `/tenant/settings/webhooks` directly.
- **Editing or deleting an already-published outbound post** — the Outbound Publishing & Scheduling screen only offers Reschedule/Cancel for a still-scheduled post.
