...
**Learning & Development Writer addendum, 2026-09-10 (scheduled queue pass, continuing oldest-first from `4eac552`):** added six new screens from Epic 11, verified by reading current source and their own contract tests since a large share of this pass's queue-recorded commit hashes no longer exist in this repository's real git history (evidently lost to a later rebase/squash/bulk-merge — the underlying files still exist, just under different, later commit hashes; flagged for the Documentation Steward, not something this role's file can fix) — connecting a CRM system and pushing to it (Stories 11.1/11.2), daily digest email preferences (Stories 11.3/11.4 — real gap: no email is ever actually sent), the Topic Evolution timeline (Stories 11.5/11.6 — real but unlinked from anywhere in the app), a separate Outbound Publishing & Scheduling screen (Stories 11.7/11.8 — real for LinkedIn/Facebook only), and the Unified Social Inbox (Stories 11.9/11.10 — real gap: a reply to anything but Facebook/LinkedIn is recorded as sent without actually being delivered). Carried forward from the prior pass: three screens confirmed built and contract-verified from Epic 10 — Prospecting lists (Story 10.2), ad-hoc analytics query (Story 10.5), and real-time alerts (Story 10.10) — plus YouTube Data API as an eleventh connector, Tenant-Admin-connected only (Story 10.13; this project's own epic-10 story document mislabels 10.13 as "Compliance audit pack" — flagged for the Documentation Steward, not resolved here). Epic-10 story numbers 10.11/10.12/10.14 appear mislabelled in `docs/user-stories/epic-10-adr-0086-to-0094.md`; there is no in-app Trust/Compliance UI to document here, and takedown/DSR backend work is tracked under later stories (e.g. Stories 16.1/16.2).
...
4. **A boolean watchlist now has a real visual query builder (Story 12.4)** instead of a raw text box — pick AND/OR/NOT, add clauses (keyword, hashtag, mention, author, source, sentiment, or date), and group them, with a live query preview. Typing a query directly still works too. **A warning badge appears on any clause one of your selected platforms genuinely can't evaluate** — real-time, right in the form, not on the connector status screen.
...
**Current limitations:**
- Watchlist compatibility warnings (which boolean-query features a given connector can't natively evaluate) are still not shown on *this* screen — as of Story 12.4 they're shown on the watchlist form itself instead; see "Managing watchlists" above.
...
7. **If a post has sentiment for more than one distinct aspect** (Story 12.5/12.6), an "Aspect-Level Sentiment Breakdown" list appears on its AI analysis card. Display-only — unlike the rest of the enrichment, you can't hand-correct per-aspect sentiment.
...
3. **Export CSV** and **Push to CRM** (sending the list's entries to a connected CRM system such as Dynamics 365, Salesforce, or HubSpot — see "Connecting a CRM system" below) are both available from a list's own page, but only to the list's owner — not to anyone it's merely shared with.

## Connecting a CRM system, and pushing items to it (Stories 11.1, 11.2)

Open "CRM Connectors" from Tenant settings to connect a CRM system for your tenant — Microsoft Dynamics 365, Salesforce, or HubSpot. This isn't a Tenant-Admin-only action; any tenant member can connect one.

1. Pick a provider, enter the credentials it needs, and save.
2. Once at least one is connected, a "Push to CRM" button appears on a post's own detail panel and on a prospecting list's own page. It opens a dialog to choose the target CRM, add a note, and confirm.
3. **Pushing is real, not simulated** — it's a genuine API call to your connected CRM (Dataverse for Dynamics 365, the REST API for Salesforce, the v3 API for HubSpot), with a real success or failure result.
...
Open "Digest" from Tenant settings to set up a personal daily email summary — on/off, send time and timezone, what's included (AI summary, top posts, topic breakdown), and an optional watchlist scope. A "Preview" button builds a real digest from your tenant's actual current data right now.

**Current limitation, real and significant: no digest email is ever actually sent.** Your preferences save correctly and Preview genuinely works, but the scheduled send itself isn't wired into any running job, and sending isn't implemented even when that job is run directly. Turning this "on" doesn't currently put anything in your inbox.

## Topic evolution timeline (Stories 11.5, 11.6)

A real "Topic Evolution" screen exists at `/tenant/analytics/topics` — mention volume, sentiment mix, top authors, and top keywords for a topic over time, with day/week/month granularity, a "Compare Previous" toggle, and rising/falling trend badges from real data.

**Current limitation:** nothing in the app links to this screen — not the Analytics dashboard, not the sidebar. You'd need its direct URL to reach it.

## Scheduling and publishing outbound posts (Stories 11.7, 11.8)

A separate "Outbound Publishing & Scheduling" screen exists at `/tenant/posts/outbound` for posts with a publish date attached, distinct from the Compose screen described above.

1. "Compose Post" lets you choose LinkedIn, Facebook, Bluesky, or X (Twitter) as the target, write your text, and either publish now or schedule it. As you type, it also suggests real authors from your tenant's own data to @-mention (Stories 11.11/11.12) — click one to insert it. Tabs filter by All Activities, Scheduled, Published, Failed, or Cancelled, and a scheduled post can be cancelled or rescheduled before it fires.
2. **Publishing for real only works for LinkedIn and Facebook.** Bluesky and X (Twitter) are offered as options in this composer, but neither has a real connector behind it yet — choosing one leaves the post stuck, never actually publishing or reporting failure. Don't rely on Bluesky or X here today.

## Webhook subscriptions (Story 10.12)

A real "Webhook Subscriptions" screen exists at `/tenant/settings/webhooks` — register a URL you control and choose which events it should receive; each endpoint gets a real HMAC-SHA256-signed secret, and a "Test Ping" button sends a real signed test request right now.

**Current limitation:** nothing in the app links to this screen — you have to navigate to it directly by its URL.

## Unified Social Inbox (Stories 11.9, 11.10)

Open "Inbox" for a triage-focused view of your tenant's mentions, separate from the ordinary Posts feed.

1. Filter by All Items, 🔥 Urgent, ⚠️ High Priority, 💤 Snoozed, or ✓ Resolved. Selecting an item lets you add internal notes, write and send a reply, snooze it for a chosen number of hours, or mark it Resolved.
2. **Current limitation, real and important:** unlike the post detail panel's own Reply button (which disables itself for a platform it can't really send to), the Inbox's reply box is always enabled and always reports success. Sending is genuinely real only for Facebook and LinkedIn items — for any other platform, nothing is actually delivered, even though it's recorded as sent. Only trust a reply sent from the Inbox as real for Facebook or LinkedIn.
3. Assigning an item to a specific teammate isn't something you can do from this screen yet, despite the empty-state text mentioning it — there's no working control for it.
...
- **Watchlist compatibility warnings on the connector status screen itself** — as of Story 12.4 they're shown on the watchlist form instead.
...
- **Daily digest emails are never actually sent** (Stories 11.3/11.4) — see above; preferences and Preview both genuinely work, the send itself doesn't.
- **No in-app link to the Topic Evolution timeline** (Stories 11.5/11.6) — real screen, direct-URL-only today.
- **No in-app link to the Webhook Subscriptions screen** (Story 10.12) — real and working, direct-URL-only.
- **Bluesky and X (Twitter) publishing from the Outbound Publishing & Scheduling screen** (Stories 11.7/11.8) — offered but don't work; see above.
- **Assigning an Inbox item to a teammate** (Stories 11.9/11.10) — no working control for it yet.
- **A reply sent from the Inbox for anything but Facebook or LinkedIn is not actually delivered**, despite always being reported as sent — see "Unified Social Inbox" above.
