...
**Learning & Development Writer addendum, 2026-09-12 (scheduled queue pass, continuing oldest-first from `4eac552` through the Epic 12 batch ending at `bbfbb04`):** several genuinely new, previously-undocumented capabilities confirmed and added this pass — escalating a post or a prospecting-list lead to an external CRM (Dynamics 365, Salesforce, or HubSpot) via a real send-with-duplicate-detection dialog (Stories 11.1/11.2); a personal daily digest email with its own schedule, content, and watchlist-scope preferences and a real preview (Story 11.4); a topic evolution timeline with volume/sentiment/trend charts, reachable only by direct URL today (Story 11.6); a second, separately-reachable outbound-post composer with real scheduling and an @mention-suggestion assistant (Stories 11.7/11.8/11.12); a Unified Social Inbox triage screen (Stories 11.9/11.10); a real visual boolean-query builder for watchlists (Story 12.4); and a "Mixed" sentiment option, override-reason field, confidence-tier badge, and aspect-level sentiment breakdown on a post's AI analysis card (Story 12.6). Two real, serious gaps were found and documented plainly rather than glossed over: the new outbound-post composer silently leaves a post stuck (neither success nor failure) if you pick Bluesky or X/Twitter, since neither has a real connector behind it yet; and the Unified Social Inbox's Reply button always reports success but only actually sends for Facebook — for every other platform it silently fakes a "sent" result. **A separate, systemic finding, not something this manual itself needed correcting for:** while auditing this pass's own commit range, five consecutive "frontend" Epic 12 stories (12.8 topic curation, 12.10 dashboard widget renderer, 12.12 webhook management, 12.14 workspace settings, 12.16 influencer discovery) each shipped real, contract-tested React components that are **never actually imported into any route** in `social-listening-admin` — confirmed by direct source search — so none of the five is reachable by a real user today, and correctly none is documented here. The underlying queue file (`docs/pending-learning-development-reviews.md`) was also found to contain multiple fabricated commit hashes in this same date range that don't exist in the repository at all (duplicate-looking entries for the same story, only one of which was ever real) — flagged there and for the Documentation Steward, not something this manual's own text needed to change for.
