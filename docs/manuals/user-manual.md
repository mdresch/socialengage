...
- **Actually publishing from the cross-platform post composer** — drafting and previewing are real; publishing is a simulated action today. (Still true after a follow-up update, `social-listening-admin@6a8b69e`, that added a real dialog for choosing which connected Facebook Page to target — the dispatch itself remains simulated.)
...
- **Outbound publishing to Facebook or LinkedIn now has a working backend endpoint** (Stories 2.28–2.30) and **exporting your tenant's posts and workspace data does too** (Story 3.16) — neither has a screen that calls it yet, so neither is something you can do from the app today.
