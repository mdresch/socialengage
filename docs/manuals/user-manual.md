...
**Learning & Development Writer addendum, 2026-09-11 (scheduled queue pass, continuing oldest-first from `f7a93b1`):** confirmed and documented five more genuinely new, contract-verified Epic 11 capabilities, each traced through real `social-listening-admin`/`social-listening-core` source rather than story Acceptance-Criteria prose: escalating a post or a Prospecting-list author to a real CRM (Stories 11.1/11.2 — real Dynamics 365/Salesforce/HubSpot pushes, but with no in-app screen yet to enter your tenant's own CRM credentials), your own daily digest email preferences (Stories 11.3/11.4 — preferences and an on-demand preview are both real, but the scheduled send itself is not wired to run automatically and, even run by hand, never actually dispatches an email), a Topic Evolution timeline (Stories 11.5/11.6, limited to a fixed preset list of five topics), a separate Outbound Publishing & Scheduling screen for LinkedIn/Facebook with real immediate-or-scheduled dispatch and AI mention suggestions (Stories 11.7/11.8/11.12), and a unified social-care Inbox for triaging and replying to mentions (Stories 11.9/11.10) — this last one's own "assign" action has a real backend endpoint but no actual control anywhere in its screen to trigger it, named below as a gap rather than described as something you can do. Docs-only and ADR/BRD/FDD-sync commits in this range and two contract-healing commits with no behavior change were confirmed to ship nothing further user-facing.
...
Open "Daily Digest Email" from your tenant's settings (`/tenant/settings/digest`) to control your own personal digest subscription — this is a per-user setting; your own choices here don't affect any other user in your tenant.

1. Toggle the "Daily Digest Subscription" on or off, choose a local delivery time and your timezone from a preset list, and optionally narrow it to specific watchlists (leave none selected to include all your active watchlists).
2. Three content toggles control what's included: an AI-generated executive summary, a "Notable High-Impact Conversations" top-5 list, and a trending-topics/key-themes breakdown.
3. A "Preview" action generates a real, on-demand rendering of what your digest would currently contain, built from your own real posts.

**Current limitation, real and not yet resolved:** saving preferences and previewing both genuinely work, but the scheduled email itself is not delivered anywhere yet — the backend job that would build and send it on your chosen schedule isn't wired into anything that runs it automatically, and even run by hand, it stops short of actually dispatching an email; it only builds the digest, logs it, and marks it sent. "Preview" is the only way today to actually see your digest's content.
...
1. On a post's detail panel, next to "Reply," there's a "💼 Push to CRM" button — available on every post, regardless of platform or watchlist match. (A Prospecting list's own owner gets an equivalent "Push to CRM" button on that list's page instead — see "Prospecting lists" below.)
2. Clicking it opens a dialog: choose the target CRM (Microsoft Dynamics 365, Salesforce, or HubSpot), a record type (Support Case/Incident, Lead/Prospect, Opportunity/Deal, Account, or Contact), and optionally an assignee and internal notes.
3. Clicking "Escalate to CRM" is a real push to that CRM system's own API, not a simulation — on success you get a direct link to the new CRM record. If that same item has already been escalated to that CRM before, you're shown the existing record and asked to confirm before creating a duplicate.
4. **As of this writing, there is no screen anywhere in SocialEngage to enter your tenant's own Dynamics 365, Salesforce, or HubSpot credentials.** Without them already configured on the backend (your Tenant-Admin's job, if it's ever done at all today), escalating fails with a plain error naming which credential fields are missing.
...
Open "Topic Evolution" from the Analytics section to see a single topic's real mention volume, unique-author count, and sentiment mix charted by day, week, or month over a date range you choose, with an optional overlay comparing it to the prior period. A badge over the chart reads the topic's overall trend as Rising, Falling, or Stable for the range shown, computed from your tenant's own enriched posts.

**Current limitation:** topics are chosen from a fixed preset list of five (Artificial Intelligence, Customer Support, Product Feedback, Pricing, Security) rather than freely typed or drawn from your tenant's own real topics — there's no way yet to chart a topic that isn't on that list.
...
## Outbound publishing & scheduling (Stories 11.7, 11.8, 11.12)

Open "Outbound Publishing & Scheduling" from your tenant's Posts area (`/tenant/posts/outbound`) — a separate screen from "Compose" above, for publishing directly to LinkedIn or Facebook right now or at a scheduled future time, and tracking what happens.

1. Write your message and choose one or more target platforms. **Today, only LinkedIn and Facebook can actually be selected** — Bluesky and X/Twitter are shown but stay disabled, since no real connector exists yet to publish to either. For LinkedIn or Facebook, pick which of your own connected accounts or Pages to post to.
2. As you type, an AI mention-suggestions panel (Story 11.12) suggests real accounts to @-mention, drawn from your tenant's own posts, for whichever platforms you've selected.
3. Choose "Publish Now" or "Schedule for Later" (a future date and time). A tabbed list (All / Scheduled / Published / Failed / Cancelled) shows every outbound post queued this way; a scheduled post can be cancelled or rescheduled to a new future time right up until it actually goes out.
4. Publishing to LinkedIn or Facebook from this screen is real, not simulated — the same real connectors used elsewhere in SocialEngage (Stories 2.29, 2.30).

## Inbox — triaging and replying to mentions (Stories 11.9, 11.10)

Open "Inbox" from your tenant's screens to work through incoming posts one at a time instead of browsing the full Posts feed.

1. Filter the queue by tab — All, 🔥 Urgent, ⚠️ High Priority, 💤 Snoozed, or ✓ Resolved — or search within the current tab; each item shows its own priority badge.
2. Selecting an item shows its full content alongside triage actions: reply to it, snooze it for a chosen number of hours, or mark it Resolved.
3. Replying from here uses the same real, live send described in "Replying to a post" above — the reply box itself doesn't check which platform the item came from before showing itself, so it only actually succeeds for a platform whose connector supports replying (Facebook today); for any other platform, sending fails with a real error from that connector rather than silently doing nothing.

**Current limitation:** assigning an inbox item to a specific person has a real backend endpoint, but there is no control anywhere on this screen to actually use it.
...
- **A screen to configure your tenant's own CRM credentials (Dynamics 365, Salesforce, or HubSpot)** — "Push to CRM" (Stories 11.1/11.2) is real and can genuinely escalate a post or author once credentials are configured, but nothing in `social-listening-admin` lets anyone enter those credentials yet; escalating without them fails with a plain "not configured" error.
- **Daily digest emails are not actually delivered** (Stories 11.3/11.4) — your preferences save for real and "Preview" genuinely renders your current digest content, but the scheduled send has no real email dispatch behind it and isn't wired to run automatically either.
- **Topic Evolution is limited to a fixed preset list of five topics** (Stories 11.5/11.6) — there's no way yet to chart a topic outside that list.
- **Publishing to Bluesky or X/Twitter from the Outbound Publishing & Scheduling screen** (Stories 11.7/11.8) — both are shown but disabled, since neither has a real publishing connector yet; only LinkedIn and Facebook actually work there.
- **Assigning an inbox item to a specific person** (Story 11.9) has a real backend endpoint, but the Inbox screen itself has no control anywhere to trigger it.
