...
| C-17 | Azure API Management (Developer tier) | Cloud Infrastructure | Free, non-production, no SLA | $0 | Active — ADR-0144 (2026-09-17), gateway of record for the frontend-platform-evaluation phase; must be upgraded to Consumption or Standard (~€500–€2,500/month, per ADR-0144's own Consequences) before any frontend candidate goes to production — tracked there as an open item, not yet scheduled here |
| C-18 | GitHub (private repo: `social-listening-core`) | Development Tools | GitHub private-repo/Actions-minutes billing (differs from C-07/C-16's public-repo assumption) | TBD — not yet quantified | Active — Story 20.1/ADR-0144 (2026-09-17), `https://github.com/mdresch/social-listening-core`, own CI wired (build/typecheck/test/`openapi:generate`) |

**Documentation Steward correction, 2026-09-23.** C-17 and C-18 were both missing from the register above despite being real, already-decided/shipped facts as of Story 20.1/ADR-0144 (2026-09-17): ADR-0144 explicitly names Azure API Management's Developer-tier-now/paid-tier-later cost trajectory in its own Consequences and Open Items, and `social-listening-core` is now a real *private* GitHub repository with its own GitHub Actions CI — a materially different cost basis than C-07/C-16's existing "public repo, free" rows, which this register never revisited. Added both, following the existing C-14/C-15 convention for a "Planned"/"Active, cost not yet quantified" future item sourced from an ADR; neither dollar figure is invented here — C-17's paid-tier cost range is ADR-0144's own stated estimate, and C-18's actual GitHub billing impact (if any, given GitHub's private-repo free-tier Actions-minutes allowance) is left `TBD` rather than guessed at.
