...
**Documentation Steward correction, 2026-09-23.** "Two deployable repos" in the row above was true in the sense this plan meant at the time — two independently built/tested logical components — but not literally two separate git repositories: both lived (and `social-listening-admin` still lives) as subdirectories of this single combined `socialengage` workspace repo, per `CLAUDE.md`'s own "Facts worth not re-deriving" section prior to 2026-09-17. That changed only partially, and much later than this row's 2026-07-29 completion date: Story 20.1/ADR-0144 (2026-09-17) clean-cut extracted `social-listening-core` into its own real, standalone, independently-CI'd private GitHub repository (`https://github.com/mdresch/social-listening-core`); `social-listening-admin` has not been split out in any form. Not editing the row's own historical ✅/date (a real content/status call already made at the time, per this plan's own convention elsewhere), only adding this factual clarification of what "two deployable repos" did and didn't mean, then and now.
