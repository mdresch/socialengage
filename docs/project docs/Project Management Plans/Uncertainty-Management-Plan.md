...
| R-04 | ~~No authentication mechanism exists; `X-Tenant-Id` is an unauthenticated placeholder~~ — **corrected 2026-08-06: resolved.** Real bearer-token (Entra External ID) authentication is shipped (Stories 5.9–5.11), and `X-Tenant-Id` is fully retired as a trust mechanism (Story 5.10/ADR-0033) — see `docs/implementation-log.md`. The related two-tenant isolation proof this risk also named is likewise no longer blocked on this gap. | Security | High (certain, if triggered) | High | ~~Open — ADR-0029–0033 accepted, implementation in progress (Phase 4.5)~~ — **Resolved, corrected 2026-08-06:** Phase 4.5 shipped in full (`docs/implementation-log.md`); trigger condition closed |
| R-05 | ~~Accepted tenant/admin/user model (ADR-0030–0032) is architecturally decided but not yet implemented~~ — **corrected 2026-08-06: implemented.** Migrations, real `tenants`/`users` tables, identity resolution, and role checks are all shipped and contract-verified (Stories 5.9–5.10, 6.2) — see `docs/implementation-log.md`. | Architecture | Medium | Medium | ~~Open — ADR-0030–0032 accepted~~ — **Resolved, corrected 2026-08-06:** implementation (not just ADR acceptance) is complete |
...
- **R-04:** ~~Architecture is resolved by ADR-0029–0033; implement bearer-token middleware and Entra integration before exposing tenant operations to any external caller, and before claiming a live two-tenant isolation proof~~ — **corrected 2026-08-06:** done — bearer-token middleware and Entra integration are shipped (Stories 5.9–5.11); tenant operations already require it
- **R-05:** ~~Architecture is resolved by ADR-0030–0032; implement migrations, identity resolution, role checks, and tenant/user contract tests~~ — **corrected 2026-08-06:** done — all four are shipped and contract-verified (Stories 5.9–5.10, 6.2)
