...
4. **Track Time Spent (~~Planned, not yet installed~~ — see 2026-08-06 correction below: a real, different mechanism now ships):**
   ...
   - **Documentation Steward correction, 2026-08-06:** the substantive claim above (no real time-tracking mechanism exists — `docs/time-tracking.md` still reads "Total Time: 0 minutes") remains accurate, but the specific mechanism/path named below is now stale and, read literally, misleading. `scripts/git-hooks/pre-commit` and `.git/hooks/pre-commit` both now exist and are installed (`node scripts/setup-git-hooks.js` has been run), but that real, shipped hook is a **contract-first backstop** (blocks a commit touching `<repo>/src/` with no matching `contracts/*.contract.test.ts` staged, per `docs/implementation-methodology.md`) — an entirely different mechanism from the time-tracking prompt described below, which does not exist under this or any other name. A future time-tracking hook, if built, would need its own separate script/hook file, not this one.
   - **Documentation Steward correction, 2026-08-06 (later the same day — the "future time-tracking hook" the note above said was still undesigned has since shipped, commits `cd308eb`/`406bf2c`):** a real, auto-derived time-tracking mechanism now exists, and the "log table has zero rows" claim above is now stale too — `docs/time-tracking.md`'s log table currently carries real rows. It is **not** the prompt-based mechanism the bullets below describe, and was deliberately built differently rather than completing that original design: `scripts/git-hooks/post-commit` (the same hook that already queues the Ideal Manager/Documentation Steward/Learning & Development Writer reviews) now also appends one row per commit automatically, with no prompt — inferring an Activity category and a Story/ADR reference from the commit message alone. Start/End Time and Duration are honestly recorded as `—`/`auto`, never fabricated, because this mechanism has no way to know real wall-clock time (`docs/time-tracking.md`'s own "Approach" note). Concretely, against the five bullets immediately below: prompting for date/start/end/duration/activity/story/ADR/notes was **not built** (a blocking prompt would hang on Claude Code's own no-TTY commits, which are most of this project's commits — `docs/time-tracking.md`'s own account of why); an entry **is** automatically added per commit; duration is **not** auto-calculated (no start/end times exist to calculate from); bypassability is **moot**, not achieved — the hook runs post-commit, after the commit has already succeeded, so there is structurally nothing to bypass (`docs/time-tracking.md`'s own "Bypass: not applicable" note); and estimation-accuracy metrics are **still not enabled** — `docs/time-tracking.md`'s own "Metrics Derived from Time Tracking" section marks every duration-dependent metric "Not tracked," not "TBD," since this mechanism structurally cannot collect real duration data. The `docs/implementation-log.md` proxy named in the last bullet remains the accurate near-term substitute.
   - **Designed mechanism (not yet built/installed):** ~~`scripts/git-hooks/pre-commit`~~ — that path is now taken by the real contract-first hook (see correction above); a time-tracking-specific hook is still entirely undesigned as to its own file/location
   - **Installation (not yet run):** ~~`node scripts/setup-git-hooks.js` would activate it, if/when built~~ — **corrected 2026-08-06:** this script now exists and has been run (it installs the real contract-first `pre-commit` hook and a `post-commit` hook that queues commits for the Ideal Manager/Documentation Steward/Learning & Development Writer review queues — neither does time tracking)
