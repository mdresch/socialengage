...
| Azure AI Language | Usage-based | Billed per text record processed; not yet incurred (enrichment not wired into ingestion) |
...
| At least one connector ingesting real data end-to-end | Newswire connector (ADR-0024) | Real GlobeNewswire/PR Newswire posts ingested, not fixtures | Story 2.6 contract test, proven against live feeds | **Met** — shipped |
...
**2026-07-31 note, added by the AI Business & Requirements Analyst persona — a real tension in the row above, named rather than smoothed over.** The "At least one connector ingesting real data end-to-end" row is marked **Met** against the Newswire connector (ADR-0024/Story 2.6) — accurately; that story did ship and is contract-verified against live feeds. But `docs/implementation-plan.md`'s own Phase 1 goal is narrower and still open: *"the smallest real slice — one platform, one tenant, one watchlist, posts flowing from that platform into a queryable API... This is the phase that proves the architecture, not just the first platform."* Newswire is architecturally a Phase-4, third-or-later connector that shipped ahead of schedule as a deliberate, ADR-sanctioned deviation (`docs/implementation-plan.md` Phase 4) — it satisfies this row's literal wording without closing Phase 1's own MVP gap, which stayed unbuilt because RSS/News was chosen as a *category* (2026-07-29) and never given a concrete provider. That gap is now drafted as **Story 2.7 / ADR-0026** (GNews API, publication-as-Author modeling) — ADR-0026 was accepted by Menno on 2026-07-31, the same day it was drafted, so Story 2.7 is Ready, but **it is not yet built**: accepting the ADR cleared the blocking gate, it didn't write the connector code. This row should not be read as evidence that Phase 1 itself is done; it is evidence that this specific success criterion's *wording* was satisfied by a different, later-phase connector than the one that was supposed to satisfy it first — and won't be genuinely closed until Story 2.7 itself is built.

---

# 9. Strategic Recommendation & Implementation Roadmap

**Recommendation: proceed**, reaffirming Business Case §4's original conclusion under this document's more rigorous weighted-decision and traceability instruments (Sections 3 and 8) rather than superseding it. Three conditions gate further investment, in this order:
...
3. **Execute Milestone M7 (Phase 1 review)** only once all four Business Case §8 success criteria are independently re-verified — two of five Section 8 traceability rows are not yet met (AI enrichment, event publishing), and the isolation criterion needs a live two-tenant proof, not just a passing contract, before this business case's own recommendation to defer downstream subsystems (Brand Reputation, Social Care, Social Selling) can be revisited.
