...
| Field | Value |
|---|---|
| Document Title | SocialEngage Social Listening / Insights — Author/Topic Signal Minimal v1 |
| Version | 1.0 |
| Date | 2026-08-22 |
| Author(s) | Menno, Product Owner / Requirements Analyst |
...
|---|---|---|---|
| 0.1 | 2026-08-22 | Menno | Initial draft from ADR-0007, Story 4.1, Story 4.4, and related feature designs |
...
- The data model must remain additive; adding a computed score later must not break existing consumers.
- All derived data must remain tenant-scoped and protected by Postgres RLS.
- Refresh must avoid live recomputation against unbounded post tables for latency reasons.
...
The platform ingests and stores individual `SocialPost` records, each linked to a normalized `Author` row (ADR-0004). However, there is no pre-aggregated, author/topic-level artifact. Any consumer that wants to know "who writes most about topic X" or "which authors are most consistently active on topic Y" must aggregate posts itself, recompute counts and engagement, and resolve its own ranking.

**Pain points:**
- Consumers repeat the same expensive aggregation logic.
- There is no shared, tenant-isolated source of truth for author/topic activity.
- Latency would be unpredictable if every request recomputed `activeMonthsCount` or `sentimentBreakdown` over the full post table.

---

## 7. Future State (To-Be)

After implementation, a scheduled job refreshes a tenant-scoped materialized view (`AuthorTopicSignal`) that contains pre-computed raw signals. The API exposes `GET /topics/:topic/authors?sortBy=activeMonths|mentionCount`, returning the raw fields directly. Consumers choose which dimension to sort by, and future subsystems can combine the same fields with their own weights.

**Expected capabilities:**
- Fast lookup of authors for a given topic.
- Choice of two sorting dimensions: sustained activity (`activeMonths`) or total presence (`mentionCount`).
- Extensible raw-signal schema that can later receive additive fields without breaking existing callers.
- Consistent tenant isolation through RLS and the existing `Author` normalization pattern.
...
|---|---|---|---|
| Author count per topic | Track coverage of the expert-finder capability | Product team / Platform Operator | Per refresh |
| Refresh latency / success | Ensure the materialized view stays current | Platform Operator | Per refresh |
| `GET /topics/:topic/authors` request latency | Monitor API performance | Platform Operator / Engineering | Continuous |
| Sort-by usage (`activeMonths` vs `mentionCount`) | Understand consumer preference | Product team | Weekly (from logs) |
...
- `AuthorTopicSignal` contains no `expertiseScore` or equivalent computed field — only raw counts, dates, averages, and breakdowns.
- `GET /topics/:topic/authors` accepts `sortBy=activeMonths|mentionCount` and sorts accordingly.
- The view is refreshed on a schedule, not recomputed live on every request (see Story 4.4 / ADR-0022).
...
| Term | Definition |
|---|---|
| `AuthorTopicSignal` | A tenant-scoped, periodically refreshed materialized view of raw author/topic activity signals. |
| Raw signal | A count, date, average, or breakdown computed directly from post data, without a composite or weighted score. |
| `activeMonthsCount` | The number of distinct calendar months in which an author posted about a topic. |
| `avgEngagement` | The average engagement value across an author's posts on a given topic. |
| `sentimentBreakdown` | The distribution of post sentiment (positive, neutral, negative) for an author/topic. |
| Expert finder | The product capability that lets a consumer identify authors knowledgeable about a topic. |
...
- `docs/adr/0007-author-topic-signal-minimal-v1.md` — Accepted 2026-07-28.

### Appendix B — Related ADRs
- `docs/adr/0004-author-normalized-separately-from-post.md` — `Author` model.
- `docs/adr/0022-derived-data-caching-and-refresh-strategy.md` — Refresh cadence and caching.

### Appendix C — Related User Stories
- `docs/user-stories/epic-4-derived-data-analytics-and-health.md`:
  - **Story 4.1** — Raw author-topic signals for expert-finding.
  - **Story 4.4** — Derived-data caching and refresh strategy (hourly `AuthorTopicSignal` refresh).

### Appendix D — Related Feature Designs
- `docs/product-research/feature-designs/05-influencer-discovery.md` — describes how a future `influence_score` can be layered on `AuthorTopicSignal` raw signals.
- `docs/product-research/feature-designs/04-ai-topic-clustering.md` — references the `AuthorTopicSignal` refresh pattern and topic-level exploration.
- `docs/product-research/feature-designs.md` — high-level feature catalog.

### Appendix E — Missing / Not Resolvable Sources
- The Design Spec §4.5 "AuthorTopicSignal (materialized view, periodic refresh)" cited in ADR-0007 was not found as a separate file in the repository. This BRD was drafted from the ADR itself and the related user stories and feature designs.
- No dedicated `docs/product-research/reports/<feature>-deep-research.md` file for `AuthorTopicSignal` or expert-finding was found.
