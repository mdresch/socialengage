...
| Field | Value |
|---|---|
| Document Title | Social Listening / Insights – Instagram Business Connector Business Requirements Document |
| Version | 1.0 |
| Date | 2026-08-20 |
...
|---|---|---|---|
| 1.0 | 2026-08-20 | BRD Writer Agent | Initial BRD derived from ADR-0068, Story 2.24, and Story 6.34 |
...
- Meta Platform Terms §3.b and GDPR privacy obligations strictly prohibit personal account scraping.
- Instagram Graph API returns media in strict newest-first order; no alternative ordering is supported.
- Meta CDN media URLs are signed and may expire; the UI must not treat them as permanent assets.
- Country is the only reliably normalized geospatial signal because most posts lack explicit location tags.
...
Social Listening / Insights already ingests content from GNews, Newswire, tenant-owned RSS feeds, Wikipedia, and Facebook Pages. Instagram, however, is not yet a supported provider. Teams monitoring visual brand presence must rely on manual review or external tools, creating a gap between owned Facebook Page content and owned Instagram Business content.

**Pain points:**
- No compliant, API-based ingestion for Instagram Business and Creator accounts.
- Visual brand campaigns and creator partnerships are invisible in the unified post feed.
- Risk of ad-hoc or non-compliant scraping attempts to fill the visibility gap.
- No structured error handling or reconnection flow for expired Instagram OAuth tokens.

---

## 7. Future State (To-Be)

After implementation, a Tenant Administrator or authorized user authenticates with Meta, selects one or more Instagram Business/Creator accounts from a multi-account picker, and the connector begins scheduled ingestion. The system polls each selected account, creates one `SocialPostSummary` row per media item, and presents the content in the post feed with the hosting `@username`, engagement counters, and carousel galleries.

**Expected capabilities:**
- OAuth-driven account discovery and selection without manual ID entry.
- Continuous, bounded background ingestion of Instagram photos, videos, Reels, and carousels.
- Single-row carousel modeling with ordered child media and truncation handling.
- `reconnect_required` alerts triggered by expired tokens, revoked permissions, or unlinked accounts.
- Admin UI status, health, re-sync, and post-detail rendering for Instagram Business content.
...
|---|---|---|---|
| Instagram connector health status | Track `healthy`, `reconnect_required`, and `stalled` states | Tenant Admin / Analyst | Real-time on status screen |
| Connected Instagram account count | Show how many accounts are actively ingesting | Tenant Admin | On-demand on connectors screen |
| Last successful ingestion timestamp | Verify recency of Instagram content | Tenant Admin / Analyst | On status screen |
| Ingested Instagram post volume | Measure visual channel coverage | Product Team | Per polling cycle / dashboard |
| Engagement counts (likes, comments) | Surface audience reaction to visual posts | Analyst | At ingestion snapshot |
| Rate-limit / error events | Monitor API quota and credential health | Technical Lead | Per polling cycle |
...
- ADR-0068 is accepted and all decisions are reflected in the BRD.
- Story 2.24 contract tests pass: standard photo, video/Reel, and carousel media mapping; carousel single-row creation with ordered `rawPayload.children` (and truncation flag if >10); 30-day / 100-item pagination precedence; and Graph API errors `190`/`10`/`100` reclassified to `reconnect_required` with alert emission.
- Story 6.34 contract tests pass: platform definition registers `instagram` with Tier-3 scope and correct metadata; post card renders hosting handle `@username` and media preview; post detail panel renders carousel gallery from `rawPayload.children`; connector status screen displays operational metrics and `reconnect_required` badge.
- No personal Instagram account or unauthorized scraping path is exposed in the connector or UI.
- All Instagram OAuth credentials are stored encrypted with `owner_type = 'user'`.
- `reconnect_required` alert is raised within one polling tick of encountering Graph API errors `190`, `10`, or `100`.
...
| Term | Definition |
|---|---|
| **Instagram Business Account** | An Instagram Professional account categorized as a business, linked to a Facebook Page. |
| **Instagram Creator Account** | An Instagram Professional account categorized as a creator, linked to a Facebook Page. |
| **Instagram Graph API** | Meta's official API for accessing Instagram Business and Creator account content. |
| **CAROUSEL_ALBUM** | An Instagram post containing multiple media items displayed as a swipeable gallery. |
| **Reel** | A short-form Instagram video; ingested as `media_type: VIDEO` through the Graph API. |
| **Tier-3 Credential** | A user-delegated, tenant-scoped credential stored with `owner_type = 'user'`. |
| **Long-lived Page Access Token** | A durable OAuth token used for background polling of Facebook/Instagram resources. |
| ** reconnect_required** | A connector health status indicating the user must re-authorize the connection. |
| **SocialPostSummary** | The canonical single-row model for an ingested social post in the system. |
| **rawPayload** | A JSON field storing provider-specific metadata for an ingested post. |
...
- [ADR-0068: Instagram Connector (`instagram`) — Platform Connector Scope, Business/Creator Account Model, Hosting Profile Dependency, and Tier-3 OAuth Harmonization](../../../docs/adr/0068-instagram-connector.md)
- [docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md](../../../docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md) — Story 2.24
- [docs/user-stories/epic-6-tenant-admin-ui.md](../../../docs/user-stories/epic-6-tenant-admin-ui.md) — Story 6.34
- [docs/user-stories/README.md](../../../docs/user-stories/README.md) — Epic index

### Missing Source Notice

No dedicated `docs/product-research/feature-designs/` or `docs/product-research/reports/` file was found for the Instagram connector. The BRD is derived directly from ADR-0068 and the related user stories (Story 2.24 and Story 6.34).
