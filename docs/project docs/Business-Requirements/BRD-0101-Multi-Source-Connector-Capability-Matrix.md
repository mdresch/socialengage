...
| Field | Value |
|---|---|
| Document Title | Multi-Source Connector Capability Matrix – Business Requirements Document |
| Version | 0.1 |
| Date | 2026-08-23 |
...
> **Note:** ADR-0101 is currently **Proposed**. This BRD is therefore a draft for review and may change if the ADR is revised or rejected.

### Revision History

| Version | Date | Author | Description of Changes |
|---|---|---|---|
| 0.1 | 2026-08-23 | BRD Writer Agent | Initial draft from ADR-0101, feature design `01-multi-source-ingestion`, and Epic 12 stories 12.1–12.2. |
...
This BRD defines a **Multi-Source Connector Capability Matrix**. Each connector will declare its capabilities (poll, count, publish, reply, backfill), a registry will expose those declarations, and the product will surface a `GET /v1/connectors/capabilities` endpoint so the admin UI and composer can render only the actions that are valid for the current tenant and platform. Connector health will also reflect whether a connector is healthy but capability-limited.

Once implemented, tenant administrators and end users will see only the platforms and actions they can actually use, the product can ship new connector capabilities without front-end rewrites, and the architecture will be ready for the deeper `count`, `publish`, `reply`, and `backfill` features described in ADR-0077, ADR-0073, ADR-0098, and the multi-source ingestion feature design.
...
- Defining a capability matrix contract for `SocialConnector` implementations (poll, count, publish, reply, backfill).
...
- Full implementation of the `count`, `publish`, `reply`, and `backfill` features themselves (this BRD only authorizes the discovery layer).
...
- Must remain aligned with the existing `SocialConnector` framework and `registry.ts` in `social-listening-core`.
- Must follow the contract-first methodology used by the project.
- Must not break existing connector health or activation flows.
...
The platform already supports multiple source types: RSS/news (GNews, Newswire), tenant-owned feeds, Wikipedia, and OAuth-based Facebook, with more planned. Each connector is built on a common `SocialConnector` framework with `poll()` loops, health reporting, and activation state.

However, the connector capabilities are not centrally discoverable. The UI does not have a standard mechanism to know whether a platform supports `count`, `publish`, `reply`, or `backfill`. This leads to the following pain points:

- Users may see composer or action options that are not supported by the selected platform.
- Front-end releases are required to hide or enable new actions as connectors evolve.
- Tenant admins must rely on documentation or trial and error to understand what a connector can do.
- The health endpoint cannot distinguish between a connector that is fully healthy and one that is healthy but limited by its API tier.

---

## 7. Future State (To-Be)

After this initiative, every connector will expose its capabilities through a standard `getCapabilities()` contract. The connector registry will answer capability queries, and the product will expose a `GET /v1/connectors/capabilities` endpoint. The admin UI and composer will consume this endpoint to display only the actions a connector supports.

The new or improved process is:

1. A connector implementation declares its capabilities in code, including tier- or version-specific limits.
2. The connector registry serves these declarations through a typed interface.
3. `GET /v1/connectors/capabilities` returns a list of all registered connectors with their `platformId`, `name`, `authMode`, and `capabilities`.
4. `GET /v1/connectors/:platformId/health` combines `ConnectorHealth` with `getCapabilities()` so the UI can show whether a connector is healthy but capability-limited.
5. The composer and connector list in `social-listening-admin` read the capabilities and disable or hide unsupported actions, showing tooltips where useful.
6. Backfill remains an explicit, admin-triggered action bounded by `maxLookbackDays`.

Expected capabilities:

- The UI reflects real connector capabilities without hardcoding.
- New capabilities can be added to a connector without a front-end release.
- `count`, `publish`, `reply`, and `backfill` features have a clear gating mechanism.
- Connector health reporting is more informative and accurate.
...
|---|---|---|---|
| Connector capability coverage | Track how many registered connectors declare each capability | Product / Engineering | Per release |
| Capability-limited health instances | Detect healthy connectors that cannot offer a planned feature | Operations / Product | Weekly |
| Backfill requests by connector | Monitor historical-ingestion usage and quota impact | Product / Customer Success | Weekly |
| Capabilities endpoint latency | Ensure UI read performance is acceptable | Engineering | Continuous |
...
- `SocialConnector` returns `poll`, `count`, `publish`, `reply`, and `backfill` capability declarations.
- Capabilities can differ by connector implementation and tenant API tier.
- `GET /v1/connectors/capabilities` returns all registered connectors with their capabilities.
- `GET /v1/connectors/:platformId/health` reports health combined with capability limits.
- At least three existing connectors return realistic, contract-valid capability objects.
- `ConnectorsView` in the admin UI shows capability badges for `poll`, `publish`, `reply`, and `count`.
- The composer only offers publishable platforms for the current tenant.
- Unsupported platforms and actions are shown as disabled with a tooltip.
...
| Term | Definition |
|---|---|
| **Capability matrix** | The set of feature flags and configuration values that describe what each connector supports. |
| **Connector activation** | The per-tenant or per-user record that determines whether a connector is active and who owns its credentials. |
| **Connector health** | The runtime status of a connector (e.g., healthy, failing, limited) reported by `GET /v1/connectors/:platformId/health`. |
| **Backfill** | A one-time, admin-triggered ingestion of historical posts beyond the normal polling window. |
| **Platform** | A third-party source such as GNews, Facebook, Wikipedia, or Newswire. |
| **`platformId`** | A short, stable identifier for a platform (e.g., `gnews`, `facebook`). |
| **SocialConnector** | The common interface that every source implementation follows in `social-listening-core`. |
| **Tenant API tier** | A service level or contract tier that can affect which features a connector can use. |
...
- `docs/adr/0101-multi-source-connector-capability-matrix.md` — source ADR (Proposed as of 2026-08-23).
- `docs/product-research/feature-designs/01-multi-source-ingestion.md` — parent feature design.
- `docs/product-research/feature-adr-scoping.md` — feature-to-ADR scoping plan.
- `docs/user-stories/epic-12-adr-0101-to-0108.md` — implementation stories 12.1 and 12.2.

### Related ADRs

- ADR-0051: Connector activation
- ADR-0073: Connector `reply?()`
- ADR-0077: Connector `count?()`
- ADR-0098: Connector `publish?()`

### Missing source note

No `docs/product-research/reports/01-multi-source-ingestion-deep-research.md` file was found; if a deep-research brief is created later, it should be linked here.
