...
interface ComputedStats {
  total: number;
  indexScore: number;
  countries: Record<string, { count: number; positive: number; negative: number; name: string }>;
  macroRegions: Record<string, { count: number; name: string }>;
  languages: Record<string, { count: number; name: string }>;
  cities: Record<string, { count: number; name: string }>;
}
...
export const LocationDashboardTab: React.FC<LocationDashboardTabProps> = ({
  ...
  // Dynamic calculations from filteredPosts
  const computedStats = useMemo<ComputedStats>(() => {
    let positive = 0;
    let negative = 0;
    let total = 0;

    const countries: Record<string, { count: number; positive: number; negative: number; name: string }> = {};
    const macroRegions: Record<string, { count: number; name: string }> = {};
    const languages: Record<string, { count: number; name: string }> = {};
    const cities: Record<string, { count: number; name: string }> = {};
    ...
    filteredPosts.forEach((post) => {
      ...
      // Countries
      if (post.location) {
        const { countryId, countryName, city } = post.location;
        if (!countries[countryId]) {
          countries[countryId] = { count: 0, positive: 0, negative: 0, name: countryName };
        }
        countries[countryId].count++;
        if (post.sentiment === 'positive') countries[countryId].positive++;
        if (post.sentiment === 'negative') countries[countryId].negative++;

        // Continents / Groups
        let continent = 'europe';
        if (['us', 'ca'].includes(countryId)) continent = 'north_america';
        else if (['in', 'cn', 'jp'].includes(countryId)) continent = 'asia';
        
        if (!macroRegions[continent]) {
          macroRegions[continent] = { count: 0, name: continent.replace('_', ' ').toUpperCase() };
        }
        macroRegions[continent].count++;

        // Cities
        if (city) {
          if (!cities[city]) {
            cities[city] = { count: 0, name: city };
          }
          cities[city].count++;
        }
      }
      ...
      // Languages
      const langCode = post.languageCode;
      const langName = post.languageName;
      if (!languages[langCode]) {
        languages[langCode] = { count: 0, name: langName };
      }
      languages[langCode].count++;
    });

    const indexScore = total > 0 ? Number(((positive - negative) / total * 10).toFixed(1)) : 10.0;
  ...
  const sentimentByRegion = useMemo(() => {
    const countriesObj = computedStats.countries as Record<string, { count: number; positive: number; negative: number; name: string }>;
    const list = Object.entries(countriesObj).map(([id, data]) => {
      const scoreVal = data.count > 0 ? 5 + ((data.positive - data.negative) / data.count) * 5 : 10;
      const scoreStr = `${scoreVal >= 5 ? '+' : ''}${Math.round(scoreVal)}`;
      return {
        id,
        name: data.name,
        fullName: data.name,
        score: scoreStr,
        widthPercent: Math.round(scoreVal * 10),
        trend: scoreVal >= 7.5 ? 'up' : scoreVal < 4 ? 'down' : 'flat',
      };
    ...
    if (list.length === 0) {
      return [
        { id: 'uk', name: 'United Kingdom', fullName: 'United Kingdom', score: '+10', widthPercent: 100, trend: 'up' },
  ...
  const locationGroups = useMemo(() => {
    const macroRegionsObj = computedStats.macroRegions as Record<string, { count: number; name: string }>;
    const list = Object.entries(macroRegionsObj).map(([id, data]) => {
      const maxVal = Math.max(1, ...Object.values(macroRegionsObj).map(m => m.count));
      return {
        id,
        name: data.name,
        count: data.count,
        widthPercent: Math.round((data.count / maxVal) * 80) || 5,
        trend: 'flat' as const,
      };
    });
    ...
    if (list.length === 0) {
      return [
        { id: 'europe', name: 'EUROPE', count: 63, widthPercent: 80, trend: 'flat' as const },
        { id: 'asia', name: 'ASIA', count: 4, widthPercent: 6, trend: 'flat' as const },
        { id: 'north_america', name: 'NORTH AMERICA', count: 4, widthPercent: 6, trend: 'flat' as const },
      ];
    }
    return list;
  }, [computedStats]);
  ...
  const countryLocations = useMemo(() => {
    const countriesObj = computedStats.countries as Record<string, { count: number; positive: number; negative: number; name: string }>;
    const list = Object.entries(countriesObj).map(([id, data]) => {
      const maxVal = Math.max(1, ...Object.values(countriesObj).map(c => c.count));
      return {
        id,
        name: data.name,
        count: data.count,
        widthPercent: Math.round((data.count / maxVal) * 75) || 5,
        trend: 'flat',
      };
  ...
  const cityLocations = useMemo(() => {
    const citiesObj = computedStats.cities as Record<string, { count: number; name: string }>;
    const list = Object.entries(citiesObj).map(([id, data]) => ({
      id,
      name: data.name,
      count: data.count,
      trend: 'flat',
  ...
  const phrasesByRegion = useMemo(() => {
    // Generate phrases based on active post phrases map
    const map: Record<string, string[]> = {};
    filteredPosts.forEach(p => {
      if (p.location) {
        const cname = p.location.countryName;
        if (!map[cname]) map[cname] = [];
        p.keyPhrases.forEach(ph => {
          if (!map[cname].includes(ph)) map[cname].push(ph);
        });
      }
    });

    const list = Object.entries(map).map(([cname, phrases]) => ({
      country: cname.length > 13 ? `${cname.slice(0, 10)}...` : cname,
      countryId: cname.toLowerCase().slice(0, 2),
      phrases: phrases.slice(0, 3).map((ph, i) => ({
        text: ph,
        size: i === 0 ? 'text-base font-normal text-slate-800' : 'text-xs text-slate-500',
      })),
    })).slice(0, 5);
  ...
  const languagesData = useMemo(() => {
    const languagesObj = computedStats.languages as Record<string, { count: number; name: string }>;
    const list = Object.entries(languagesObj).map(([id, data]) => {
      const maxVal = Math.max(1, ...Object.values(languagesObj).map(l => l.count));
      return {
        id,
        name: data.name,
        count: data.count,
        widthPercent: Math.round((data.count / maxVal) * 100),
    ...
    <div className="space-y-4" id="location-tab-view">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-white rounded-none border border-slate-200/90 p-4 shadow-2xs" id="widget-location-sentiment">
            <div className="flex items-center justify-between pb-2">
              ...
              <button
                type="button"
                onClick={() => onExportWidgetData('Sentiment', { index: computedStats.indexScore, change: 10.0 })}
            ...
            <div className="pt-2 flex items-center justify-between">
              ...
              <div className="text-left">
                <div className="text-3xl font-extralight text-slate-900 tracking-tight">
                  {computedStats.indexScore > 0 ? `+${computedStats.indexScore}` : computedStats.indexScore}
                </div>
              ...
              <div className="relative flex items-center justify-center">
                <svg className="w-24 h-24 transform -rotate-90">
                  ...
                  <circle
                    ...
                    strokeWidth="7"
                    strokeDasharray="213"
                    strokeDashoffset={Math.max(0, 213 - (computedStats.indexScore + 10) / 20 * 213)}
                    fill="transparent"
                  />
                </svg>
            ...
            <div className="mt-4 pt-2">
              ...
              <div className="relative h-3 w-full bg-slate-100 border border-slate-200/80 overflow-hidden flex">
                ...
                <div
                  ...
                  style={{
                    width: `${Math.max(0, Math.min(100, (computedStats.indexScore + 10) / 20 * 100))}%`
                  }}
                />
              </div>
            </div>
              ...
              {sentimentByRegion.map((item) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-1 rounded-xs transition-colors ${
                    activeRegionFilter === item.id ? 'bg-slate-100 font-semibold border-l-2 border-slate-800 pl-2' : 'hover:bg-slate-50'
                  }`}
                >
            ...
            <div className="space-y-2.5 mt-2">
              {locationGroups.map((grp) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-1 rounded-xs transition-colors ${
                    activeRegionFilter === grp.id ? 'bg-slate-100 font-semibold border-l-2 border-slate-800 pl-2' : 'hover:bg-slate-50'
                  }`}
                >
        ...
        <div className="lg:col-span-6 space-y-4">
          {/* 2.1 LOCATION INSIGHTS (Big World Map with UK Cluster) */}
          <div className="bg-white rounded-none border border-slate-200/90 p-4 shadow-2xs" id="widget-big-location-map">
            <div className="flex items-center justify-between pb-2">
              ...
              <button
                type="button"
                onClick={() => onExportWidgetData('LocationInsightsMap', { totalLocations: computedStats.total })}
                className="text-slate-400 hover:text-slate-700 p-0.5"
            ...
            <div className="relative w-full h-[320px] bg-[#B9CEEB] border border-slate-200 overflow-hidden select-none">
              <svg viewBox="0 0 1000 500" className="w-full h-full object-cover">
                ...
                >
                  ...
                  />
                  <text x="485" y="125" fill="#FFFFFF" fontSize="13" fontWeight="bold" textAnchor="middle" fontFamily="sans-serif">
                    {computedStats.countries.uk?.count || 59}
                  </text>
                </g>
                ...
                >
                  <circle
                    ...
                    className="transition-all duration-200"
                  />
                  <text x="340" y="184" fill="#FFFFFF" fontSize="10" fontWeight="bold" textAnchor="middle" fontFamily="sans-serif">
                    {computedStats.countries.us?.count || 4}
                  </text>
                </g>
                ...
                >
                  <circle
                    ...
                    className="transition-all duration-200"
                  />
                  <text x="510" y="158" fill="#FFFFFF" fontSize="9" fontWeight="bold" textAnchor="middle" fontFamily="sans-serif">
                    {computedStats.countries.it?.count || 2}
                  </text>
                </g>
                ...
                >
                  <circle
                    ...
                    className="transition-all duration-200"
                  />
                  <text x="612" y="193" fill="#FFFFFF" fontSize="10" fontWeight="bold" textAnchor="middle" fontFamily="sans-serif">
                    {computedStats.countries.in?.count || 4}
                  </text>
                </g>
              </svg>
                  ...
                  >
                    {selectedMapPoint === 'uk' && (
                      <div className="text-left">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-300 gap-4">
                          <span>Buzz Volume:</span>
                          <span className="font-mono font-bold text-white">{computedStats.countries.uk?.count || 59} posts</span>
                        </div>
                    ...
                    {selectedMapPoint === 'us' && (
                      <div className="text-left">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-300 gap-4">
                          <span>Buzz Volume:</span>
                          <span className="font-mono font-bold text-white">{computedStats.countries.us?.count || 4} posts</span>
                        </div>
                        <div className="flex items-center justify-between text-[11px] text-slate-300">
                          <span>Sentiment:</span>
                          <span className="text-emerald-400 font-semibold">+10.0 ↗</span>
                    ...
                    {selectedMapPoint === 'it' && (
                      <div className="text-left">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-300 gap-4">
                          <span>Buzz Volume:</span>
                          <span className="font-mono font-bold text-white">{computedStats.countries.it?.count || 2} posts</span>
                        </div>
                        <div className="flex items-center justify-between text-[11px] text-slate-300">
                          <span>Sentiment:</span>
                          <span className="text-emerald-400 font-semibold">+10.0 ↗</span>
                    ...
                    {selectedMapPoint === 'in' && (
                      <div className="text-left">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-300 gap-4">
                          <span>Buzz Volume:</span>
                          <span className="font-mono font-bold text-white">{computedStats.countries.in?.count || 4} posts</span>
                        </div>
                        <div className="flex items-center justify-between text-[11px] text-slate-300">
                          <span>Sentiment:</span>
                          <span className="text-emerald-400 font-semibold">+10.0 ↗</span>
              ...
              <div className="absolute bottom-2 left-2 bg-slate-800/80 text-white p-2 rounded-none text-[10px] space-y-1 backdrop-blur-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-0.5 text-[9px] text-slate-300 text-left">
                    <div>Max: {computedStats.countries.uk?.count || 59}</div>
                    <div>Min: 0</div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute bottom-1 right-24 text-[8px] text-slate-600 font-sans">
                © 2026 Microsoft Corporation © HERE
              </div>
              ...
              <div className="space-y-2 mt-2">
                {countryLocations.map((loc) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-1 rounded-xs transition-colors ${
                      activeRegionFilter === loc.id ? 'bg-slate-100 font-semibold border-l-2 border-slate-800 pl-2' : 'hover:bg-slate-50'
                    }`}
                  >
                    <div className="flex items-center justify-between w-28 shrink-0 pr-2 text-left">
            ...
            <div className="bg-white rounded-none border border-slate-200/90 p-4 shadow-2xs" id="widget-cities">
              ...
              <div className="space-y-2 mt-2">
                {cityLocations.map((city) => (
                  <div key={city.id} className="flex items-center justify-between p-1 text-xs">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="flex-1 pl-3 space-y-2 text-xs">
                {locationCoverageData.items.map((item) => (
                  <div
                    key={item.name}
                    className="w-full flex items-center justify-between p-1 rounded-xs"
                  >
            ...
            <div className="space-y-2.5 mt-2">
              {phrasesByRegion.map((row) => (
                <div key={row.country} className="flex items-center gap-2">
                  <span className="w-20 text-[11px] text-slate-700 truncate shrink-0 text-left">
                    {row.country}
                  </span>
