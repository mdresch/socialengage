...
**2026-08-01 update — Admin UI complete.** Minimal admin UI implemented in `social-listening-admin/public/`: `index.html` (landing page with navigation), `connect.html` (connect/disconnect platforms with auth support), `watchlists.html` (full CRUD for watchlists), `connectors.html` (health status dashboard with auto-refresh). All pages use `CORE_API_BASE_URL` environment variable to locate core API. Updated `src/lib/core-client.ts` with typed functions for all Phase 1 endpoints. See `.claude/skills/core-api-client/SKILL.md`.

Phase 1 now closes on its own original terms ("one platform, one tenant, one watchlist... end to end") for the storied half of that deliverable. The "also build, not storied" half is now **complete**: watchlist CRUD (Story 1.5), connector connect/disconnect endpoints (Story 1.6), and **admin UI** (connect flow, watchlist management screen, connector status view) with `public/connect.html`, `public/watchlists.html`, `public/connectors.html`, and `public/index.html`.
