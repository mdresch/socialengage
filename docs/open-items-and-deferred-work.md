...
|---|---|
| **1 (MVP)** | **The RSS/News connector shipped 2026-08-01** (Story 2.7/ADR-0026, GNews API) — see the dated note below. Watchlist CRUD (Story 1.5) and connector connect/disconnect (Stories 1.6/1.7) are also now complete. Still not built: the admin UI itself. **2026-08-04: the admin UI's own user stories are now drafted** (`docs/user-stories/epic-6-admin-ui.md`, Stories 6.1–6.7), covering the connect flow (6.3), watchlist management (6.4), and connector status view (6.5) named here — plus a prerequisite auth/session story (6.1, sourced from a new ADR-0036, Proposed), a role-gated routing shell (6.2), and self-service tenant sign-up (6.7, sourced from a new ADR-0037, Proposed). **Three real, previously unnamed backend gaps surfaced by these drafting passes:** no `social-listening-core` endpoint exposes a signed-in caller's resolved role/identity over HTTP (blocks Story 6.1); Story 6.5's status view needs a "list this tenant's connectors" endpoint that doesn't exist either (only single-platform `GET /v1/connectors/:platformId` does); and no endpoint or authorization mechanism exists for an unauthenticated-until-that-instant caller to provision their own tenant (blocks Story 6.7 — `createTenant()` is reachable only through `platform_admin_role` today, confirmed against `migrations/0017_create_tenants.sql`). The 2026-08-03 design mockup (`docs/design/admin-ui-mockup-2026-08-03.html`) remains a layout/flow reference only, not yet reconciled screen-by-screen against these stories. |
...
| **5 (Production readiness)** | The entire phase: security review, load/chaos testing against the isolation guarantees this whole ADR series argued for (RLS, per-tenant rate limits, per-tenant event filtering), operational runbooks for the failure modes ADR-0009/0010/0023 describe, real authentication (see Section B), final go-live checks. **2026-08-06: "final go-live checks" is now a defined, gated decision, not an informal phrase** — see `docs/project docs/Project Management Plans/Go-Live-Readiness-Definition.md`. |
...
- **Resolved 2026-08-05, Story 5.11:** `GET /v1/me` now exposes a resolved caller's own identity/role over HTTP, mounted behind the same auth middleware every other substantive `/v1` route uses, deriving identity exclusively from `req.identity` (ADR-0036 §5's Clarification) — see `contracts/epic-5/story-5.11.get-v1-me.contract.test.ts` and `docs/implementation-log.md`. The admin UI's own role-gating (Story 6.2) and Platform Admin console (Story 6.6) are no longer blocked on this specific gap.
...
- **Still open, confirmed 2026-08-04, ADR-0037 now Accepted: no self-service tenant-creation authorization mechanism or endpoint exists yet.** `tenantStore.ts`'s `createTenant()` is callable only through `platform_admin_role`'s own Postgres grant (`migrations/0017_create_tenants.sql`) — an unauthenticated-until-that-instant sign-up caller cannot reach it, and widening `platform_admin_role` itself would violate ADR-0030 §2's already-locked boundary. **ADR-0037 (Accepted 2026-08-04)** decides a new, narrower `tenant_signup_role` and the domain-match/public-email-provider handling this needs; the `POST /v1/tenants/self-service-signup`-shaped endpoint itself is named as required but not yet designed or built. Story 6.7 (self-service sign-up) is Ready, not Blocked, but still depends on this endpoint existing.
- **New, confirmed 2026-08-04: ADR-0037 §8 (decided at acceptance) names a Tenant-Admin-facing "Same-Domain Invite Assist" surface that no Epic 6 story yet owns.** On a domain-match rejection, the attempt is recorded (a new tenant-scoped `domain_signup_attempts` table) and must be surfaced to the matched tenant's own Tenant-Admin as a one-click-actionable proposal, with visibility escalating on repeated attempts, plus a Platform-Admin-visible escalation write to `platform_admin_audit_log`. None of Stories 6.1–6.6 (drafted before this decision) name this screen/surface — a real, new admin-UI requirement without a story yet, not just a missing backend endpoint. Also open: whether/when the escalation signal graduates from durably-logged to real-time alerting, and whether this project ever builds outbound-notification capability (ADR-0037 §8c/§8d name both as real future enhancements, not designed or built).
