...
## 2026-08-24 — Story 3.17 — social-listening-core@bdd9bcf

- **Full commit:** `bdd9bcf6939f90255f6c2cbad1c9ae89a02c7d47`
- **Repo:** social-listening-core
- **Story / ADR:** 3.17 / ADR-0076
- **Contract:** social-listening-core/contracts/epic-3/story-3.17.composer-deep-research.contract.test.ts
- **SKILL.md:** social-listening-core/.claude/skills/composer-research/SKILL.md
...
- **Epic-3 suite at merge:** PASS (19/19 suites, 137/137 tests)

**Subagent implementation + one healing pass:** the initial implementation was generated by a background subagent following `implement-story`. The contract initially failed AC3 (`SEARCH_PROVIDER_UNAVAILABLE` vs. `http_401`) because the orchestration ran Azure OpenAI extraction before checking Brave/Bing availability; re-ordering `discoverActiveSearchProviders()` to occur before the first `research()` call lets the endpoint return the correct error code without consuming AI quota. Also fixed a contract type issue (`RequestInfo` not in the project's TS lib) by using `string | URL | Request`.

---

## 2026-08-24 — Story 6.31 — social-listening-admin

- **Full commit:** `9260f6f73f84f3f6a9955546878d54d0268d97ab`
- **Repo:** social-listening-admin
- **Story / ADR:** 6.31 / ADR-0071 (Human-in-the-Loop Post Enrichment Cascading Edit Drawer)
- **Contract:** social-listening-admin/contracts/epic-6/story-6.31.post-enrichment-cascading-edit-drawer.contract.test.ts (11/11)
...
- **Contract:** social-listening-core/contracts/epic-9/story-9.5.onboarding-checklist-state.contract.test.ts (17/17)
- **SKILL.md:** social-listening-core/.claude/skills/onboarding-checklist/SKILL.md
...
- **SKILL.md:** `social-listening-core/.claude/skills/rag-connector/SKILL.md`
- **Suite:** new contract PASS (6/6); epic-9 RAG regression suite PASS (24/24 — `story-9.7`/`9.8`/`9.9`/`9.10`); `npm run typecheck` PASS with 0 errors. Full accumulated `social-listening-core` suite re-run for the shared-file touches (see Notes): pre-existing failures only, verified against a clean baseline — no new regressions attributable to this story.
- **Files touched:**
  - `docs/implementation-plan.md`
  - `docs/project docs/Business-Requirements/BRD-0136-RAG-Connector-Provider-Abstraction-Namespace-Per-Tenant.md`
  - `docs/project docs/Functional-Design/FDD-0136-RAG-Connector-Provider-Abstraction-Namespace-Per-Tenant.md`
  - `docs/user-stories/README.md`
  - `docs/user-stories/epic-19-adr-0136-to-0140.md`
  - `social-listening-core/.claude/skills/rag-connector/SKILL.md`
  - `social-listening-core/contracts/epic-19/story-19.1.rag-connector-namespace-isolation.contract.test.ts`
  - `social-listening-core/migrations/0082_force_rls_rag_chunks.sql`
  - `social-listening-core/src/rag/pgvectorConnector.ts`
  - `social-listening-core/src/rag/types.ts`
- **Notes:**
  - **Pre-implementation blocker, resolved before any code was written:** ADR-0136 was Accepted (2026-08-28) but BRD-0136 and FDD-0136 were one-line stubs from the 2026-08-23 batch-drafting convention — the exact same gap Story 17.3 hit for ADR-0131/BRD-0131/FDD-0131 (see that story's own 2026-09-09 entry above). Followed the same resolution: spawned the `ba-requirements-analyst` agent to draft real BRD/FDD Decision content from ADR-0136's own already-complete Decision text (ADR-0136 itself, unlike ADR-0131 at the time, was already fully drafted — only its BRD/FDD companions were stubs), before any implementation code was written.
  - **A real, verified finding surfaced by that BRD/FDD-drafting pass, more specific than ADR-0136's own Context §3:** direct inspection of the already-shipped Story 9.7 code showed `rag_chunks` already carried an RLS policy (`migrations/0046_create_rag_chunks_and_sync_tables.sql`, aligned to accept either `app.tenant_id` or `app.current_tenant_id` by migration `0047`), but `PgvectorRAGConnector`'s own queries ran exclusively through `getAdminPool()` (`src/db/adminPool.ts`, Postgres superuser — connects as `PGUSER ?? 'postgres'`), which bypasses RLS unconditionally regardless of policy presence. So the policy was structurally present but never evaluated against this connector's real traffic, and Story 9.9's own contract test named `story-9.9.rag-vector-rls.contract.test.ts` ("guarantees tenant isolation") verified only the connector's in-process application-level `tenant_id` filter, not the database-level policy its own filename claims.
  - **Fix:** `PgvectorRAGConnector`'s tenant-scoped queries (`upsert`, both `search()` DB reads — `rag_chunks` and the Story 16.2 `social_posts` restricted-post check — `deletePost`, `deleteTenant`, and `status(tenantId)`) now route through `withTenant(tenantId, ...)` (`src/db/withTenant.ts`), which uses `getPool()`'s `app_user` role (RLS-subject) and sets the `app.tenant_id` session variable transaction-locally — this project's own established, already-documented (`postgres-tenant-db/SKILL.md`) mechanism, which this connector simply wasn't using. The existing application-level `tenant_id` equality checks are unchanged, now correctly serving as mandatory secondary defense-in-depth rather than the sole mechanism. `status()`'s cross-tenant aggregate branch (`tenantId` omitted) keeps `getAdminPool()` — verified via `ragRouter.ts` and `ragReconciliationService.ts` that no real production caller ever omits `tenantId`, and there is no single tenant to scope an RLS session context by for a true cross-tenant count.
  - **Interface additions** (`src/rag/types.ts`, ADR-0136 Decision §1): `RAGConnector` gains an optional `ensureTenantNamespace?(tenantId: string): Promise<void>` (pgvector correctly omits implementing it — its isolation is a static, already-provisioned table-level RLS policy, not a per-tenant-provisioned resource); `RAGConnectorStatus` gains a required `isolationModel: 'namespace' | 'shard' | 'row-level-rls' | 'metadata-filter-only'` field, set to `'row-level-rls'` for pgvector. **Resolved ADR-0136's own restated-but-never-reconciled drift** (its Decision text still shows ADR-0081's original, never-shipped `ConnectorStatus`/`RAGConnector` names): attached both new members to the actually-shipped `RAGConnectorStatus`/`RAGConnector` in `types.ts`, since that is what every real caller (`ragRouter.ts`, `ragSearchService.ts`, all four epic-9 RAG contract tests) actually consumes — flagged as an open question in FDD-0136 §13 Q2, resolved here per the FDD's own stated (non-mandatory) recommendation.
  - **Migration `0082_force_rls_rag_chunks.sql`:** adds `FORCE ROW LEVEL SECURITY` to `rag_chunks`, closing the one remaining gap relative to every other tenant table's established pattern (`migrations/0002_enable_rls_social_posts.sql`). Not functionally required for this fix alone (`app_user` is a non-owner grantee, so `ENABLE ROW LEVEL SECURITY` alone already applies to it) but brings `rag_chunks` to full parity with this project's own convention, per ADR-0136 Decision §2's "exact RLS pattern already established project-wide."
  - **Incidental, same-file consistency fix, not scope creep:** `search()`'s second DB call (the Story 16.2/ADR-0126 `social_posts` restricted-post-id check) was also switched from `getAdminPool()` to `withTenant()`, since it lives in the same method, in the same class, under the same "this connector's queries must not run as superuser" mandate this story exists to enforce — leaving one call site on the old pattern would have been an inconsistent half-fix in the exact file this story rewrites, not a separate feature.
  - **Contract test** (`contracts/epic-19/story-19.1.rag-connector-namespace-isolation.contract.test.ts`, 6/6) exercises the **real production call site**: a `jest.spyOn` call-site check (not a behavior mock — the real DB calls still execute) proves `upsert`/`search`/`deletePost`/`deleteTenant`/`status(tenantId)` never call `getAdminPool()`; a real, unfiltered `withTenant()` query (Story 5.4's own AC3 pattern, extended to `rag_chunks`) against data written through the connector's real `upsert()` call proves cross-tenant rows are unreachable independent of the connector's own application-level filter; `deleteTenant()` mechanics and the deterministic vector-ID scheme are confirmed unchanged; and `ragConnectorRegistry.ts` is confirmed to still register only `pgvector` (scope guard against Story 19.3 creep). Real tenant fixtures (`INSERT INTO tenants ... RETURNING id` via `getPlatformAdminPool()`, this project's own established fixture pattern) were required because `rag_chunks.tenant_id REFERENCES tenants(id)` — an arbitrary UUID fails the FK constraint and was silently swallowed by the connector's existing `catch` degrade-to-in-memory behavior, which is itself worth noting: **this means Story 9.7/9.8/9.9's own existing contract tests, which use hardcoded non-existent tenant UUIDs, have likely never actually exercised a real successful `rag_chunks` database write in any test run** — they pass entirely via the in-memory fallback path. Not fixed here (those tests' own hardcoded UUIDs are unchanged, outside this story's scope) — flagged for whoever next touches Story 9.7/9.8/9.9.
  - **Full accumulated suite verification, done carefully after an initial false signal:** a first `npm run test:contracts` comparison against a concurrently-running baseline on `main` showed a large, suspicious jump (32 → 106 raw failure lines) — traced to the two full 150+-suite runs contending for the same shared local Postgres container/CPU at once (the same "resource-contention-under-maximum-parallel-load pattern" `docs/environment-gotchas.md` and Story 17.3's own entry above already document, made worse by running two full suites simultaneously instead of one). Redone properly: a clean solo run of this story's full suite (43 distinct failing suites, all real-third-party-credential-dependent — Azure Key Vault/Blob Storage, Entra, GNews/Facebook/Instagram/LinkedIn/Brave/Bing/YouTube connector APIs, CRM webhooks — matching the documented pattern), then `git stash` of exactly this story's three source/migration changes and a direct re-run of a 9-suite sample of the newly-appearing names in the *same* environment — all 9 failed identically with the fix reverted (e.g. `story-3.8`: `RestError: The subscription associated with this vault has been disabled` — an Azure account-level condition, unrelated to any code in this repo), confirming non-attribution the same way Story 17.3's own entry above did, not merely assumed from the category match.
  - **Documentation gap noted, not fixed (out of scope for this story):** `docs/adr/README.md`'s master ADR index already lists ADR-0136 correctly (Accepted), but `docs/user-stories/README.md`'s Epics master table has no row at all for Epic 19 (or Epics 15–17) — the same pre-existing gap already flagged for Epic 17 in Story 17.3's own entry above, now confirmed to also cover Epic 19. Flagged for `documentation-steward`, not backfilled here (a dated note was added to `docs/user-stories/README.md` pointing at this gap, per that file's own established correction convention, without attempting the full multi-epic backfill).
  - **A stale story-file field corrected in place, not as scope creep:** `docs/user-stories/epic-19-adr-0136-to-0140.md`'s Story 19.1 header said "ADR-0136 (Proposed 2026-08-28)" — stale; ADR-0136 itself and `docs/adr/README.md` both already show it Accepted 2026-08-28. Corrected with a dated note, consistent with this project's own established correction convention for this exact kind of drift.
  - **Known, pre-existing, unrelated loose end also noted, not fixed:** the primary checkout was found sitting directly on `feat/story-19.1` (0 commits, 0 diff vs. `main`) rather than `main` at session start, alongside an unrelated uncommitted change to `.github/instructions/codacy.instructions.md`. Moved the primary checkout back to `main` (safe — no unique commits existed on that branch) before provisioning this story's own isolated worktree, per the worktree-isolation process; the `codacy.instructions.md` drift itself is unrelated and left untouched. Separately, a leftover `../socialengage-story-12.17` worktree (branch `feat/story-12.17`) was found still provisioned at session start, from a prior session that did not complete its own Step 10 teardown — not this story's to clean up, flagged here for whoever next touches Story 12.17.
...
- **SKILL.md:** `social-listening-core/.claude/skills/rag-chunking-pipeline/SKILL.md`
- **Suite:** new contract PASS (5/5); epic-9 RAG suite plus Story 19.1's own contract PASS (30/30 — `story-9.7`/`9.8`/`9.9`/`9.10`/`19.1`); `npm run typecheck` PASS with 0 errors. Full accumulated `social-listening-core` suite re-run for the shared-file touch (`ragIndexingPipeline.ts`): one suite (`story-1.10.postgres-readiness-and-health`) flagged as newly failing in the full 152-suite parallel run, confirmed non-attributable — passes cleanly (8/8) in isolation (see Notes).
- **Files touched:**
  - `docs/implementation-plan.md`
  - `docs/adr/0137-rag-post-chunking-and-embedding-namespace-routing.md`
  - `docs/project docs/Business-Requirements/BRD-0137-RAG-Post-Chunking-And-Embedding-Namespace-Routing.md`
  - `docs/project docs/Functional-Design/FDD-0137-RAG-Post-Chunking-And-Embedding-Namespace-Routing.md`
  - `docs/user-stories/epic-19-adr-0136-to-0140.md`
  - `social-listening-core/.claude/skills/rag-chunking-pipeline/SKILL.md`
  - `social-listening-core/contracts/epic-19/story-19.2.rag-chunking-embedding-namespace-routing.contract.test.ts`
  - `social-listening-core/migrations/0083_force_rls_rag_chunks_sync.sql`
  - `social-listening-core/src/rag/ragIndexingPipeline.ts`
- **Notes:**
  - **Pre-implementation blocker, more severe than Story 19.1's own:** ADR-0137 was Accepted (2026-08-28) but, unlike sibling ADR-0136, was itself a one-line stub with no Context/Decision/Consequences at all — yet ADR-0136's own Context §4 and Consequences #4 both already forward-referenced ADR-0137 by name, asserting as settled fact ("This is confirmed explicitly in ADR-0137 and ADR-0139's own 'Relation to ADR-0136' sections") a confirmation ADR-0137 had never actually delivered. Spawned `ba-requirements-analyst` to draft real ADR-0137/BRD-0137/FDD-0137 content, grounded in ADR-0136's own forward-references, ADR-0082's original Decision, and direct inspection of the already-shipped pipeline code — before any implementation code was written, the same precedent as Story 19.1/17.3.
  - **Two real, previously-unflagged findings surfaced by that drafting pass, beyond the confirmation ADR-0136 already expected:**
    1. ADR-0136 Decision §1 added the optional `ensureTenantNamespace?()` hook to `RAGConnector` (Story 19.1), but nothing in the shipped codebase ever called it — the hook existed with zero callers.
    2. `indexPostForRAG()` (`ragIndexingPipeline.ts`) has its own direct `rag_chunks_sync` write via `getAdminPool ? getAdminPool() : getPool()` — the exact same superuser-bypass anti-pattern Story 19.1 fixed for `rag_chunks` in `PgvectorRAGConnector`, except one layer up, in the pipeline rather than the connector, on `rag_chunks_sync` (a sibling table from the same migrations `0046`/`0047`, same RLS policy shape, same "structurally present, never evaluated" gap).
  - **Fix:** `indexPostForRAG()` now calls `await connector.ensureTenantNamespace?.(tenantId)` immediately before `connector.upsert(...)`, inside the function's existing per-attempt retry block (optional-chained — a no-op for pgvector, the only registered provider today, ready for Story 19.3's future namespace-capable provider without a second breaking pipeline change). The `rag_chunks_sync` write now routes through `withTenant(tenantId, ...)` (`app_user`, RLS-subject) instead of `getAdminPool()`/`getPool()` directly — identical SQL shape, only the pool/role and transaction wrapper changed. Migration `0083_force_rls_rag_chunks_sync.sql` adds `FORCE ROW LEVEL SECURITY` to `rag_chunks_sync`, matching migration `0082`'s precedent for `rag_chunks` (Should, not Must, per ADR-0137 Open Question Q1 — done anyway for consistency, since `app_user` is a non-owner grantee and the change is a single, low-risk line).
  - **Explicit scope exclusions, verified not just asserted:** `backfill.ts` (`getAdminPool()` for its own cross-tenant reads) confirmed a legitimate, already-sanctioned batch-job exception (`.claude/skills/postgres-tenant-db/SKILL.md`'s Known Gaps precedent), left unchanged. `ragReconciliationService.ts`'s `reconcileOrphanedRAGChunks()` has a *separate, more severe* defect — it never calls `withTenant()` at all, so its queries fail closed (return zero rows unconditionally) rather than merely bypassing RLS — flagged for whoever picks up ADR-0138/Story 19.3, not fixed here. A third, unrelated defect was also found and named but explicitly not fixed: `indexPostForRAG()`'s Step 3 orphan-chunk-cleanup loop computes an obsolete vector ID but never deletes it — dead code, meaning ADR-0082 Decision §5's own cleanup requirement has never actually been implemented despite ADR-0082 claiming otherwise. `rag-chunking-pipeline/SKILL.md`'s own Key Invariant 5 repeated that same false claim; corrected in place as part of this story's SKILL.md update (the file was already being touched for the real Story 19.2 changes) rather than left standing. Flagged for a future ADR-0082/Story 9.8 revisit, not decided or fixed under ADR-0137.
  - **Contract test** (`contracts/epic-19/story-19.2.rag-chunking-embedding-namespace-routing.contract.test.ts`, 5/5) exercises the **real production call site**: a mock namespace-capable connector proves call order (`ensureTenantNamespace` before `upsert`); a real `PgvectorRAGConnector` (which doesn't implement the hook) proves the no-op path still succeeds; a `jest.spyOn` call-site check proves `getAdminPool()` is never invoked by a real `indexPostForRAG()` pass; a real, unfiltered `withTenant()` query under tenant B's session context proves tenant A's `rag_chunks_sync` row (written via the real `indexPostForRAG()` call site) is unreachable; and `RAGChunkingService.split()`/`.embed()` are confirmed unchanged. Real tenant fixtures (`getPlatformAdminPool()`, Story 19.1's own established pattern) were required for the same FK reason Story 19.1 documented (`rag_chunks_sync.tenant_id REFERENCES tenants(id)`).
  - **Full accumulated suite verification:** a clean solo run of the full suite showed exactly one suite beyond Story 19.1's own already-documented 43-suite environmental baseline: `contracts/epic-1/story-1.10.postgres-readiness-and-health.contract.test.ts`. Re-run in isolation immediately after: 8/8 passing — confirming this was resource-contention flakiness under full 152-suite parallel load (this specific suite spawns real child processes and performs timed connectivity checks, a known-sensitive shape under CPU/IO contention), not a regression, consistent with the non-attribution method Story 19.1's own entry above established.
  - **Documentation gap, already flagged, reconfirmed, not re-fixed:** `docs/user-stories/README.md`'s Epics master table still has no row for Epic 19 — already flagged in Story 19.1's own entry above and via a dated note in that file; no further action taken here, per that same file's own established convention of one dated note per confirmed gap rather than repeating it per story.
...
- **SKILL.md:** `social-listening-core/.claude/skills/rag-vector-rls/SKILL.md`
- **Suite:** new contract PASS (3/3); epic-19 suite PASS (14/14); `npm run typecheck` PASS with 0 errors. Epic-9 suite re-run for the shared RAG area: `story-9.2.metric-explainability` returns 500s in isolation — a pre-existing, unrelated failure (Azure/credential-dependent), not attributable to this change (see Notes).
- **Files touched:**
  - `docs/implementation-plan.md`
  - `docs/adr/README.md`
  - `docs/adr/0081-rag-connector-provider-abstraction.md`
  - `docs/adr/0083-rag-vector-store-rls-and-metadata.md`
  - `docs/adr/0138-rag-vector-store-namespace-per-tenant-isolation.md`
  - `docs/project docs/Business-Requirements/BRD-0138-RAG-Vector-Store-Namespace-Per-Tenant-Isolation.md`
  - `docs/project docs/Functional-Design/FDD-0138-RAG-Vector-Store-Namespace-Per-Tenant-Isolation.md`
  - `docs/user-stories/epic-19-adr-0136-to-0140.md`
  - `project-progress-dashboard/src/lib/project-dashboard/data.ts`
  - `social-listening-core/.claude/skills/rag-vector-rls/SKILL.md`
  - `social-listening-core/contracts/epic-19/story-19.3.rag-vector-store-namespace-isolation.contract.test.ts`
  - `social-listening-core/src/rag/ragReconciliationService.ts`
- **Notes:**
  - **Same-session spec-drafting precedent:** ADR-0138 was Accepted (2026-08-28) but was itself a one-line stub with no Decision text — the same pattern resolved for ADR-0131 (Story 17.3), ADR-0136's own BRD/FDD (Story 19.1), and ADR-0137 in full (Story 19.2). Real ADR-0138/BRD-0138/FDD-0138 content was drafted this pass, grounded in ADR-0136's own forward-references, ADR-0083's original Decision, and direct inspection of `pgvectorConnector.ts`, `ragReconciliationService.ts`, `withTenant.ts`, `adminPool.ts`, and migrations `0046`/`0047`/`0082`/`0083`.
  - **Fix:** `RAGReconciliationService.reconcileOrphanedRAGChunks()`'s real-DB branch previously called `getPool()` directly with no `withTenant()` session-context call at all — not the admin-pool-bypass pattern Stories 19.1/19.2 fixed, but total absence of the RLS session mechanism. With `app.tenant_id` never set, `FORCE`d RLS on `rag_chunks`/`social_posts`/`rag_chunks_sync` made the orphan-detection `SELECT` return zero rows unconditionally — a fail-closed bug (reconciliation silently found nothing, never a leak). Now routes both the `SELECT` and the loop's cleanup `DELETE FROM rag_chunks_sync` through one `withTenant(tenantId, ...)` call, the same mechanism already applied throughout `src/rag/` under Stories 19.1/19.2. No new migration needed (`rag_chunks`/`rag_chunks_sync` already carry `FORCE ROW LEVEL SECURITY`, migrations `0082`/`0083`).
  - **Contract test** (`contracts/epic-19/story-19.3.rag-vector-store-namespace-isolation.contract.test.ts`, 3/3) exercises the **real production call site**: a `jest.spyOn` call-site check proves `withTenant()` is invoked during a real reconciliation pass; a real orphan/live-post pair proves the fix's actual effect (the orphan's `rag_chunks`/`rag_chunks_sync` rows are found and deleted; the live post's rows are untouched); and a cross-tenant check proves reconciling tenant A leaves tenant B's own orphaned rows alone. Real tenant fixtures (`getPlatformAdminPool()`, Story 19.1's own established pattern) required for the `rag_chunks`/`social_posts`/`rag_chunks_sync` FK constraints.
  - **Foreign-suite attribution:** `npm test contracts/epic-9` surfaced `story-9.2.metric-explainability` (2 tests) failing with 500s. Re-run in isolation: same 500s — a pre-existing, Azure/credential-dependent failure unrelated to this story's `ragReconciliationService.ts` change (the suite's own AC5/AC9 already fail standalone on `main` semantics; no file in this story touches the explain endpoint, its service, or its cache). Not fixed here — flagged as a separate `heal-contract-failure` candidate for Story 9.2, not this story's to absorb.
  - **Generated telemetry artifact committed as-is:** `docs/synthesis/Self-Learning-Synthesis-Epic-14.md` was already modified in the worktree by `scripts/synthesize-telemetry.mjs` (a self-learning telemetry compile, not this story's scope). Left committed rather than reverted — regenerable, but the worktree is torn down on merge and the change would otherwise be lost.
  - **Scope exclusions confirmed:** `ragConnectorRegistry.ts` still registers only `pgvector`; no Pinecone/Weaviate connector code exists or was built (ADR-0138 documents their architecture only); the reconciliation scheduler (ADR-0083 Decision §4 item 5) remains unbuilt — confirmed by codebase-wide search that nothing anywhere invokes `RAGReconciliationService`; `RAGChunkMetadata`/`RAGConnector` interface unchanged.
...
- **SKILL.md:** `social-listening-core/.claude/skills/openapi-spec-generation/SKILL.md` (new)
- **Suite:** new contract PASS (4/4); baseline suite verification PASS (`story-1.10.postgres-readiness-and-health`, 8/8, after the environmental fix below); full accumulated suite: 120/158 suites, 1014/1193 tests pass — the 38 failing suites are a single, confirmed pre-existing/external root cause unrelated to this story (see Notes); `npm run typecheck` PASS with 0 errors.
- **Files touched:**
  - `docs/environment-gotchas.md`
  - `docs/synthesis/Self-Learning-Synthesis-Epic-14.md`
  - `docs/user-stories/epic-20-adr-0144.md`
  - `project-progress-dashboard/src/lib/project-dashboard/data.ts`
  - `social-listening-core/.claude/skills/openapi-spec-generation/SKILL.md`
  - `social-listening-core/.gitignore`
  - `social-listening-core/contracts/epic-20/story-20.1.openapi-spec-generation.contract.test.ts`
  - `social-listening-core/jest.global-setup.js`
  - `social-listening-core/jest.global-teardown.js`
  - `social-listening-core/jest.setup.js`
  - `social-listening-core/package.json`
  - `social-listening-core/scripts/generateOpenApiSpec.ts`
  - `social-listening-core/scripts/testDbClone.ts`
  - `social-listening-core/src/http/openapi/generateOpenApiDocument.ts`
  - `social-listening-core/src/http/openapi/registry.ts`
  - `social-listening-core/src/http/versions/v1/router.ts`
  - `social-listening-core/src/http/versions/v1/watchlistsRouter.ts`
- **Notes:**
  - **Resumption of interrupted work, not a fresh pickup.** A prior session had already provisioned this story's worktree (`../socialengage-story-20.1`, `feat/story-20.1`) and written most of the OpenAPI-generation code and its contract test, but ended before committing (no commit ever landed on the branch — its tip still equaled `main`'s). Per `docs/implementation-methodology.md`'s "Resuming a story interrupted before commit," the uncommitted state was re-validated from Step 1 rather than assumed correct, via `heal-contract-failure`.
  - **Scope re-validated against ADR-0144/BRD-0144/FDD-0144/TDS-0144 and the story's own three ACs.** Confirmed the prior session's own scoping judgment was right: only "backend publishes its OpenAPI spec as a CI artifact on every build" is a testable code behavior. "Repo builds and passes CI with no frontend code present" is already true (zero frontend deps, own subdirectory) and needed no change. "Original combined repo history preserved via `git filter-repo` or clean-cut" is a one-time manual git-and-new-remote operation ADR-0144's own Open Items explicitly leave to Menno's call — not performed, and not appropriate to perform autonomously (creating/pushing to a new GitHub repository and rewriting history is exactly the kind of hard-to-reverse, high-blast-radius action that needs the maintainer's own hand, per ADR-0144 itself).
  - **One real bug found and fixed in the uncommitted code:** `watchlistsRouter.ts` had `import { requireFeatureGate } from '../../auth/featureGates';` placed *after* two `registerOpenApiOperation(...)` calls, i.e. an import statement textually stranded among executable statements. ES module imports are hoisted regardless of position so this wasn't a runtime bug, but it was a real correctness/readability defect (the next edit to that block could easily have broken it) — moved above the executable code, next to the other imports.
  - **Missing component SKILL.md** — the directory `.claude/skills/openapi-spec-generation/` existed (from the interrupted session) but was empty; both new route files' own comments already referenced `.claude/skills/openapi-spec-generation/SKILL.md` by path. Written per `docs/templates/component-skill-template.md`.
  - **Real, machine-specific environmental root cause found and fixed, affecting the entire suite, not just this story:** every contract test hung indefinitely at ~0% CPU immediately after `jest.global-setup.js`'s own "Container ... Healthy" log line, with zero further output — reproduced identically on a known-good, unrelated baseline (`story-1.10.postgres-readiness-and-health`), proving it wasn't this story's code. Root-caused directly, not assumed: a standalone `pg.Client` connect to `'localhost'` timed out at 5000ms while the identical connect to `127.0.0.1` succeeded in 18ms — this machine's Node resolves `'localhost'` to the IPv6 loopback (`::1`) first, but Docker Desktop only publishes port 5434 on IPv4, and `jest.global-setup.js`'s `maintClient` had no `connectionTimeoutMillis`, so the failed IPv6 attempt hung forever instead of failing fast. A separate, real, coincidental factor was also present and cleared first (two duplicate `jest --watch` processes from two simultaneously-open editor windows on the same repo, hammering the same shared Postgres container) — but ruling that out did not fix the hang, proving it wasn't the actual cause. Fixed by hardcoding `127.0.0.1` instead of `'localhost'` in `jest.global-setup.js`, `jest.global-teardown.js`, `jest.setup.js`, and `scripts/testDbClone.ts` (the test harness's own connections only — the various `src/db/*Pool.ts` files' own `process.env.PGHOST ?? 'localhost'` production fallback default is untouched, since `PGHOST` is always forced by the harness during tests regardless). Documented in `docs/environment-gotchas.md`.
  - **Full-suite attribution, verified not assumed:** after the environmental fix, a full `--runInBand` run showed 38 failing suites / 179 failing tests. Every single one traces to one external cause: the real Azure Key Vault `sociallistening-kv` (westeurope) sits on a **disabled Azure subscription** (`RestError: The subscription associated with this vault has been disabled`, plus a secondary `RestError: This request is not authorized to perform this operation using this permission` on the same vault) — confirmed by grep across the full run's output, not sampled. This is a superset of the already-documented `story-3.8`/`story-13.1` missing-credentials class in `docs/environment-gotchas.md` (that entry is updated to point at this more specific, more severe root cause: not merely missing third-party API keys, but the shared Key Vault itself being unreachable account-wide). Nothing this story touched (an OpenAPI registry/generator, two router files' registration calls, and four test-harness files' hardcoded host) has any path to Key Vault. **Flagged directly to Menno as a real, external infrastructure issue needing attention outside this session** — likely a lapsed/disabled Azure subscription, not a code or test-harness problem.
  - **Repository-split infrastructure explicitly not performed** (AC1/AC3) — see the scope note above. Story 20.1's `**Status:**` is recorded as "Built (partial — see Notes)" in `docs/user-stories/epic-20-adr-0144.md` rather than a plain "Built", specifically so this doesn't read as full-AC completion at a glance.
  - **Generated telemetry artifact committed as-is**, same precedent as Story 19.3's entry above: `docs/synthesis/Self-Learning-Synthesis-Epic-14.md` was already modified in the worktree (pre-dating this healing pass, not caused by it) by the self-learning telemetry pipeline. Left committed rather than reverted — regenerable, but the worktree is torn down on merge and the change would otherwise be lost.
...
- **SKILL.md:** none
- **Suite:** N/A in this repo. In the new standalone repo: build/typecheck PASS; `npm test` 118/158 suites pass (40 fail — the same pre-existing Azure/Key-Vault-credential-gap class documented in `docs/environment-gotchas.md`, now present there too since no Azure secrets are configured for the new repo yet); `openapi:generate` PASS, producing a 3-path `openapi.json` matching the local result exactly.
- **Files touched:**
  - `docs/implementation-log.md` (this entry)
  - `docs/user-stories/epic-20-adr-0144.md`
- **Notes:**
  - **Menno explicitly asked to continue with the repository split** (AC1/AC3) after the prior OpenAPI-generation healing pass above deliberately left it undone, per ADR-0144's own Open Items deferring the history-split method to his call. Two decisions were confirmed with him directly before any git-history or GitHub-repo action was taken: **clean-cut** (not `git filter-repo`) and a **new private GitHub repo, created and pushed immediately** (not left local-only).
  - **Rollback point tagged first**, per TDS-0144 step 1: `git tag pre-story-20.1-repo-split` on this repo's `main` at `8e77ceb8` (the commit prior to `fc5a6856`'s OpenAPI-generation work) — full prior history for `social-listening-core/` remains fully readable from this repo's own `git log` regardless of anything done in the new repo.
  - **Extraction method:** `git archive HEAD -- social-listening-core | tar -x --strip-components=1` into a new sibling directory — deliberately not a raw filesystem copy, which would have pulled in `node_modules/`, any local `.env`, `dist/`, and `coverage/` (none of those are tracked, so `git archive` naturally excludes them; a plain `cp -r` would not have). Added `.env`/`.env.*` (with a `.env.example` exception) to the new repo's own top-level `.gitignore` — that protection previously lived only at this workspace repo's *root* `.gitignore`, one level above `social-listening-core/`'s own, and the new repo has no such parent level.
  - **New repo:** `https://github.com/mdresch/social-listening-core` (private), initial commit `3c1e636` ("Initial commit: extract social-listening-core from the socialengage monorepo"), default branch `main`. A new `.github/workflows/ci.yml` was authored for it (this repo's own combined `ci.yml` assumes both `social-listening-core/` and `social-listening-admin/` exist side by side and doesn't apply) — build → typecheck → test → `openapi:generate` → upload `openapi.json` as a build artifact, matching TDS-0144's CI/CD summary. A follow-up commit `b4d0711` added `if: always()` to the generate/publish steps after the first real CI run showed the OpenAPI-artifact step being skipped whenever the (unrelated, pre-existing) Azure-credential-gap test failures made the job fail first — AC2 says "on every build," not conditional on that gap.
  - **Verified standalone, not assumed:** after `npm install` in the new repo (560 packages, 0 vulnerabilities), confirmed directly: `npm run build` and `npm run typecheck` both pass with zero frontend dependencies and zero `.tsx`/`.jsx`/`.razor` files anywhere in the tree (AC1); `npm run openapi:generate` produces the identical 3-path document as the source repo (AC2); `npx jest contracts/epic-20` passes 4/4 standalone. Real CI on GitHub (not just local) confirmed the same result independently: build/typecheck green, `openapi.json` now published as an artifact regardless of the test step's own outcome.
...
## 2026-09-24 — Documentation Steward correction: Story 20.1's own first log entry (2026-09-17, "Story 20.1 — social-listening-core@pending") still reads `pending`

- **Full commit:** N/A — a correction entry, per this file's own header rule ("never edit or remove an existing entry")
- **What's being corrected:** the entry above headed `## 2026-09-17 — Story 20.1 — social-listening-core@pending` (its own `**Full commit:**` field also still reads `pending`) never actually got backfilled with its real hash, `fc5a685646e303b792fd0a4263b887a757b26d54` — confirmed directly: `git diff-tree --no-commit-id --name-only -r fc5a6856` lists exactly the 17 files that entry's own "Files touched" list names (`docs/environment-gotchas.md`, `docs/implementation-log.md`, `docs/synthesis/Self-Learning-Synthesis-Epic-14.md`, `docs/user-stories/epic-20-adr-0144.md`, `project-progress-dashboard/src/lib/project-dashboard/data.ts`, and the twelve `social-listening-core/*` OpenAPI-generation files), a one-to-one match.
- **Root cause:** per this file's own header note (line 3) and the Story 17.4 pre-commit-hook-defect entry (2026-09-13) above, `scripts/git-hooks/pre-commit`'s pending-hash backfill is guarded to skip a file whose own staged diff introduces a *new* pending marker, to avoid the exact mis-attribution bug that entry found. The very next commit after `fc5a6856`, `4c9db6f2`, itself added a brand-new pending entry ("Story 20.1 (repo-split follow-up)") to this same file — tripping that guard and leaving `fc5a6856`'s own entry un-backfilled. `c23bc4ca` ("docs: backfill commit hash for Story 20.1 repo-split log entry") then backfilled only the `4c9db6f2` entry's own `pending` → `4c9db6f2` (confirmed via `git show c23bc4ca -- docs/implementation-log.md`), not this earlier one — a real, narrower fix than its own commit message's singular framing suggested. No later commit in the ADR-0144/Story 20.1 batch (`6ac96109`, `648faeb3`) touched it either.
- **Fix:** this dated correction entry records the real hash directly, without editing the original entry's text, per this file's own append-only rule. Readers of the `## 2026-09-17 — Story 20.1 — social-listening-core@pending` entry above should read its own `**Full commit:**` field as `fc5a685646e303b792fd0a4263b887a757b26d54`, not `pending`.
