...
- **Amendment Log entry, 2026-08-20 — Historical Post-Watchlist Match Backfill and Discovery-Driven Watchlist Attribution (Story 3.12).**
  - **Resolves Open Question 1 (Historical Backfill):** Authorizes a retroactive backfill mechanism (`backfillPostWatchlistMatches(tenantId?)`) and migration/script in `social-listening-core` to iterate existing historical `social_posts` rows, evaluate them against active tenant watchlists using the existing AST evaluator (`matchesAst(watchlistToAst(watchlist), post)`), and persist the resulting matches into `post_watchlist_matches ... ON CONFLICT (post_id, watchlist_id) DO NOTHING`. This closes the historical data gap for posts ingested before Story 3.11/migration `0036` was deployed.
  - **Discovery-driven Watchlist Attribution:** Authorizes explicit discovery-driven watchlist attribution in connectors where discovery search is explicitly parameterized by a tenant's watchlist terms (specifically `pollWikipedia.ts`, Story 2.14). When an article/post is discovered via a specific watchlist's discovery query, that discovering watchlist ID is explicitly included in the match allocation at ingestion alongside the general AST evaluation for all other active tenant watchlists. This guarantees that 100% of discovered Wikipedia content is credited in `post_watchlist_matches` to the watchlist that initiated the discovery.
