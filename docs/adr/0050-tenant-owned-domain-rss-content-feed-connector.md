...
**Acceptance note:** Accepted by Menno 2026-08-11, verbatim: *"ADR 0048, ADR 0049 and ADR 0050 approved."* Accepted as drafted, no revisions. A new story, **Story 2.11** (`docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md`), is added at this acceptance, per the ADR-0024/0026 "no story until acceptance" precedent named at drafting time. Decision §4's "rule of three" organization-as-Author generalization is written into ADR-0004 at this same acceptance — see ADR-0004's own new "Supersession update" note confirming the exact clause text (Open Question 6, resolved).
...
- **2026-08-11** — **Accepted by Menno**, verbatim: *"ADR 0048, ADR 0049 and ADR 0050 approved."* Accepted as drafted, no revisions. Status changed from Proposed to Accepted. Story 2.11 added to `docs/user-stories/epic-2-ingestion-connectors-and-rate-limits.md`. ADR-0004's own "Pending supersession note — 2026-08-10 (ADR-0050, organization-as-Author generalization)" gains its matching "Supersession update" entry, with the generalized clause's exact wording written at this acceptance per Open Question 6.
- **2026-08-20 — requested directly by Menno, partially resolving Open Questions 2 and 5.** Two additive amendments, neither requiring re-acceptance (both narrow, backward-compatible extensions of an already-Accepted decision, the same "implementation default, not a re-opened Decision" treatment this ADR's own Amendment Log already uses):
  - **Feed identification naming (narrows Open Question 2's "UX... need[s] explicit design at Story time," specifically for a feed's own display identity — not the broader per-domain Author-identity-separation half of that Question, which stays open).** A tenant-owned feed activation may now carry an optional, tenant-owner-set `name` (nullable `TEXT` column, `migrations/0037`) — a display-only label distinct from `domain`/`feedUrl`, letting a tenant with more than one connected feed (Story 6.20/ADR-0057's own multi-feed support) tell them apart by something more meaningful than a raw domain string in the connector setup screen. Leaving it unset is fully supported — the setup UI falls back to `domain` for display, never a forced/required field. Does **not** touch `Author` modeling, `providerId`, or any Analytics/filter grouping key — those all still key on the verified `domain` (Author) and the fixed `tenant-owned-feed` `providerId` (source/provider badges), exactly as Decision §4 already established; a feed's own `name`, when set, is additionally denormalized into each of its ingested posts' `rawPayload.feedName` for possible future display use, but nothing reads it for grouping/filtering as of this amendment — a genuinely separate, larger change (see Open Question 2's still-open per-domain-Author-identity half) if ever wanted.
  - **Per-item author identity (resolves Open Question 5, for the display layer only — not an Author-identity-resolution redesign).** `feedItemParser.ts` now extracts a per-item byline when the feed itself provides one — tried in priority order: RSS's Dublin Core `<dc:creator>` (the de facto standard most CMS platforms, e.g. WordPress, already emit per post), RSS 2.0's own flat `<author>` tag (spec says email, but real feeds often put a plain name there instead — extracted as-is, never parsed further), and Atom's nested `<author><name>`. Absent from the feed → `null`, never fabricated, the same "honest absence, not a placeholder" discipline every other extraction in this project follows. **Deliberately does not touch `Author`/`authorId` modeling** — `authorExternalId`/the `authors` table row stays the verified `domain` (organization-as-Author, Decision §4), unchanged; the per-item byline is denormalized only into `rawPayload.author`, a display-only field `social-listening-admin`'s already-existing `extractAuthor()` picks up automatically (same precedence chain Newswire's `issuer`/Facebook's `pageName` already established, 2026-08-20, same day). This closes Open Question 5's own narrower "can a per-item name be surfaced at all" question; it does **not** attempt the harder question Open Question 5 and this ADR's Consequences both name — replacing organization-as-Author with individual-as-Author for `Author`/topic-signal/analytics modeling — which stays a genuinely separate, not-yet-decided design question.
