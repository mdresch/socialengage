...
- [x] **[Q-0034-1]** ~~**Whether Tenant-Admin's revocation authority over a user-bound credential (§3) is the right reading of ADR-0028's silence on the point** — flagged explicitly for Menno's confirmation, not treated as settled by this ADR alone.~~ — **Resolved at acceptance, 2026-08-03:** confirmed as drafted. See Acceptance note.
- [ ] **[Q-0034-2]** **Exact route/parameter shape for disconnect's dual-actor case** (§5) — left to implementation.
- [ ] **[Q-0034-3]** **The two brainstormed, still-open questions this ADR inherits directly** (`docs/adr/README.md`'s 2026-07-30 note): whether connector *activation* needs its own table separate from `platform_credentials` (this ADR's answer, implicitly: no, `owner_type`/`user_id` columns on the existing table suffice for what's currently named); and whether a tenant needs multiple activations of one platform (e.g. several Facebook Pages) — **not resolved by this ADR**, since nothing currently named in this project's connector roster (GNews, Newswire) needs it; the schema in §1 does not structurally prevent multiple tenant-wide rows for the same `platformId` (no uniqueness constraint on `(tenant_id, platform_id)` is added here), but resolving *whether that should be allowed and how it's surfaced in the UI* is left open, not decided.
- [ ] **[Q-0034-4]** **The missing `implementation-log.md` entry for Story 1.6** (Context) — flagged for correction, not performed by this ADR.

## Amendment Log

- 2026-08-03 — Initial proposal, drafted by the AI Business & Requirements Analyst persona.

## Supersession update (2026-08-12) — ADR-0051 resolves this ADR's own open "activation table" question

**This ADR's own Open Questions section** named, but explicitly did not resolve, "whether connector *activation* needs its own table separate from `platform_credentials`," offering only its own implicit, non-binding reading ("this ADR's answer, implicitly: no, `owner_type`/`user_id` columns on the existing table suffice for what's currently named"). **ADR-0051** (Accepted 2026-08-12) resolves this question directly, and reaches the opposite conclusion from this ADR's own implicit reading: activation **does** need a new table, separate from `platform_credentials` — a new `connector_activations` table, credential-independent, because `authMode: 'none'` connectors (Newswire, concretely) have no credential row at all for an activation flag to live on, and because conflating "has a stored secret" with "is turned on" for credentialed connectors was itself found to be a real gap (no way to pause a connector without deleting its credential). This does not change anything in this ADR's own Decision or Consequences text above — §1's `owner_type`/`user_id` schema work on `platform_credentials` stands exactly as decided; ADR-0051 adds a wholly new, separate table alongside it, not a revision to this one. **Supersession update (2026-08-12), same day:** ADR-0051 is now Accepted (Story 1.11, `docs/user-stories/epic-1-repository-and-api-foundation.md`) — but per this series' own "don't assume already-shipped code changes automatically" convention, this only actually changes anything in `social-listening-core` once Story 1.11 is itself built, not at acceptance alone.
