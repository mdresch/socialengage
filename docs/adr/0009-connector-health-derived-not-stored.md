...
**ADR-0051** (Accepted 2026-08-12) introduces connector *activation* — a new, real, persisted `connector_activations` table recording whether a Tenant-Admin has turned a connector on for a tenant. This is a genuinely separate concept from `ConnectorHealth`, and this ADR's own Decision is unaffected: `status`, `lastSuccessfulFetchAt`, `lastAttemptAt`, and `consecutiveFailures` remain fully derived from `IngestionRun` history at read time, exactly as decided above — activation adds no new field to `ConnectorHealth`'s own derivation and does not make any part of it stored, mutable state. ADR-0051 names, as a separate implementation note on ADR-0022 (not here), that `GET /v1/connectors/:platformId`'s *response* will need to combine activation state with this ADR's derived health into one answer — that is a response-shape question for the endpoint, not a change to what this ADR decides `ConnectorHealth` itself is or how it's computed. No re-decision follows from ADR-0051.
