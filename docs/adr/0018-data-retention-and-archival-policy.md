...
- 2026-07-30 — Story 3.5 implemented, two implementation-default deviations from this ADR's literal text, both discovered while building, not assumed up front:
  - `SocialPost` is partitioned by `created_at`, not `publishedAt` as originally named. `publishedAt` is nullable (Story 4.2 — only enriched posts have it), and Postgres requires a partitioned table's partition key to be `NOT NULL` (it must be part of the primary key). `created_at` is also the more conceptually correct choice: retention is about hot-storage age since ingestion, not the source platform's original publish date.
  - `social_posts.acquisitionId`'s foreign key into `ingestion_runs` is no longer a DB-enforced constraint. Postgres refuses `DETACH PARTITION` on `ingestion_runs` while any row elsewhere holds a live FK into it — a real, enforced FK is fundamentally incompatible with `IngestionRun`'s own whole-row archival (this ADR's "move to the archival tier" for `IngestionRun`, decided as "the whole row leaves Postgres" when implementing this story). The reference stays as a populated, useful column; the "every post traces to a real run" guarantee is application-enforced only now. Confirmed with the user before dropping the constraint, not decided silently.
  
  Neither changes the durable decision (tiered retention, `IngestionRun` archived not hard-deleted, monthly partitioning as the mechanism) — both are implementation-default corrections, logged here rather than superseding. See `social-listening-core/.claude/skills/data-retention-and-archival/SKILL.md` for the full technical reasoning.

- 2026-08-09 — Partition eligibility condition for `archiveAgedRawPayloads()` corrected: the original implementation required a partition's entire month to have ended before the cutoff (`monthEnd <= cutoff`), which caused rows correctly older than the retention window to be skipped whenever they landed in the boundary month (i.e. the month that straddles the cutoff date). The corrected condition is `monthStart < cutoff` — any partition whose month started before the cutoff may contain aged rows — combined with a per-row `AND created_at < $1` filter in the inner SELECT so that rows still within the retention window are never archived, even from a partially-aged partition. The durable decision (DETACH/ATTACH PARTITION mechanism, 90-day default, field-level tiering of `rawPayload`) is unchanged — this is a correction to the partition-selection algorithm, not the architecture. Surfaced when the Docker test container's clock was one day ahead of the local machine, causing a 100-day-old test post to land in the boundary-month partition (May 2026) rather than the fully-elapsed prior month (April 2026), exposing the edge case.
