...
## Note on relation to ADR-0051 (2026-08-12, connector activation)

**ADR-0051** (Accepted 2026-08-12) introduces connector *activation* — a new, persisted `connector_activations` table, wholly separate from `ConnectorHealth`/`IngestionRun`. This ADR's own Decision (the 60-second `ConnectorHealth` cache mechanism, TTL, and in-process locality) is unaffected — activation adds no new field to what `deriveConnectorHealth()` computes, and this cache remains, as decided, "strictly a time-bounded snapshot of the same derivation ADR-0009 defines... never a value updated by any path other than recomputation."

**A real implementation note for whoever picks up the resulting Story, not a change to this ADR's own cache-mechanism Decision:** once `connector_activations` exists, whatever endpoint answers "is this connector usable for this tenant" (most likely `GET /v1/connectors/:platformId`) will need to combine two now-separate signals — activation state (from the new table, not cached by this ADR's mechanism at all) and derived health (from this ADR's existing 60-second cache) — into one response. `connectorHealthCache.ts`'s current implementation (`social-listening-core/src/connectors/connectorHealthCache.ts`, verified directly: a module-level `Map`, TTL overridable via `CONNECTOR_HEALTH_CACHE_TTL_MS`, only a blunt `flush()`-clears-everything invalidation) has no awareness of activation state today and doesn't need to gain one to stay correct under this ADR's own Decision — activation can simply be read fresh (uncached) alongside the cached health value, since `connector_activations` is a small, directly-queryable table with no equivalent aggregation cost to what `deriveConnectorHealth()` does over `IngestionRun` history. Whether that combined read is worth its own caching treatment is left to whoever builds it; not decided here.
