...
## Healing note, 2026-08-17 — this Decision's own "backed by Azure Key Vault" clause was never mechanically enforced at the connect route's own configuration boundary

Found live: Menno hit "Failed to store credential" connecting GNews through the real browser UI. `connectorsRouter.ts`'s `POST /connect` had always silently fallen back to the literal string `'placeholder-key-id'` (a stale Phase 1 shim, per its own comment) whenever `KEY_VAULT_KEY_ID` was unset — not a valid Key Vault key identifier, so every real, non-test connect attempt was always broken; every contract test creates its own ephemeral test key, so this was invisible to the accumulated suite. Fixed via `heal-contract-failure`, Story 1.7's own new AC9 — the route now fails fast with a clear, actionable error when the key isn't genuinely configured, never a silent invalid-key attempt. This Decision's own text is unaffected — it already said "backed by Azure Key Vault," full stop; this closes a mechanical-enforcement gap against it, not a new decision. See `docs/implementation-log.md`'s matching healing-pass entry (`social-listening-core@eeb9c8c`) and `credential-envelope-encryption/SKILL.md`'s new Load-bearing constraint.
