...
export function AdrView({ onSelectItem }: AdrViewProps) {
  ...
  const totalPages = Math.ceil(filteredAdrs.length / pageSize) || 1;
  const paginatedAdrs = useMemo(() => {
    const start = (currentPage - 1) * pageSize;
    return filteredAdrs.slice(start, start + pageSize);
  }, [filteredAdrs, currentPage, pageSize]);
  ...
  const handleExportCsv = () => {
    const headers = "ID,Title,Cluster,Status,File,StoryRefs\n";
    const rows = filteredAdrs
      .map(
        (a) =>
          `"${a.id}","${a.title.replace(/"/g, '""')}","${a.cluster}","${a.status}","${a.file}","${a.storyRefs.join(";")}"`
      )
      .join("\n");
    ...
    a.href = url;
    a.download = `socialengage-adrs-${Date.now()}.csv`;
    a.click();
  };
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="flex items-center justify-center py-2">
            <DonutChart
              ...
              onSelectSegment={(cluster) => {
                setSelectedCluster(cluster || "All");
                setCurrentPage(1);
              }}
              selectedSegment={selectedCluster !== "All" ? selectedCluster : null}
            />
          </CardContent>
        </Card>
            ...
            {CLUSTERS.filter((c) => c !== "All").map((c) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setSelectedCluster(isSelected ? "All" : c);
                    setCurrentPage(1);
                  }}
              ...
              >
                {CLUSTERS.map((c) => (
                  <option key={c} value={c}>
                    {c === "All" ? "All Architectural Domains" : c}
                  </option>
                ))}
              </select>
              <select
        ...
        <CardContent>
                        ...
                        {adr.storyRefs.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {adr.storyRefs.map((st) => (
                              <span
                                key={st}
                                className="inline-block px-1.5 py-0.5 rounded bg-slate-100 text-slate-700 text-xs font-mono"
                              >
                                Story {st}
                              </span>
                            ))}
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 text-xs text-slate-500">
            <div>
              ...
              {Math.min(currentPage * pageSize, filteredAdrs.length)} of {filteredAdrs.length} decisions
            </div>
            <div className="flex items-center gap-2">
              <select
