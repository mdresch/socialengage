...
- `contracts/epic-6/story-6.2.role-gated-routing-shell.contract.test.ts` — `getRoleShell()`/`getTenantShellActions()`/`isShellAllowed()`/`isResolvedIdentity()` unit behavior against the real `ResolvedIdentity` union, plus real route-guard integration tests (a real encrypted session, `next/headers`/`next/navigation` mocked the same way Story 6.1's AC7 test does) proving a session of each type — `tenant_user`, `tenant_admin`, `platform_admin`, and (2026-08-12) a genuinely unresolved `null` identity — is actually redirected off any route it isn't allowed to render, not just that the right link is shown.
