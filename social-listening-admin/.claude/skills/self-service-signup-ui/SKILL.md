...
- **This contract's own spawned `next dev` server sets `NEXT_DIST_DIR: '.next/test-story-6-7'` in its child env — do not remove.** Healed 2026-08-10: shares the exact same root cause and fix as `admin-auth-session/SKILL.md`'s own equivalent note for Story 6.1's contract (which also spawns a real `next dev` from this same project directory) — see that entry for the full explanation.
