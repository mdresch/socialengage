...
| ADR-0112 | Plan, feature gates, and seat limit management surface added at `/platform-admin/tenants/:tenantId/plan` | 13.6 |
...
- `ProvisionTenantForm.tsx` (Client Component) — `name`/`licenseSeatCount`/optional `domain` (`domain` added 2026-08-12, sent only when non-empty) → `POST /api/admin/tenants` → `createAdminTenant()`.
...
- `src/app/api/admin/tenants/route.ts` (`POST`), `src/app/api/admin/tenants/[tenantId]/route.ts` (`PATCH`), `src/app/api/admin/tenants/[tenantId]/plan/route.ts` (`GET`), `src/app/api/admin/tenants/[tenantId]/break-glass/request/route.ts` (`POST`), `src/app/api/admin/tenants/[tenantId]/break-glass/requests/[requestId]/execute/route.ts` (`POST`) — thin same-origin proxies to `core-client.ts`. The update route forwards `plan` and `featureGates` (Story 13.6, ADR-0112) in addition to `status`/`licenseSeatCount`/`name`, and still strips `domain`/`activeSeatCount`. **Named `[tenantId]`, not `[id]`** — every dynamic segment at this directory level (the plan and break-glass routes are its siblings) must share one parameter name; a real Next.js App Router routing conflict was found and fixed here via `heal-contract-failure`, see "Known gaps" below.
