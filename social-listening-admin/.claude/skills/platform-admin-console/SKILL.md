...
- `TenantAdminControls.tsx` (Client Component) — per-row `status`/`licenseSeatCount` only → `PATCH /api/admin/tenants/:id` → `updateAdminTenant()`. Never sends `domain` or `activeSeatCount`.
