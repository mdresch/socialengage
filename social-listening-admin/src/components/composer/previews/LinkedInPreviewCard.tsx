...
export function LinkedInPreviewCard({ config, text, media }: LinkedInPreviewCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden text-zinc-900 dark:text-zinc-100 font-sans">
      ...
      <div className="px-4 py-2 bg-sky-50/50 dark:bg-sky-950/20 border-b border-zinc-100 dark:border-zinc-800/80 flex items-center justify-between text-xs text-sky-800 dark:text-sky-300 font-medium">
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3">
          <div className="w-11 h-11 rounded-full bg-gradient-to-tr from-sky-600 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-inner">
          ...
          <button className="text-zinc-400 hover:text-zinc-600 p-1" aria-label="Post options">
        ...
        {/* Post Text */}
        <div className="mt-3 text-[13.5px] leading-relaxed whitespace-pre-wrap break-words">
          {text ? (
            <>
              ...
              {shouldTruncate && (
                <button
                  ...
                  className="text-zinc-500 hover:text-sky-600 ml-1 font-medium hover:underline inline-block"
                >
                  ...see more
                </button>
              )}
            ...
            <span className="text-zinc-400 italic">Your LinkedIn post commentary will appear here...</span>
          )}
        </div>
      </div>
      ...
      {media && media.length > 0 && (
        <div className="border-t border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/40">
          {media.length === 1 ? (
            ...
            <div className="grid grid-cols-2 gap-0.5 max-h-72 overflow-hidden bg-zinc-200 dark:bg-zinc-800">
              {media.slice(0, 4).map((m, idx) => (
                <div key={m.id || idx} className="relative aspect-square overflow-hidden bg-zinc-900">
      ...
      <div className="px-3 py-2 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400 font-medium">
        <button className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
